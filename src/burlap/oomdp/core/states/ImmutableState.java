...
/**
 * An immutable state cannot be changed (nor subclassed). It is particularly
 * useful when memory management is crucial, as copies are only created when
 * actual changes are applied to an ImmutableState (copy on write). If many
 * objects are altered during a grounded action update, it is recommended to use
 * the bulk modification methods (replaceAllObjects, addAllObjects, etc).
 * 
 * @author Stephen Brawner, James MacGlashan
 * 
 */
public final class ImmutableState extends OOMDPState implements
	...
	public ImmutableState() {
		this.objectInstances = ImmutableList
				.copyOf(new ArrayList<ImmutableObjectInstance>());
		this.objectIndexByTrueClass = ImmutableList
				.copyOf(new ArrayList<TIntArrayList>());
	...
	public ImmutableState(State s) {
		...
		TObjectIntHashMap<String> objectMap = new TObjectIntHashMap<String>(
				size, 0.5f, -1);
		TObjectIntHashMap<String> objectClassMap = new TObjectIntHashMap<String>(
				s.getObjectClassesPresent().size() * 2, 0.5f, -1);

		List<ImmutableObjectInstance> immutableObjects = null;
		immutableObjects = this.createImmutableObjects(s.getAllObjects());
		List<ImmutableObjectInstance> objectInstances = this.createObjectLists(
				immutableObjects, objectMap, 0);
		...
		List<TIntArrayList> objectIndexByTrueClass = new ArrayList<TIntArrayList>(
				numberClasses);
		size /= 2;
		...
		this.addObjectListToList(this.objectInstances,
				this.numObservableObjects, numberClasses,
				objectIndexByTrueClass, objectClassMap);

		this.objectIndexByTrueClass = ImmutableList
				.copyOf(objectIndexByTrueClass);
		this.objectClassMap = objectClassMap;
		this.objectMap = objectMap;
		this.hashCode = 0;
	}

	public ImmutableState(List<ImmutableObjectInstance> objects,
		...
		this.addObjectListToList(this.objectInstances,
				...
				objectIndexByTrueClass, objectClassMap);
		this.objectIndexByTrueClass = ImmutableList
				.copyOf(objectIndexByTrueClass);// Collections.unmodifiableList();
		this.objectClassMap = objectClassMap;// Collections.unmodifiableMap();
		this.objectMap = objectMap;// Collections.unmodifiableMap();
		this.hashCode = 0;
	}

	public ImmutableState(ImmutableList<ImmutableObjectInstance> objects,
			TObjectIntHashMap<String> objectClassMap,
	...
	/**
	 * This method doesn't actually copy. This implementation only copies on
	 * write.
	 * 
	...
	public final ImmutableState replaceAndHash(
	...
	private final List<ImmutableObjectInstance> createImmutableObjects(
		...
		List<ImmutableObjectInstance> immutableObjects = new ArrayList<ImmutableObjectInstance>(
				objects.size());
		for (ObjectInstance object : objects) {
			ImmutableObjectInstance immutable = (object instanceof ImmutableObjectInstance) ? (ImmutableObjectInstance) object
					: new ImmutableObjectInstance(object);
			immutableObjects.add(immutable);
		}
		return immutableObjects;
	}

	private final List<ImmutableObjectInstance> createObjectLists(
			List<ImmutableObjectInstance> objectList,
			TObjectIntHashMap<String> objectMap, int offset) {
		List<ImmutableObjectInstance> objectInstances = new ArrayList<ImmutableObjectInstance>(
				objectList.size());
		for (ImmutableObjectInstance object : objectList) {
			...
			int displaced = objectMap.put(object.getName(),
					objectInstances.size() + offset);
			if (displaced >= 0) {
	...
	@Override
	public final ImmutableState addAllObjects(
	...
	public final ImmutableState removeAllObjects(
	...
	public final ImmutableState replaceObject(ObjectInstance objectToReplace,
		...
		if (!oldObjectName.equals(newObjectName)
				|| !objectToReplace.getClassName().equals(
						newObject.getClassName())) {
			throw new RuntimeException(
					"In order to replace, the objects must have the same name and class. Try remove and add instead");
		}
		...
		return new ImmutableState(ImmutableList.copyOf(objects),
				this.objectClassMap, this.objectIndexByTrueClass,
				this.objectMap, 0);
	}

	public final ImmutableState replaceAllObjects(
	...
	public final ImmutableState replaceAllObjectsUnsafe(
			List<ObjectInstance> objectsToAdd) {
		List<ImmutableObjectInstance> objects = new ArrayList<ImmutableObjectInstance>(
				objectsToAdd.size());
	...
	public boolean equals(Object other) {
		...
		Set<Integer> matchedObjects = new HashSet<Integer>(
				(int) (this.numTotalObjects() / 0.75) + 1);
		for (int i = 0; i < this.objectIndexByTrueClass.size(); i++) {
			...
			String oclass = this.getObject(objectIndices.get(0))
					.getObjectClass().name;
			String ooclass = "";
			...
			for (int j = 0; j < objectIndices.size(); j++) {
				...
				// otherwise if there is another object with a different name
				// but is equals
				boolean foundMatch = false;
	...
	private List<TIntArrayList> getPossibleBindingsIndicesGivenParamOrderGroups(
			String[] paramClasses, String[] paramOrderGroups) {
		...
		List<String> uniqueRenames = this
				.identifyUniqueClassesInParameters(Arrays
						.asList(paramOrderGroups));
		TIntArrayList uniqueParamClasses = new TIntArrayList(new TIntHashSet(
				paramClassIds));// this.identifyUniqueClassesInParameters(paramClassIds);
		Map<String, Integer> uniqueParamClassCounts = this
				.getUniqueClassCounts(uniqueRenames,
						Arrays.asList(paramOrderGroups));

		List<Integer> currentObjects = new ArrayList<Integer>();
		int initialSize = 1;
		// first make sure we have objects for each class parameter; if not
		// return empty list
		for (int i = 0; i < uniqueParamClasses.size(); i++) {
			int classId = uniqueParamClasses.get(i);
			// for(Integer classId : uniqueParamClasses){
			int n = this.getNumOccurencesOfClassInParameters(classId,
					paramClassIds);
		...
		List<TIntArrayList> resIndices = new ArrayList<TIntArrayList>(
				initialSize);
		...
		this.getPossibleRenameBindingsHelper(resIndices, currentBindingSets, 0,
				currentObjects, uniqueRenames, paramClassIds,
				Arrays.asList(paramOrderGroups), uniqueParamClassCounts,
				retainPredicate, combSet);
		return resIndices;
	...
	private void getPossibleRenameBindingsHelper(List<TIntArrayList> res,
			...
			Map<String, Integer> uniqueParamOrderCounts,
		...
		if (bindIndex == uniqueOrderGroups.size()) {
			...
			res.add(this.getBindingFromCombinationSet(currentBindingSets,
					uniqueOrderGroups, paramOrderGroups));
			return;
		}
		...
		Integer cId = this.parameterClassAssociatedWithOrderGroup(r,
				paramOrderGroups, paramClassIds);
		...
		this.addBindingCombination(res, currentBindingSets, bindIndex,
				...
				paramOrderGroups, uniqueParamOrderCounts, combList,
				retainPredicate, combSet);
	...
	/**
	 * Iterates through combinations. Modified code from:
	 * http://compprog.wordpress.com/tag/generating-combinations/
	 * 
	 * @param comb
	 *            the last combination of elements selected
	 * @param k
	 *            number of elements in any combination (n choose k)
	 * @param n
	 *            number of possible elements (n choose k)
	 * @return 0 when there are no more combinations; 1 when a new combination
	 *         is generated
	 */
	private int nextComb(int[] comb, int k, int n) {
		...
		/*
		 * comb now looks like (..., x, n, n, n, ..., n). Turn it into (..., x,
		 * x + 1, x + 2, ...)
		 */
		for (i = i + 1; i < k; i++) {
			comb[i] = comb[i - 1] + 1;
