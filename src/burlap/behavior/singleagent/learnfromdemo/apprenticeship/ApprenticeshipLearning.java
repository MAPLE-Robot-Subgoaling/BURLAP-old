...
/**
 * This algorithm will take expert trajectors and return a policy that models
 * them. It is an implementation of the algorithm described by Abbel and Ng [1].
 * Both the projection method and quadractic programming version are available.
 * 
 * 
 * 
 * 1. Abbeel, Peter and Ng, Andrew.
...
public class ApprenticeshipLearning {
	...
	/**
	 * Calculates the Feature Expectations given one demonstration, a feature
	 * mapping and a discount factor gamma
	 * 
	 * @param episodeAnalysis
	 *            An EpisodeAnalysis object that contains a sequence of
	 *            state-action pairs
	 * @param featureFunctions
	 *            Feature Mapping which maps states to features
	 * @param gamma
	 *            Discount factor gamma
	 * @return The Feature Expectations generated (double array that matches the
	 *         length of the featureMapping)
	 */
	public static double[] estimateFeatureExpectation(
			EpisodeAnalysis episodeAnalysis,
			StateToFeatureVectorGenerator featureFunctions, Double gamma) {
		return ApprenticeshipLearning.estimateFeatureExpectation(
				Arrays.asList(episodeAnalysis), featureFunctions, gamma);
	}

	/**
	 * Calculates the Feature Expectations given a list of demonstrations, a
	 * feature mapping and a discount factor gamma
	 * 
	 * @param episodes
	 *            List of expert demonstrations as EpisodeAnalysis objects
	 * @param featureFunctions
	 *            Feature Mapping which maps states to features
	 * @param gamma
	 *            Discount factor for future expected reward
	 * @return The Feature Expectations generated (double array that matches the
	 *         length of the featureMapping)
	 */
	public static double[] estimateFeatureExpectation(
		...
		for (EpisodeAnalysis episodeAnalysis : episodes) {
			for (int i = 0; i < episodeAnalysis.stateSequence.size(); ++i) {
				double[] fvi = featureFunctions
						.generateFeatureVectorFrom(episodeAnalysis.stateSequence
								.get(i));
	...
	/**
	 * Generates an anonymous instance of a reward function derived from a
	 * FeatureMapping and associated feature weights Computes (w^(i))T phi from
	 * step 4 in section 3
	 * 
	 * @param featureFunctions
	 *            The feature mapping of states to features
	...
	public static RewardFunction generateRewardFunction(
		...
		final FeatureWeights newFeatureWeights = new FeatureWeights(
				featureWeights);
		return new RewardFunction() {
			...
			public double reward(State state, GroundedAction a, State sprime) {
				...
				double sumReward = 0;
				double[] fv = newFeatureFunctions
						.generateFeatureVectorFrom(state);
	...
	public static State getInitialState(List<EpisodeAnalysis> episodes) {
		...
		EpisodeAnalysis randomEpisodeAnalysis = episodes.get(rando
				.nextInt(episodes.size()));
		return randomEpisodeAnalysis.getState(0);
	}

	/**
	 * Computes a policy that models the expert trajectorys included in the
	 * request object.
	 * 
	 * @param request
	 ...
	 * 
	 * @param request
	 *            contains all of the IRL request informaiton.
	 * @return the computed {@link Policy}
	 */
	private static Policy maxMarginMethod(ApprenticeshipLearningRequest request) {

		// Need to evaluate policies with trajectory lengths equal to that of
		// the demonstrated episodes
		...
		for (EpisodeAnalysis expertEpisode : expertEpisodes) {
			maximumExpertEpisodeLength = Math.max(maximumExpertEpisodeLength,
					expertEpisode.numTimeSteps());
		}
		...
		StateToFeatureVectorGenerator featureFunctions = request
				.getFeatureGenerator();
		List<double[]> featureExpectationsHistory = new ArrayList<double[]>();
		double[] expertExpectations = ApprenticeshipLearning
				.estimateFeatureExpectation(expertEpisodes, featureFunctions,
						request.getGamma());

		// (1b) Compute u^(0) = u(pi^(0))
		EpisodeAnalysis episodeAnalysis = policy.evaluateBehavior(request
				.getStartStateGenerator().generateState(), new UniformCostRF(),
				maximumExpertEpisodeLength);
		double[] featureExpectations = ApprenticeshipLearning
				.estimateFeatureExpectation(episodeAnalysis, featureFunctions,
						request.getGamma());
		...
		for (int i = 0; i < maxIterations; ++i) {
			...
			while (featureWeights == null) {
				featureWeights = solveFeatureWeights(expertExpectations,
						featureExpectationsHistory);
			}

			for (int z = 0; z < featureWeights.weights.length; z++) {
				DPrint.c(debugCodeRFWeights, z + ": "
						+ featureWeights.weights[z] + "; ");
			}
			DPrint.cl(debugCodeRFWeights, "");

			// (3) if t^(i) <= epsilon, terminate
			if (featureWeights == null
					|| Math.abs(featureWeights.getScore()) <= request
							.getEpsilon()) {
			...
			RewardFunction rewardFunction = ApprenticeshipLearning
					.generateRewardFunction(featureFunctions, featureWeights);

			// (4b) Compute optimal policy for pi^(i) give R
			planner.resetSolver();
			planner.solverInit(domain, rewardFunction, terminalFunction,
					request.getGamma(), stateHashingFactory);
			planner.planFromState(request.getStartStateGenerator()
					.generateState());
			...
			for (int j = 0; j < policyCount; ++j) {
				evaluatedEpisodes.add(policy.evaluateBehavior(request
						.getStartStateGenerator().generateState(),
						rewardFunction, maximumExpertEpisodeLength));
			}
			featureExpectations = ApprenticeshipLearning
					.estimateFeatureExpectation(evaluatedEpisodes,
							featureFunctions, request.getGamma());
			featureExpectationsHistory.add(featureExpectations);
	 ...
	 * feature expectation within epsilon of an expert's feature expectation. As
	 * described in: Abbeel, Peter and Ng, Andrew.
	 * "Apprenticeship Learning via Inverse Reinforcement Learning"
	 * 
	 * Takes in an expert's samples in some domain given some features, and
	 * returns a list of policies that can be evaluated algorithmically or
	 * manually.
	 * 
	...
	private static Policy projectionMethod(ApprenticeshipLearningRequest request) {
		...
		for (int j = 0; j < request.getPolicyCount(); ++j) {
			sampleEpisodes.add(policy.evaluateBehavior(request
					.getStartStateGenerator().generateState(),
					new UniformCostRF(), maximumExpertEpisodeLength));
		}
		double[] curFE = ApprenticeshipLearning.estimateFeatureExpectation(
				sampleEpisodes, featureFunctions, request.getGamma());
		...
		for (int i = 0; i < maxIterations; ++i) {
			...
			// THIS IS THE KEY DIFFERENCE BETWEEN THE MAXIMUM MARGIN METHOD AND
			// THE PROJECTION METHOD
			// On the first iteration, the projection is just set as the current
			// feature expectation
			if (lastProjFE == null) {
				newProjFE = curFE.clone();
			} else {
				newProjFE = projectExpertFE(expertExpectations, curFE,
						lastProjFE);
			}
			FeatureWeights featureWeights = getWeightsProjectionMethod(
					expertExpectations, newProjFE);
			tHistory[i] = featureWeights.getScore();
			DPrint.cl(debugCodeScore, "Score: " + tHistory[i]);
			lastProjFE = newProjFE; // don't forget to set the old projection to
									// the new one!
			...
			curFE = ApprenticeshipLearning.estimateFeatureExpectation(
					evaluatedEpisodes, featureFunctions, request.getGamma());
	 ...
	 * 
	 * @param expertExpectations
	 *            Feature Expectations calculated from the expert demonstrations
	 * @param featureExpectations
	 *            Feature History of feature expectations generated from past
	 *            policies
	...
	private static FeatureWeights solveFeatureWeights(
		...
		// We set the requirement that all feature expectations generated have
		// to be less than the expert
		...
		for (double[] expectations : featureExpectations) {
			...
			expertBasedWeightConstraints
					.add(new LinearMultivariateRealFunction(difference, 1));
		}

		// L2 norm of weights must be less than or equal to 1. So
		// P = Identity, except for the last entry (which cancels t).
		double[][] identityMatrix = Utils.createConstantDiagonalMatrix(
				weightsSize + 1, 1);
		identityMatrix[weightsSize][weightsSize] = 0;
		expertBasedWeightConstraints
				.add(new PSDQuadraticMultivariateRealFunction(identityMatrix,
						null, -0.5));

		OptimizationRequest optimizationRequest = new OptimizationRequest();
		optimizationRequest.setF0(objectiveFunction);
		optimizationRequest
				.setFi(expertBasedWeightConstraints
						.toArray(new ConvexMultivariateRealFunction[expertBasedWeightConstraints
								.size()]));
		...
		OptimizationResponse optimizationResponse = optimizer
