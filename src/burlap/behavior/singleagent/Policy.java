 ...
 * queried and perform common operations with the policy.
 * 
...
public abstract class Policy {
    ...
    public static double getProbOfActionGivenDistribution(State s,
	    GroundedAction ga, List<ActionProb> distribution) {
     ...
     * getAction method and getActionDistribution method, the subclass needs to
     * only define the getActionDistribution method and the getAction method can
     * simply call this method to return an action.
     * 
     * @param s
    ...
    private State followAndRecordPolicy(EpisodeAnalysis ea, State cur,
	    RewardFunction rf) {
	...
	if (ga.action.isPrimitive() || !this.evaluateDecomposesOptions) {
		...
		if (annotateOptionDecomposition) {
		    ...
		    // action taken
		    NullAction annotatedPrimitive = new NullAction(o.getName()
			    + "(" + ns + ")-" + cga.action.getName());
		    GroundedAction annotatedPrimitiveGA = new GroundedAction(
			    annotatedPrimitive, cga.params);
