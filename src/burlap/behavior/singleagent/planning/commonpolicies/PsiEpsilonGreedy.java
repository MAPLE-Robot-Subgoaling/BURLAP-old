...
/**
 * This class is an extension of PolicyBlocksPolicy that uses an additional psi value when 
 * determining which action to choose. 
 * 
 */
public class PsiEpsilonGreedy extends PolicyBlocksPolicy implements PlannerDerivedPolicy {
	...
	public AbstractGroundedAction getAction(State s) {
		...
		if(rand.nextDouble() <= psi && optionValues.size() > 0) {
			...
			if(rand.nextDouble() <= epsilon) {
				return primitiveValues.get(rand.nextInt(primitiveValues.size())).a;
			}
			else {
