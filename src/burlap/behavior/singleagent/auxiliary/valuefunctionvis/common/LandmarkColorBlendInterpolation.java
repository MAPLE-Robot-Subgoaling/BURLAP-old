...
/**
 * A {@link ColorBlend} instance that takes as input a set of "landmark"
 * {@link java.awt.Color} objects and corresponding values between them. For a
 * given value, it interpolates between the two landmarks values than lower
 * bound and upper bound the input. The interpolation is by default linear.
 * However, it can also be specified to first find the normalized distance
 * between the bounding landmark values and raise it to a power, which may
 * produce more obvious perceptual changes to small changes in value.
 * 
...
public class LandmarkColorBlendInterpolation implements ColorBlend {
    ...
    /**
     * Initializes the color blend with a power to raise the normalized distance
     * of values.
     * 
     * @param polyDegree
     */
     ...
     * 
     * @param val
     *            the value of the landmark
     * @param c
     *            the color for the landmark
     */
     ...
     *            the end value
     * @param t
     *            the normalized distance between s and e
