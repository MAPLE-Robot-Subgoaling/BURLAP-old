...
public class State {
    ...
    private List<ObjectInstance> objectInstances; // list of observable object
						  // instances that define the
						  // state
    private List<ObjectInstance> hiddenObjectInstances; // list of hidden object
							// instances that
							// facilitate domain
							// dynamics and infer
							// observable values
    private Map<String, ObjectInstance> objectMap; // map from object names to
						   // their instances

    private Map<String, List<ObjectInstance>> objectIndexByTrueClass; // map of
								      // object
								      // instances
								      // organized
								      // by
	...
	if (objectMap.containsKey(oname)) {
	    return; // don't add an object that conflicts with another object of
		    // the same name
	}
	...
	if (!objectMap.containsKey(oname)) {
	    return; // make sure we're removing something that actually exists
		    // in this state!
	}
     ...
     * 
     * @param i
     *            the index of the observable object instance to return
     * @return the observable object instance indexed at position i, or null if
     *         i > this.numObservableObjects()
     */
     ...
     * 
     * @param i
     *            the index of the hidden object instance to return
     * @return the hidden object instance indexed at position i, or null if i >
     *         this.numHiddenObjects()
     */
	...
	return new ArrayList<List<ObjectInstance>>(
		objectIndexByTrueClass.values());
    }

    /**
     * Returns a string representation of this state using only observable
     * object instances.
     * 
     * @return a string representation of this state using only observable
     *         object instances.
     */
     ...
     * 
     * @param a
     *            the action from which to generate GroundedAction objects.
	...
	List<List<String>> bindings = this
		.getPossibleBindingsGivenParamOrderGroups(
			a.getParameterClasses(), a.getParameterOrderGroups());
    ...
    /**
     * Returns a list of GroundedAction objects for all grounded actions that
     * can be generated from the provided list of actions.
     * 
     * @param actions
     *            the list of actions from which to generate GroudnedAction
     *            objects.
     * @return a list of GroundedAction objects for all grounded actions that
     *         can be generated from the provided list of actions.
     */
    ...
    /**
     * Returns all GroundedProp objects for the source propositional function pf
     * in this state.
     * 
     * @param pf
     *            the propositional function from which to generate GroundedProp
     *            objects.
     * @return all GroundedProp objects for the source propositional function pf
     *         in this state.
     */
	...
	List<List<String>> bindings = this
		.getPossibleBindingsGivenParamOrderGroups(
			pf.getParameterClasses(), pf.getParameterOrderGroups());
     ...
     * 
     * @param pf
     *            the propositional function to check
     * @return true if some GroundedProp of pf is true in this state; false
     *         otherwise
     */
	...
	List<String> uniqueRenames = this
		.identifyUniqueClassesInParameters(paramOrderGroups);
	List<String> uniqueParamClases = this
		.identifyUniqueClassesInParameters(paramClasses);
	...
	for (String oclass : uniqueParamClases) {
	    int n = this.getNumOccurencesOfClassInParameters(oclass,
		    paramClasses);
	...
	this.getPossibleRenameBindingsHelper(res, currentBindingSets, 0,
		objectInstances, uniqueRenames, paramClasses, paramOrderGroups);
    ...
    private void getPossibleRenameBindingsHelper(List<List<String>> res,
	    List<List<String>> currentBindingSets, int bindIndex,
	    List<ObjectInstance> remainingObjects,
	    List<String> uniqueOrderGroups, String[] paramClasses,
	    String[] paramOrderGroups) {

	if (bindIndex == uniqueOrderGroups.size()) {
	    // base case, put it all together and add it to the result
	    res.add(this.getBindngFromCombinationSet(currentBindingSets,
	...
	String c = this.parameterClassAssociatedWithOrderGroup(r,
		paramOrderGroups, paramClasses);
	List<ObjectInstance> cands = this.objectsMatchingClass(
		remainingObjects, c);
	...
	for (List<String> cb : combs) {
	    ...
	    List<List<String>> nextBinding = new ArrayList<List<String>>(
		    currentBindingSets.size());
	    for (List<String> prevBind : currentBindingSets) {
		nextBinding.add(prevBind);
	    }
	    nextBinding.add(cb);
	    List<ObjectInstance> nextObsReamining = this.objectListDifference(
		    remainingObjects, cb);

	    // recursive step
	    this.getPossibleRenameBindingsHelper(res, nextBinding,
		    bindIndex + 1, nextObsReamining, uniqueOrderGroups,
		    paramClasses, paramOrderGroups);
	...
	List<ObjectInstance> remaining = new ArrayList<ObjectInstance>(
		objects.size());
    ...
    private int getNumOccurencesOfClassInParameters(String className,
	    String[] paramClasses) {
	int num = 0;
    ...
    private String parameterClassAssociatedWithOrderGroup(String orderGroup,
	    String[] orderGroups, String[] paramClasses) {
	for (int i = 0; i < orderGroups.length; i++) {
    ...
    private List<ObjectInstance> objectsMatchingClass(
	    List<ObjectInstance> sourceObs, String cname) {
     ...
     */
    private List<String> getBindngFromCombinationSet(
	    List<List<String>> comboSets,
	    List<String> orderGroupAssociatedWithSet, String[] orderGroups) {
    ...
    private List<List<String>> getAllCombinationsOfObjects(
	    List<ObjectInstance> objects, int k) {
    ...
    private List<String> getListOfBindingsFromCombination(
	    List<ObjectInstance> objects, int[] comb) {
