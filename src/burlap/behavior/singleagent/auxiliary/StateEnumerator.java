 ...
 * for each possible state and the hashcode of a state cannot reliably give a
 * unique number. This class is used to take hashable states and assign them a
 * unique number. States can be enumerated iteratively on an as needed basis or
 * all reachable states from a source state can be queried to seed the list of
 * states. This class can also go in the reverse direction by returning the
 * state associated with an enumeration id, as long as that id has already been
 * assigned to a state.
 * 
...
public class StateEnumerator {
     ...
     * 
     * @param domain
     *            the domain of the states to be enumerated
    ...
    /**
     * Finds all states that are reachable from an input state and enumerates
     * them
     * 
     * @param from
     *            the state from which all reachable states should be searched
     */
    public void findReachableStatesAndEnumerate(State from) {
	Set<StateHashTuple> reachable = StateReachability
		.getReachableHashedStates(from, (SADomain) this.domain,
			this.hashingFactory);
    ...
    /**
     * Finds all states that are reachable from an input state and enumerates
     * them. Will not search from states that are marked as terminal states.
     * 
     * @param from
     *            the state from which all reachable states should be searched
     * @param tf
     *            the terminal function that prevents expanding from terminal
     *            states
     */
    public void findReachableStatesAndEnumerate(State from, TerminalFunction tf) {
	Set<StateHashTuple> reachable = StateReachability
		.getReachableHashedStates(from, (SADomain) this.domain,
			this.hashingFactory, tf);
	for (StateHashTuple sh : reachable) {
	    this.getEnumeratedID(sh);
	}
    }
     ...
     * 
     * @param s
     *            the state to get the enumeration id
    ...
    /**
     * Returns the state associated with the given enumeration id. A state must
     * have previously be associated with the input enumeration id, or a runtime
     * exception is thrown.
     * 
     * @param id
     *            the enumeration id
	...
	if (s == null) {
	    throw new RuntimeException(
		    "There is no state stored with the enumeration id: " + id);
	}
	return s;
    }
