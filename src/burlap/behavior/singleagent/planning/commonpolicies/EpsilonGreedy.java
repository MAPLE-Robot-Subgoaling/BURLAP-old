 ...
 * QComputable planner to be specified. With probability epsilon the policy will
 * return a random action (with uniform distribution over all possible action).
 * With probability 1 - epsilon the policy will return the greedy action. If
 * multiple actions tie for the highest Q-value, then one of the tied actions is
 * randomly selected.
 * 
...
public class EpsilonGreedy extends Policy implements PlannerDerivedPolicy {
	...
	for (QValue q : qValues) {
	    ...
	    ActionProb ap = new ActionProb(q.a, this.epsilon
		    * (1. / qValues.size()));
	    dist.add(ap);
	}
