...
/**
 * A domain generator for basic grid worlds. This domain generator allows for
 * the creation of arbitrarily sized grid worlds with user defined layouts. The
 * grid world supports classic north, south, east, west movement actions that
 * may be either deterministic or stochastic with user defined stochastic
 * failures. <br/>
 * The domain consists of only two object classes: an agent class and a location
 * class, each of which is defined by and x and y position. Locations also have
 * an attribute defining which type of location it is. The number of possible
 * types can be set using the {@link #setNumberOfLocationTypes(int)} method.
 * Setting location types may be useful if terminating "pits" and goal locations
 * exist in the world <br/>
 * Walls are not considered objects. Instead walls are considered part of the
 * transition dynamics. There are 2 types of walls supported. Walls that are
 * more like obstacles and occupy an entire cell of the map and 1D walls. 1D
 * walls are specified as either a horizontal wall on the north side of a cell
 * or a vertical wall on the east side of the wall. A cell may also have a 1D
 * north and east wall in it. The type of wall for each cell is specified by a
 * 2D int matrix provided to the constructor. Cells in the matrix with a 0 are
 * clear of any walls and obstacle; 1s indicate a full cell obstacle; 2s a 1D
 * north wall; 3s a 1D east wall; and 4s indicate that the cell has both a 1D
 * north wall and 1D east wall. <br/>
 * Note that if you change the stochastic transition dynamics or the map of the
 * domain generator *after* generating a domain with {@link #generateDomain()},
 * the previously generated domain will use the settings prior to its
 * generation. To use the new settings, you will need to generate a new domain
 * object. <br/>
 * There are five propositional functions supported: atLocation(agent,
 * location), wallToNorth(agent), wallToSouth(agent), wallToEast(agent), and
 * wallToWest(agent).
 * 
...
public class GridWorldDomain implements DomainGenerator {
	...
	/**
	 * Will return a state object with a single agent object and no location
	 * objects
	 * 
	 * @param d
	 ...
	 *            dimensions
	 * @param ax
	 *            the starting x position of the agent
	 * @param ay
	 *            the starting y position of the agent
	 * @return a state object with a single agent object and no location objects
	 */
	...
	/**
	 * Will return a state object with a single agent object and a single
	 * location object
	 * 
	 * @param d
	 *            the domain object that is used to specify the min/max
	 *            dimensions
	 * @return a state object with a single agent object and a single location
	 *         object
	 */
	public static State getOneAgentOneLocationState(Domain d) {
		...
		s.addObject(new MutableObjectInstance(d.getObjectClass(CLASSLOCATION),
				CLASSLOCATION + 0));
		s.addObject(new MutableObjectInstance(d.getObjectClass(CLASSAGENT),
				CLASSAGENT + 0));
	 ...
	 * 
	 * @param s
	 *            the state with the location object
	 * @param i
	 *            specifies which location object index to set
	 * @param x
	...
	/**
	 * Sets the i'th location object to the specified x and y position and
	 * location type.
	 * 
	 * @param s
	 *            the state with the location object
	 ...
	 *            the y position of the location
	 * @param locType
	 *            the location type of the location
	 */
	...
	protected State move(State s, int xd, int yd, int[][] map) {
		...
		if (nx < 0 || nx >= map.length || ny < 0 || ny >= map[0].length
				|| map[nx][ny] == 1
				|| (xd > 0 && (map[ax][ay] == 3 || map[ax][ay] == 4))
				|| (xd < 0 && (map[nx][ny] == 3 || map[nx][ny] == 4))
				|| (yd > 0 && (map[ax][ay] == 2 || map[ax][ay] == 4))
