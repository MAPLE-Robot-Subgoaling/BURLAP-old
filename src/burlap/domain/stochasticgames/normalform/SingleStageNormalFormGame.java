...
/**
 * This stochastic game domain generator provides methods to create N-player
 * single stage games. There is only one object class, a "player" object class,
 * which has one attribute, its player number. A state consists simply of an
 * object instance for each player. Different players maybe have different
 * numbers of available actions and the actions available to each player may
 * have different names. The SingleAction's are created such that a player can
 * only execute a single action if that action is available to that player.
 * Therefore, when agents joint a world for one of these games, their
 * {@link burlap.oomdp.stochasticgames.SGAgentType} can be specified to have all
 * of the possible actions, because they will only be able to execute the
 * relevant ones. The method {@link #getAgentTypeForAllPlayers(SGDomain)} will
 * return such an {@link burlap.oomdp.stochasticgames.SGAgentType} class that
 * can be used for all agents.
 * <p/>
 * In addition to this generator being able to return the domain object, it may
 * also be used to return the corresponding joint reward function. The method
 * {@link #getRepatedGameActionModel()} will return a joint action mode that
 * always returns to the same state, which can be used for repeated game
 * playing.
 * <p/>
 * This class also provides static methods for returning generators for a number
 * of classic bimatrix games: prisoner's dilemma, chicken, hawk dove, battle of
 * the sexes 1, battle of the sexes 2, matching pennies, and stag hunt.
 * <p/>
 * This class also has a method for streamlining the world creation process so
 * that repeated games (or single shot games) can be easily played in the
 * constructed game. For this use either the
 * {@link #createRepeatedGameWorld(burlap.oomdp.stochasticgames.SGAgent...)} or
 * {@link #createRepeatedGameWorld(SGDomain, burlap.oomdp.stochasticgames.SGAgent...)}
 * method. The former method will create an new domain instance using the
 * {@link #generateDomain()} method; the latter will use an already generated
 * version of the domain that you provide to it.
 * <p/>
 * Finally, this class's payout function definition (and other properties) can
 * be modified without affecting previously generated domains or payout
 * functions, allowing it to be reused multiple times.
 * 
...
public class SingleStageNormalFormGame implements DomainGenerator {
	 ...
	 * names for each row/column will take the form of:
	 * DEFAULTBIMATRIXACTIONBASENAMEi where i is the row/column index. More
	 * specifically, it will be "actioni", since DEFAULTBIMATRIXACTIONBASENAME =
	 * "action"
	 */
	...
	/**
	 * The pay out function for each player. This each element of the array
	 * takes {@link StrategyProfile} objects as input to return the pay out for
	 * the player at the given index in the array.
	 */
	...
	/**
	 * A construtor for a bimatrix game where the row player payoffs and colum
	 * player payoffs are provided in two different 2D double matrices. The
	 * action names for each row/column will be named "action0" ... "actionN"
	 * where N is the maximum number of rows/columns.
	 * 
	 * @param rowPayoff
	 *            the payoff matrix for the row player
	 * @param colPayoff
	 *            the payoff matrix for the column player
	 */
	public SingleStageNormalFormGame(double[][] rowPayoff, double[][] colPayoff) {

		if (rowPayoff.length != colPayoff.length
				|| rowPayoff[0].length != colPayoff[0].length) {
	...
	/**
	 * A constructor for games with a symmetric number of actions for each
	 * player. The payoffs will not be set.
	 * 
	 * @param actionSets
	 *            an nxm matrix specifying the m actions names for each of the n
	 *            players. actionSets[i][j] specifies the jth action name for
	 *            player i
	 */
	public SingleStageNormalFormGame(String[][] actionSets) {
		this.nPlayers = actionSets.length;
	...
	/**
	 * Returns the action index of the action named <code>actionName</code> of
	 * player <code>pn</code>
	 * 
	 * @param pn
	 *            the player number
	 ...
	 * @return the action index of the action named <code>actionName</code> of
	 *         player <code>pn</code>
	 */
	...
	/**
	 * Creates a world instance for this game in which the provided agents join
	 * in the order they are passed.
	 * 
	 * @param agents
	 *            the agents to join the created world.
	...
	/**
	 * Creates a world instance for this game in which the provided agents join
	 * in the order they are passed. This object uses the provided domain
	 * instance generated from this object instead of generating a new one.
	 * 
	 * @param agents
	 *            the agents to join the created world.
	 * @return a world instance with the provided agents having already joined.
	...
	public World createRepeatedGameWorld(SGDomain domain, SGAgent... agents) {
		...
		// grab the joint reward function from our bimatrix game in the more
		// general BURLAP joint reward function interface
		JointReward jr = this.getJointRewardFunction();

		// game repeats forever unless manually stopped after T times.
		TerminalFunction tf = new NullTermination();

		// set up the initial state generator for the world, which for a
		// bimatrix game is trivial
		SGStateGenerator sg = new ConstantSGStateGenerator(
				SingleStageNormalFormGame.getState(domain));

		// agent type defines the action set of players and OO-MDP class
		// associated with their state information
		// in this case that's just their player number. We can use the same
		// action type for all players, regardless of wether
		// each agent can play a different number of actions, because the
		// actions have preconditions that prevent a player from taking actions
		// that don't belong to them.
		SGAgentType at = SingleStageNormalFormGame
				.getAgentTypeForAllPlayers(domain);

		// create a world to synchronize the actions of agents in this domain
		// and record results
	 ...
	 * this game.
	 * 
	 * @return a {@link burlap.oomdp.stochasticgames.JointReward} function for
	 *         this game.
	 */
	public JointReward getJointRewardFunction() {
		return new SingleStageNormalFormJointReward(this.nPlayers,
				ActionNameMap
						.deepCopyActionNameMapArray(this.actionNameToIndex),
				AgentPayoutFunction.getDeepCopyOfPayoutArray(this.payouts));
	}
