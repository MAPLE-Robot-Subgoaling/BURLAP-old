...
/**
 * This class specifies which action each agent took in a world. The class is
 * backed by a Map from agent names to the
 * {@link burlap.oomdp.stochasticgames.agentactions.GroundedSGAgentAction} taken
 * by that respective agent. The
 * {@link burlap.oomdp.stochasticgames.agentactions.GroundedSGAgentAction}
 * objects of this class can also be iterated over.
 * 
 * @author James MacGlashan
 * 
 */
public class JointAction implements AbstractGroundedAction,
		Iterable<GroundedSGAgentAction> {

	protected static void allJointActionsHelper(
			List<List<GroundedSGAgentAction>> individualActionChoices, int i,
			LinkedList<GroundedSGAgentAction> currentSelections,
			List<JointAction> allJointActions) {
		...
		List<GroundedSGAgentAction> agentsChoices = individualActionChoices
				.get(i);
		for (GroundedSGAgentAction gsa : agentsChoices) {
			currentSelections.push(gsa);
			allJointActionsHelper(individualActionChoices, i + 1,
					currentSelections, allJointActions);
			currentSelections.pop();
		}
	}

	public static List<JointAction> getAllJointActions(State s,
			List<SGAgent> agents) {

		// get all possible individual choices
		List<List<GroundedSGAgentAction>> individualActionChoices = new ArrayList<List<GroundedSGAgentAction>>(
				agents.size());
		for (SGAgent agent : agents) {
			List<GroundedSGAgentAction> gsas = SGAgentAction
					.getAllApplicableGroundedActionsFromActionList(s,
							agent.worldAgentName, agent.agentType.actions);
		...
		allJointActionsHelper(individualActionChoices, 0,
				new LinkedList<GroundedSGAgentAction>(), allJointActions);
	...
	/**
	 * Adds all {@link GroundedSGAgentAction} objects in a list to this joint
	 * action.
	 * 
	 * @param actions
	 *            the actions to add to this joint action.
	 */
	 ...
	 * 
	 * @param agentName
	 *            the name of the agent whose taken action is to be returned.
	...
	public boolean equals(Object o) {
		...
		Iterator<Map.Entry<String, GroundedSGAgentAction>> taIter = this.actions
				.entrySet().iterator();
		Iterator<Map.Entry<String, GroundedSGAgentAction>> oaIter = oja.actions
				.entrySet().iterator();
	...
	/**
	 * Returns a list of the actions in this joint action. Modifying the
	 * returned list does not modify the structure of this joint action.
	 * 
	...
	/**
	 * Returns a list of the names of all agents who are represented in this
	 * joint action.
	 * 
	 * @return a list of the names of all agents who are represented in this
	 *         joint action.
	 */
	...
	public String[] getParametersAsString() {
		...
		int i = 0;
		for (Map.Entry<String, GroundedSGAgentAction> e : this.actions
				.entrySet()) {
			vals[i] = e.toString();
			i++;
		}
	...
	/**
	 * Returns a string representation of this joint aciton without including
	 * the parameters of any parameterized actions. This method can be useful
	 * for generating hash codes since two grounded single actions with
	 * different parameter orders may be the same action if the parameters
	 * belong to the same order group.
	 * 
	 * @return a string representation of this joint aciton without including
	 *         the parameters of any parameterized actions
	 */
	...
	public AbstractGroundedAction translateParameters(State sourceState,
			State targetState) {
		...
		for (GroundedSGAgentAction aa : this.actions.values()) {
			if (aa instanceof AbstractObjectParameterizedGroundedAction
					&& ((AbstractObjectParameterizedGroundedAction) aa)
		...
		for (GroundedSGAgentAction gsa : this.actions.values()) {
			GroundedSGAgentAction ngsa = (GroundedSGAgentAction) AbstractObjectParameterizedGroundedAction.Helper
					.translateParameters(gsa, sourceState, targetState);
			nja.addAction(ngsa);
		}
