...
/**
 * This class extends the OOMDP planner to define a class of planners that
 * compute state value functions using the tabular Bellman update, such as
 * ValueIteraiton. It defines data members for storing hashed transition
 * dynamics (so that they can be quickly retrieved without multiple calls to the
 * action transition generation) and a map from states to their values. It also
 * adds support for the QComputable planner which can return Q-values by using
 * the transition dynamics and the stored value function.
 * <p/>
 * Note that by default ValueFunction planners will cache the transition
 * dynamics so that they do not have to be procedurally generated by the
 * {@link burlap.oomdp.singleagent.Action}. Transition dynamic caching can be
 * disable by calling the {@link #toggleUseCachedTransitionDynamics(boolean)}
 * method. This may be desirable if the transition dynamics are expected to
 * change with time, such as when the model is being learned in model-based RL.
 * 
 * @author James MacGlashan
 * 
 */
public abstract class ValueFunctionPlanner extends OOMDPPlanner implements
	QComputablePlanner {
    ...
    /**
     * Common init method for ValueFunction Planners. This will automatically
     * call the OOMDPPLanner init method.
     * 
     * @param domain
     ...
     */
    public void VFPInit(Domain domain, RewardFunction rf, TerminalFunction tf,
     ...
     * @return the Q-value
     */
    protected QValue getQ(StateHashTuple sh, GroundedAction a,
	...
	if (this.useCachedTransitions) {
	    ...
	    List<GroundedAction> gas = sh.s
		    .getAllGroundedActionsFor(this.actions);
	    for (GroundedAction ga : gas) {
	...
	if (ga.action instanceof Option) {
	    ...
	    List<TransitionProbability> tps = ga.action.getTransitions(sh.s,
		    ga.params);
	    for (TransitionProbability tp : tps) {
		double vp = this.value(tp.s);
