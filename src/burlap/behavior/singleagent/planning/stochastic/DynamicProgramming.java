...
/**
 * A class for performing dynamic programming operations: updating the value
 * function using a Bellman backup. It defines data members for storing hashed
 * transition dynamics (so that they can be quickly retrieved without multiple
 * calls to the action transition generation) and a map from states to their
 * values. It also implements {@link burlap.behavior.valuefunction.QFunction}
 * which can return Q-values by using the transition dynamics and the stored
 * value function.
 * <p/>
 * Note that by default
 * {@link burlap.behavior.singleagent.planning.stochastic.DynamicProgramming}
 * instances will cache the transition dynamics so that they do not have to be
 * procedurally generated by the {@link burlap.oomdp.singleagent.Action}.
 * Transition dynamic caching can be disable by calling the
 * {@link #toggleUseCachedTransitionDynamics(boolean)} method. This may be
 * desirable if the transition dynamics are expected to change with time, such
 * as when the model is being learned in model-based RL.
 * 
 * @author James MacGlashan
 * 
 */
public class DynamicProgramming extends MDPSolver implements ValueFunction,
		QFunction {

	/**
	 * This class is used to store tabular value function values that can be
	 * manipulated with the {@link DynamicProgramming} methods. It has no
	 * planning method defined and will throw a runtime exception if you try to
	 * call it. When you pass it a seed value function (represented as a
	 * {@link java.util.Map}), it copies the values into its internal stored
	 * value function so that changes to the original value function may be made
	 * without affecting this objects values.
	 */
	public static class StaticVFPlanner extends DynamicProgramming {

		/**
		 * Initializes. The source value function will be *copied* into this
		 * objects value function so that changes to the source will not affect
		 * this object. The action list will also be copied.
		 * 
		 * @param domain
		 *            the planning domain
		 * @param rf
		 ...
		 * @param allActions
		 *            the set of actions for computing Q-values
		 * @param srcValueFunction
		 *            the source value function to copy.
		 */
		public StaticVFPlanner(Domain domain, RewardFunction rf, double gamma,
				HashableStateFactory hashingFactory, List<Action> allActions,
				Map<HashableState, Double> srcValueFunction) {
			this.DPPInit(domain, rf, new NullTermination(), gamma,
					hashingFactory);
			...
			for (Map.Entry<HashableState, Double> e : srcValueFunction
					.entrySet()) {
	...
	/**
	 * A boolean toggle to indicate whether the transition dynamics should
	 * cached in a hashed data structure for quicker access, or computed as
	 * needed by the Action methods. The default is true, to cache the
	 * transition dynamics. However, this value should be set to false if it is
	 * expected that the transition dynamics can change over time which might be
	 * the case in model learning scenarios.
	 */
	protected boolean useCachedTransitions = true;

	/**
	 * A data structure for storing the hashed transition dynamics from each
	 * state, if this algorithm is set to use them.
	 */
	...
	/**
	 * The value function initialization to use; defaulted to an initialization
	 * of 0 everywhere.
	 */
	...
	/**
	 * Computes the Q-value using the uncached transition dynamics produced by
	 * the Action object methods. This computation *is* compatible with
	 * {@link burlap.behavior.singleagent.options.Option} objects.
	 * 
	 * @param sh
	 *            the given state
	 * @param ga
	...
	protected double computeQ(HashableState sh, GroundedAction ga) {
		...
		if (ga.action instanceof Option) {
			...
			for (TransitionProbability tp : tps) {
				...
				// note that for options, tp.p will be the *discounted*
	...
	/**
	 * Returns the Q-value for a given set and the possible transitions from it
	 * for a given action. This computation *is* compatible with
	 * {@link burlap.behavior.singleagent.options.Option} objects.
	 * 
	 * @param s
	 *            the given state
	 * @param trans
	 *            the given action transitions
	 ...
	 */
	public void DPPInit(Domain domain, RewardFunction rf, TerminalFunction tf,
	...
	/**
	 * Returns the stored action transitions for the given state. If the action
	 * transitions are not already cached and this object is set to use caching,
	 * then they will be cached.
	 * 
	 * @param sh
	 *            the input state from which to get the transitions
	...
	protected List<ActionTransitions> getActionsTransitions(HashableState sh) {
		...
		if (allTransitions == null) {
			...
			for (GroundedAction ga : gas) {
				ActionTransitions at = new ActionTransitions(sh.s, ga,
						hashingFactory);
	...
	/**
	 * This method will return all states that are stored in this planners value
	 * function.
	 * 
	...
	public DynamicProgramming getCopyOfValueFunction() {
		...
		dpCopy.DPPInit(this.domain, this.rf, this.tf, this.gamma,
				this.hashingFactory);
	...
	/**
	 * Gets a Q-Value for a hashed state, grounded action, and object instance
	 * matching from the hashed states an internally stored hashed transition
	 * dynamics. If the input state is a terminal state, then the value 0 is
	 * returned.
	 * 
	 * @param sh
	 *            the input state
	 * @param a
	 *            the action to get the Q-value for
	 * @param matching
	 *            the object instance matching from sh to the corresponding
	 *            state stored in the value function
	 * @return the Q-value
	 */
	protected QValue getQ(HashableState sh, GroundedAction a,
	...
	public List<QValue> getQs(State s) {
		...
		for (Action a : actions) {
			// List <GroundedAction> applications =
			// s.getAllGroundedActionsFor(a);
			List<GroundedAction> applications = a
					.getAllApplicableGroundedActions(s);
			for (GroundedAction ga : applications) {
				if (matching == null
						&& ga instanceof AbstractObjectParameterizedGroundedAction) {
					matching = sh.s.getObjectMatchingTo(indexSH.s, false);
				}
	 ...
	 * @param s
	 *            the state to check
	 * @return true if the the value for the given state has already been
	...
	/**
	 * Options need to to have transition probabilities computed and keep track
	 * of the possible termination states using as hashed data structure. This
	 * method tells each option which state hashing factory to use.
	 */
	 ...
	 * 
	 * @param sh
	 *            the hashed state on which to perform the Bellman update.
	...
	/**
	 * Performs a Bellman value function update on the provided state. Results
	 * are stored in the value function map as well as returned. If this object
	 * is set to used cached transition dynamics and the transition dynamics for
	 * this state are not cached, then they will be created and cached.
	 * 
	 * @param s
	 *            the state on which to perform the Bellman update.
	 * @return the new value of the state.
	 */
	...
	/**
	 * Performs a fixed-policy Bellman value function update (i.e., policy
	 * evaluation) on the provided (hashed) state. Results are stored in the
	 * value function map as well as returned. If this object is set to used
	 * cached transition dynamics and the transition dynamics for this state are
	 * not cached, then they will be created and cached.
	 * 
	 * @param sh
	 *            the hashed state on which to perform the Bellman update.
	 * @param p
	...
	protected double performFixedPolicyBellmanUpdateOn(HashableState sh,
		...
		double weightedQ = 0.;
		List<ActionProb> policyDistribution = p
		...
		if (this.useCachedTransitions) {
			...
			for (ActionTransitions at : transitions) {
				...
				double policyProb = Policy.getProbOfActionGivenDistribution(
						at.ga, policyDistribution);
				if (policyProb == 0.) {
			...
			for (GroundedAction ga : gas) {
				...
				double policyProb = Policy.getProbOfActionGivenDistribution(ga,
						policyDistribution);
				if (policyProb == 0.) {
					continue; // doesn't contribute
				}
	...
	/**
	 * Performs a fixed-policy Bellman value function update (i.e., policy
	 * evaluation) on the provided state. Results are stored in the value
	 * function map as well as returned. If this object is set to used cached
	 * transition dynamics and the transition dynamics for this state are not
	 * cached, then they will be created and cached.
	 * 
	 * @param s
	 *            the state on which to perform the Bellman update.
	 * @param p
	 ...
	 * 
	 * @param vfInit
	 *            the object that defines how to initializes the value function.
	 */
	public void setValueFunctionInitialization(
			ValueFunctionInitialization vfInit) {
		this.valueInitializer = vfInit;
	}

	/**
	 * Sets whether this object should cache hashed transition dynamics for each
	 * for faster look up, or whether to procedurally generate the transition
	 * dynamics as needed from the {@link burlap.oomdp.singleagent.Action}
	 * objects. Letting the transition dynamics be procedurally generated may be
	 * useful if the transition dynamics can change over the time such as when
	 * using a learned model.
	 * 
	 * @param useCachedTransitions
	 *            true if the transition dynamics should be cached and stored;
	 *            false if they should always be procedurally generated from the
	 *            {@link burlap.oomdp.singleagent.Action} objects.
	 */
	...
	/**
	 * Returns the value function evaluation of the given hashed state. If the
	 * value is not stored, then the default value specified by the
	 * ValueFunctionInitialization object of this class is returned.
	 * 
	 * @param sh
	 *            the hashed state to evaluate.
