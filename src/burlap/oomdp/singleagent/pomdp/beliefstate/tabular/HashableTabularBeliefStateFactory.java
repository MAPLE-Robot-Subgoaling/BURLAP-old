...
/**
 * A {@link burlap.oomdp.statehashing.HashableStateFactory} for
...
public class HashableTabularBeliefStateFactory implements HashableStateFactory {
	...
	public static class HashableTabularBeliefState extends HashableState {
		...
		public int hashCode() {
			...
			for (Map.Entry<Integer, Double> e : ((TabularBeliefState) this.s).beliefValues
					.entrySet()) {
				int entryHash = 31 * e.getKey().hashCode()
						+ e.getValue().hashCode();
	...
	public HashableState hashState(State s) {
		...
		if (!(s instanceof TabularBeliefState)) {
			throw new RuntimeException(
					"Cannot generate HashableState for input state, because it is a "
							+ s.getClass().getName()
							+ " instance and HashableTabularBeliefStateFactory only hashes TabularBeliefState instances.");
		}
