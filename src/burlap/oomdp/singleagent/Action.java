...
/**
 * Abstract class for defining what happens when an action is executed in a
 * state. The method getTransitions(State s, String [] params) is what defines
 * the transition dynamics of the MDP for this action. If this method is not
 * overridden by subclasses, then the default behavior is to assume
 * deterministic transition dynamics which are produced by sampling the
 * performAction(State s, String [] params) method and setting its returned
 * state as having a transition probability of 1. If the domain being created is
 * only going to be used planning/learning algorithms that require a generative
 * model, rather than the fully enumerated transition dynamics, then the
 * getTransitions(State s, String [] params) does not need to be defined, but
 * for full robustness it should be.
 * 
 * Action objects may also be defined to require object parameters (which must
 * adhere to a type). Parameters can also have parameter order groups specified
 * if there is effect symmetry when changing the order of the parameters. That
 * is, if you swapped the parameter assignments for parameters in the same order
 * group, the action would have the same effect. However, if you swapped the
 * parameter assignments of two parameters in different order groups, the action
 * would have a different effect. For more information on parameter order
 * groups, see its discussion in the
 * {@link burlap.oomdp.core.PropositionalFunction} class description.
 * 
...
public abstract class Action {
     ...
     *            a comma delineated String of the names of the object classes
     *            to which bound parameters must belong
     */
    ...
    protected void init(String name, Domain domain, String[] parameterClasses,
	    String[] parameterOrderGroups) {
     ...
     * Returns true if this action can be applied in this specified state with
     * the specified parameters. Default behavior is that an action can be
     * applied in any state, but this will need be overridden if that is not the
     * case.
     * 
     * @param s
     ...
     * modified. The method will return a copy of the input state if the action
     * is not applicable in state s with parameters params.
     * 
     * @param s
	...
	if (!this.applicableInState(s, params)) {
	    return resultState; // can't do anything if it's not applicable in
				// the state so return the current state
	}

	resultState = performActionHelper(resultState, params);

	for (ActionObserver observer : this.actionObservers) {
	    observer.actionEvent(resultState, new GroundedAction(this, params),
		    resultState);
	}
     ...
     * probability. By default, this method assumes that transition dynamics are
     * deterministic and it returns a list with a single TransitionProbability
     * with probability 1 whose state is determined by querying the
     * {@link #performAction(State, String [])} method. If the transition
     * dynamics are stochastic, then the analogous method
     * {@link #getTransitions(State, String [])} needs to be overridden.
     * 
     * @param s
     ...
     */
    public final List<TransitionProbability> getTransitions(State s,
	    String params) {
     ...
     * with probability 1 whose state is determined by querying the
     * {@link #performAction(State, String [])} method. If the transition
     * dynamics are stochastic, then this method needs to be overridden.
     * 
     * @param s
     *            the state from which the transition probabilities when
     *            applying this action will be returned.
     ...
     * modified in this method since the parent method first copies the input
     * state to pass to this helper method. The resulting state (which may be s)
