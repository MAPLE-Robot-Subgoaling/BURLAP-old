...
/**
 * A minmax operator. This operator is useful for zero sum two player games. If
 * there are more than two players in the game, a runtime exception will be
 * thrown. Before solving the minmax strategy, the Q-values are transformed into
 * a minmax game. Then the resulting minmax strategy is used to compute the
 * expected "payoff" using the true Q-values of the query agent, which is then
 * returned as the new Q-value.
 * 
...
public class MinMaxQ implements SGBackupOperator {
	...
	public double performBackup(State s, String forAgent,
		...
		double[][] truePayout1 = new double[forAgentGSAs.size()][otherAgentGSAs
				.size()];
		...
		// we can use true payoff for player 1 for both players, because we're
		// ignoring the payout for the second player.
		double expectedpayoffforPlayer1 = GeneralBimatrixSolverTools
				.expectedPayoffs(truePayout1, truePayout1, forAgentStrat,
						otherAgentStrat)[0];
