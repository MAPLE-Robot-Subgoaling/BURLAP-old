 ...
 * actions for each of them to take in specific states. States are conveyed to
 * the user through a text description in the terminal and the user specifies
 * actions by typing the actions into the terminal, one line at a time for each
 * agent's action. The command format is
 * "agentName:action parameter1 parameter2" and so on for as many parameters as
 * there may be (or none if an action takes no parameters).
 * <p/>
 * When all of the agent's actions have been input, they may be executed by
 * typing "##"
 * <p/>
 * Shorthand names for actions names may be provided.
 * <p/>
 * The command #reset causes the state to reset to the initial state provided to
 * the explorer. Other special commands to modify the state may also be
 * specified. The full syntax is printed to the terminal when the explorer
 * begins. <br/>
 * <br/>
 * This explorer can also track a reward function and terminal function and
 * print them to the screen, which can be set with the
...
public class SGTerminalExplorer {
	 ...
	 * use the constructor
	 * {@link #SGTerminalExplorer(burlap.oomdp.stochasticgames.SGDomain)}.
	 * 
	 * @param domain
	 *            the domain which will be explored
	 * @param jam
	 *            the action model definition transition dynamics
	 */
	@Deprecated
	 ...
	 * with them, making the constructor parameter for it unnecessary. Instead
	 * use the constructor
	 * {@link #SGTerminalExplorer(burlap.oomdp.stochasticgames.SGDomain, java.util.Map)}
	 * .
	 * 
	 * @param domain
	 *            the domain which will be explored
	 ...
	 *            the action model definition transition dynamics
	 * @param ash
	 *            a map from shorthand names to full action names that can be
	 *            typed instead of the full action names
	 */
	@Deprecated
	public SGTerminalExplorer(SGDomain domain, JointActionModel jam,
	...
	/**
	 * Initializes the explorer with a domain and action model and shorthand
	 * names for actions
	 * 
	 * @param domain
	 *            the domain which will be explored
	 ...
	 * 
	 * @param shortHand
	 *            the shorthand for the action
	 * @param action
	 *            the full name of the action that the shorthand represents
	 */
	 ...
	 * 
	 * @param s
	 *            the state from which to start exploring.
	 */
	public void exploreFromState(State s) {
		...
		System.out
				.println("Special Command Syntax:\n"
						...
						+ "    #reset\n\n");
		System.out
				.println("Set agent actions with the notation \"agentName:action actionParam1 actionParam2 ...\"");
		System.out
				.println("Type \"##\" to commit and execute the current joint action.\n\n");
			...
			try {
				...
				if (line.equals("#reset")) {
					...
					if (this.rewardFunction != null) {
						Map<String, Double> reward = rewardFunction.reward(s,
								curJointAction, ns);
					...
					if (sa == null) {
						...
						if (sa.applicableInState(s, gsa)) {
							System.out.println("Setting action: " + agentName
									+ "::" + actionName);
							curJointAction.addAction(gsa);
						} else {
							System.out
									.println(gsa.toString()
