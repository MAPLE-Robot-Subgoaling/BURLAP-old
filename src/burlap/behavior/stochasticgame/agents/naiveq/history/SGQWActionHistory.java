 ...
 * augments states with the actions each agent took in n previous time steps.
 * 
 * <p/>
...
public class SGQWActionHistory extends SGQLAgent {
    ...
    public SGQWActionHistory(SGDomain d, double discount, double learningRate,
	    StateHashFactory hashFactory, int historySize, int maxPlayers,
	...
	Attribute histAID = new Attribute(augmentingDomain, ATTHAID,
		Attribute.AttributeType.DISC);
	histAID.setDiscValuesForRange(0, actionMap.maxValue(), 1); // maxValue
								   // is when it
								   // the action
								   // is
								   // undefined
	...
	if (this.hashFactory instanceof DiscreteStateHashFactory) {
	    ((DiscreteStateHashFactory) this.hashFactory)
		    .setAttributesForClass(CLASSHISTORY, attsForHistoryHashing);
	}
    }
	...
	qe.q = qe.q + this.learningRate.pollLearningRate(s, myAction)
		* (r + (this.discount * maxQ) - qe.q);
    ...
    protected ObjectInstance getHistoryObjectInstanceForAgent(
	    GroundedSingleAction gsa, int h) {
