 ...
 * approximation. Unlike,
 * {@link burlap.behavior.singleagent.vfa.rbf.RBFFeatureDatabase}, this version
 * takes as input a
 * {@link burlap.behavior.singleagent.vfa.StateToFeatureVectorGenerator} object
 * so that states are first converted to a double array and then provided to
 * {@link burlap.behavior.singleagent.vfa.rbf.FVRBF} objects that operate
 * directly on the feature vector of the state. This can improve performance
 * over the {@link burlap.behavior.singleagent.vfa.rbf.RBFFeatureDatabase} which
 * may require generating a feature vector for states for each RBF unit.
 * Therefore, for RBFs like ones that use Gaussian functions over Euclidean
 * distance metrics, this class is recommended. However, the standard
 * {@link burlap.behavior.singleagent.vfa.rbf.RBFFeatureDatabase} may have the
 * advantage of using RBF units that exploit the OO-MDP state representation for
 * distance measures.
 * 
...
public class FVRBFFeatureDatabase implements FeatureDatabase {
	...
	/**
	 * Specifies whether an offset RBF unit with a constant response value is
	 * included in the feature set.
	 */
	protected boolean hasOffset;

	/**
	 * A map for returning a multiplier to the number of RBF state features for
	 * each action. Effectively this ensures a unique feature ID for each RBF
	 * for each action.
	 */
	protected Map<GroundedAction, Integer> actionFeatureMultiplier = new HashMap<GroundedAction, Integer>();

	/**
	 * The next action RBF size multiplier to use for the next newly seen
	 * action.
	 */
	 ...
	 * 
	 * @param fvGen
	 *            the state feature vector generator to use to generate the
	 *            feature vectors provided to RBFs
	 * @param hasOffset
	 *            if true, an offset RBF unit with a constant response value is
	 *            included in the feature set.
	 */
	public FVRBFFeatureDatabase(StateToFeatureVectorGenerator fvGen,
			boolean hasOffset) {
	...
	protected int getActionMultiplier(GroundedAction ga) {
		...
		if (ga instanceof AbstractObjectParameterizedGroundedAction) {
			throw new RuntimeException(
					"RBF Feature Database does not support AbstractObjectParameterizedGroundedActions.");
		}
