...
public class SGQFactory implements AgentFactory {
     ...
     *            The state hashing factory the Q-learning algorithm will use
     */
    public SGQFactory(SGDomain domain, double discount, double learningRate,
	    double defaultQ, StateHashFactory stateHash) {
	this.domain = domain;
    ...
    public SGQFactory(SGDomain domain, double discount, double learningRate,
	    double defaultQ, StateHashFactory stateHash,
	    StateAbstraction storedAbstraction) {
	this.domain = domain;
	this.discount = discount;
	this.learningRate = learningRate;
	this.defaultQ = defaultQ;
	...
	SGQLAgent agent = new SGQLAgent(domain, discount, learningRate,
		defaultQ, stateHash);
