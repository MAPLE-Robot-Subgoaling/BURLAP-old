 ...
 * Elements can be added and removed dynamically. The "weights" of the objects
 * should be positive values, but do not have to specify a probability
 * distribution. The sampling will be performed according to the relative weight
 * of all objects.
 * 
 * @author James MacGlashan
 * 
 * @param <T>
 *            any Java object that will be sampled
 */
public class StochasticTree<T> {
	...
	public static void test1() {
		...
		StochasticTree<Integer> st = new StochasticTree<Integer>(weights,
				elements);
		...
		for (int i = 0; i < counts.length; i++) {
			double sample = counts[i] > 0 ? (double) counts[i] / (double) n
					: 0.;
			System.out.printf("%.2f : %.5f\n", weights.get(i) / (1. - 0.4),
					sample);
		}
	...
	public static void test2() {
		...
		/*
		 * st.remove(2); st.remove(2); st.remove(2);
		 */

		/*
		 * System.out.println(st.poll()); System.out.println(st.poll());
		 * System.out.println(st.poll()); System.out.println(st.poll());
		 * System.out.println(st.poll()); System.out.println(st.poll());
		 */
	}
	 ...
	 * 
	 * @param weights
	 *            the weights of a set objects that determine how likely they
	 *            are to be sampled
	 * @param elements
	 *            the elements of the tree that will be sampled
	 */
	...
	/**
	 * Changes the weight of the given element. For this operation to be
	 * supported T must be hashable.
	 * 
	 * @param element
	 *            the element whose weight should be changed.
	 * @param w
	 *            the new weight of the element.
	 */
	...
	/**
	 * Returns the pointer to the stored entry in this tree for the given query
	 * element. This method requires T to be a hashable object.
	 * 
	 * @param el
	 *            the element whose stored object in the tree is to be returned
	 * @return the pointer to the stored entry in this tree for the given query
	 *         element
	 */
	 ...
	 * 
	 * @param node
	 *            the node from which to insert the element
	 * @param w
	 *            the weight of the element
	 * @param el
	...
	protected void insertHelper(STNode node, double w, T el) {
		if (!node.isLeaf()) {
			...
			if (node.left == null) {
				...
				// this node has both a left and right child, so insert along
	 ...
	 * 
	 * @param node
	 *            the node whose weight change should be percolated
	 * @param delta
	 */
	...
	/**
	 * Samples an element according to a probability defined by the relative
	 * weight of objects, removes it from the tree, and returns it.
	 * 
	...
	/**
	 * Removes the given element from the tree. For this operation to be
	 * supported T must be hashable.
	 * 
	 * @param element
	 */
	...
	protected void removeHelper(STNode node) {
		...
		if (node.parent == null) {
			...
			if (target != null) {
				 ...
				 * parent.left.parent = parent; } if(parent.right != null &&
				 * !parent.right.isLeaf()){ parent.right.parent = parent; }
				 */
				...
				System.err
						.println("DOUBLE STOCHASTIC TREE REMOVAL; THIS SHOULD NOT HAPPEN UNDER NORMAL USE");
	...
	/**
	 * Samples an element according to a probability defined by the relative
	 * weight of objects from the tree and returns it
	 * 
	 * @return a sampled element
	 */
	public T sample() {
	 ...
	 * 
	 * @param node
	 *            the node from which to sample
	 * @param v
	 *            the random value used to determine which direction to go
