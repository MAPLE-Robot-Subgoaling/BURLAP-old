 ...
 * according to r^t, where r is in [0,1] and t is the time step, from an initial
 * learning rate. A minimum learning rate value can be specified so that the
 * learning rate is never rounded to zero. By default, the learning rate may
 ...
 * rate for each state that is decayed independently of other states, or it may
 * also be specified to have a learning rate that is independently decayed for
 * each state-action pair. However, the state-action decay will ignore any
 * parameterizations of actions.
 * 
...
public class ExponentialDecayLR implements LearningRate {
    ...
    /**
     * Initializes with an initial learning rate and decay rate for a state
     * independent learning rate. Minimum learning rate that can be returned
     * will be Double.MIN_NORMAL
     * 
    ...
    /**
     * Initializes with an initial learning rate and decay rate for a state
     * independent learning rate that will decay to a value no smaller than
     * minimumLearningRate
     * 
     * @param initialLearningRate
     *            the initial learning rate
     ...
     * state-action dependent learning rate. Minimum learning rate that can be
     ...
     */
    public ExponentialDecayLR(double initialLearningRate, double decayRate,
	    StateHashFactory hashingFactory, boolean useSeparateLRPerStateAction) {
	if (decayRate > 1 || decayRate < 0) {
	    throw new RuntimeException("Decay rate must be <= 1 and >= 0");
	}
	this.initialLearningRate = initialLearningRate;
    ...
    /**
     * Initializes with an initial learning rate and decay rate for a state or
     * state-action dependent learning rate that will decay to a value no
     ...
     */
    public ExponentialDecayLR(double initialLearningRate, double decayRate,
	    double minimumLearningRate, StateHashFactory hashingFactory,
