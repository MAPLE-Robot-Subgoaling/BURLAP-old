 ...
 * respect to some parameters. This interface is useful for
 * DifferentiableSparseSampling which may be used to learn the value of leaf
 * nodes in a finite horizon valueFunction.
 * 
...
public interface DifferentiableVInit extends ValueFunctionInitialization {
	 ...
	 * @author James MacGlashan
	 */
	public static abstract class ParamedDiffVInit implements
			DifferentiableVInit {
		...
		/**
		 * Returns the parameters of this reward function. The parameters are
		 * *not* copied, so changes made externally to them affect this reward
		 * function.
		 * 
		...
		/**
		 * Randomizes the parameter values using the given random number
		 * generator.
		 * 
		 * @param lowerVal
		 *            the lower parameter range value
		 * @param upperVal
		 *            the upper parameter range value
		 * @param rand
		 *            the random number generator to use
		 */
		public void randomizeParameters(double lowerVal, double upperVal,
				Random rand) {
