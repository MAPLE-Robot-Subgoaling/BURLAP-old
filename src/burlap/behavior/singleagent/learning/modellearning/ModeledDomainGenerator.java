...
public class ModeledDomainGenerator implements DomainGenerator {
    ...
    /**
     * Name of both the object class and single binary attribute used to
     * indicate a RMax fictitious state.
     */
	...
	if (useRMaxTransitionsAndFictitousState) {
	    Attribute att = new Attribute(modelDomain, RMAXFICTIOUSSTATENAME,
		    Attribute.AttributeType.BOOLEAN);
	    ObjectClass rmaxstate = new ObjectClass(modelDomain,
	...
	for (Action srcA : sourceDomain.getActions()) {
	    new ModeledAction(modelDomain, srcA, model,
		    useRMaxTransitionsAndFictitousState);
	}
	...
	/**
	 * Whether this action follows the RMax paradigm of transition to a
	 * fictious RMax state when the model is not known
	 */
	protected boolean useRMax;

	/**
	 * The fictitious RMax state this action will transition to for unknown
	 * transitions. Only used in RMax paradigms
	 */
	 ...
	 * @param useRMax
	 *            whether the RMax paradigm of transfering to fictious
	 *            states for unkown transitions is followed.
	 */
	public ModeledAction(Domain modelDomain, Action sourceAction,
		Model model, boolean useRMax) {
	    super(sourceAction.getName(), modelDomain, sourceAction
		    .getParameterClasses(), sourceAction
		...
		ObjectInstance o = new ObjectInstance(
			ModeledDomainGenerator.this.modelDomain
				.getObjectClass(RMAXFICTIOUSSTATENAME),
			"rmax");
	...
	protected State performActionHelper(State s, String[] params) {
	    ...
	    if (this.useRMax) {
		if (!this.model.transitionIsModeled(s, new GroundedAction(
			sourceAction, params))) {
		    return this.RMaxState;
		}
	    }

	    return this.model.sampleModel(s, new GroundedAction(sourceAction,
		    params));
	}
	...
	public List<TransitionProbability> getTransitions(State s,
	    ...
	    return this.model.getTransitionProbabilities(s, new GroundedAction(
		    sourceAction, params));
	}
    ...
    /**
     * An action that is only executable in the ficitious RMax state and which
     * always transitions back to the fictitious RMax state.
     * 
     * @author James MacGlashan
     * 
     */
	...
	/**
	 * Initializes for the owning ModeledDomainGenerator instance's
	 * ModeledDomain instance
	 */
