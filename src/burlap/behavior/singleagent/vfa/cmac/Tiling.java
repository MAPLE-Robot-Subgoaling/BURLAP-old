...
/**
 * This class provides a tiling specification, which tiles a state according to
 * multi-dimensional tiling of specified attributes for specified object
 * classes.
 * 
...
public class Tiling {
	...
	/**
	 * A map from object class names to attribute tile specifications for
	 * attributes of that class
	 */
	protected Map<String, List<AttributeTileSpecification>> specification;

	/**
	 * A list specifying the order that the attributes from different classes
	 * will be combined into a single multi-dimensional tile
	 */
	...
	public Tiling copy() {
		...
		nTiling.specification = new HashMap<String, List<AttributeTileSpecification>>(
				this.specification);
	...
	/**
	 * Adds an attribute tiling specification for the an attribute of the given
	 * class with the given window size and bucket/tile boundary.
	 * 
	 * @param className
	 *            the name of the object class whose attribute specification
	 *            will be provided
	 * @param attribute
	 *            the attribute for which a tiling specification will be
	 *            provided
	 * @param windowSize
	 *            the window size or width of the attribute tiling
	 * @param bucketBoundary
	 *            the offset of this tile alignment; that is, where the first
	 *            tiling boundary starts
	 */
	public void addSpecification(String className, Attribute attribute,
			double windowSize, double bucketBoundary) {
		List<AttributeTileSpecification> specsForClass = specification
				.get(className);
		if (specsForClass == null) {
			...
			Collections.sort(classOrder); // costly to sort with every new class
											// added, but this should be a very
											// short overhead with few items
		} else {
			// check if this attribute specification is already there; if so,
			// remove it
		...
		AttributeTileSpecification ats = new AttributeTileSpecification(
				className, attribute, windowSize, bucketBoundary);
		specsForClass.add(ats);

	}

	/**
	 * Returns the multi-dimensional tile from this tiling that corresponds to
	 * the given input state.
	 * 
	 * @param s
	 * @return the multi-dimensional tile from this tiling that corresponds to
	 *         the given input state.
	 */
	...
	public class StateTile {
		 ...
		 * 
		 * @param s
		 *            the state for which a state tile should be created.
		 */
		public StateTile(State s) {

			this.s = s;
			tiledObjectsByClass = new ArrayList<List<ObjectTile>>(
					Tiling.this.classOrder.size());
			this.hashCode = 1;

			for (String className : Tiling.this.classOrder) {
				List<ObjectInstance> objectsOfClass = s
						.getObjectsOfClass(className);
				List<ObjectTile> objectTiles = new ArrayList<Tiling.ObjectTile>(
						objectsOfClass.size());
		...
		public boolean equals(Object other) {
			...
			if (this.tiledObjectsByClass.size() != that.tiledObjectsByClass
					.size()) {
				return false;
			}

			for (int i = 0; i < this.tiledObjectsByClass.size(); i++) {
				List<ObjectTile> thisObjectsOfClass = this.tiledObjectsByClass
						.get(i);
				List<ObjectTile> thatObjectsOfClass = that.tiledObjectsByClass
						.get(i);
	...
	/**
	 * A class for creating a tiling of a single OO-MDP object instance which
	 * will be combined with other object instance tiles to create a single
	 * state tiling.
	 * 
	 * @author James MacGlashan
	 * 
	...
	public class ObjectTile {
		 ...
		 * 
		 * @param o
		 *            the object instance for which a tile will be created
		 */
		public ObjectTile(ObjectInstance o) {
			...
			List<AttributeTileSpecification> classSpecs = specification
					.get(className);
			for (AttributeTileSpecification ats : classSpecs) {
				...
				} else if (ats.attribute.type
						.equals(Attribute.AttributeType.REAL)
						|| ats.attribute.type
								.equals(Attribute.AttributeType.REALUNBOUND)) {
		...
		/**
		 * Returns the tiling that was used to produce this object instance
		 * tile.
		 * 
		 * @return the tiling that was used to produce this object instance
		 *         tile.
		 */
		public Tiling getOuterTiling() {
			return Tiling.this;
