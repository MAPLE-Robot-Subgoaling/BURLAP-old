...
/**
 * A class for taking an input POMDP (defined by a
 * {@link burlap.oomdp.singleagent.pomdp.PODomain} and turning it into a
 * BeliefMDP, which can then be input to any MDP solver to solve the POMDP. To
 * get the belief MDP reward function, create and instance of the static inner
 * class {@link burlap.oomdp.singleagent.pomdp.BeliefMDPGenerator.BeliefRF}. <br/>
 * The generated Belief MDP action getTransitions method, and the BeliefRF
 * require that the input state object classes implement the
 * {@link burlap.oomdp.singleagent.pomdp.beliefstate.BeliefState} and
 * {@link burlap.oomdp.singleagent.pomdp.beliefstate.EnumerableBeliefState}
 * interfaces. The getTransitions method (used by planning algorithms that
 * require the full transition dynamics) also operates by iterating over all
 * possible observations. If your domain has many observations, consider using a
 * sample-based MDP planning algorithm. <br/>
 * For more information on Belief MDPs, see the POMDP wikipedia page:
 * https://en.
 * wikipedia.org/wiki/Partially_observable_Markov_decision_process#Belief_MDP
 * 
 */
public class BeliefMDPGenerator implements DomainGenerator {
	 ...
	 * that the input states on which the action operates be an instance of
	 * {@link burlap.oomdp.singleagent.pomdp.beliefstate.BeliefState} and
	 * {@link burlap.oomdp.singleagent.pomdp.beliefstate.EnumerableBeliefState}.
	 * The
	 * {@link #getTransitions(burlap.oomdp.core.states.State, burlap.oomdp.singleagent.GroundedAction)}
	 * method (used by planning algorithms that require the full transition
	 * dynamics) operates by iterating over all possible observations.
	 */
	public class BeliefAction extends Action implements FullActionModel {

		/**
		 * The source POMDP action this action will turn into a Belief MDP
		 * action.
		 */
		 ...
		 * 
		 * @param poDomain
		 *            the POMDP {@link burlap.oomdp.singleagent.pomdp.PODomain}
		 * @param pomdpAction
		 *            the POMDP {@link burlap.oomdp.singleagent.Action} that
		 *            this
		 *            {@link burlap.oomdp.singleagent.pomdp.BeliefMDPGenerator.BeliefAction}
		 *            will wrap.
		 * @param domain
		 *            the Belief MDP {@link burlap.oomdp.core.Domain} to which
		 *            this
		 *            {@link burlap.oomdp.singleagent.pomdp.BeliefMDPGenerator.BeliefAction}
		 *            will be associated..
		 */
		public BeliefAction(PODomain poDomain, Action pomdpAction,
				SADomain domain) {
		...
		public List<GroundedAction> getAllApplicableGroundedActions(State s) {
			...
			List<GroundedAction> mdpGAs = this.pomdpAction
					.getAllApplicableGroundedActions(anMDPState);
			List<GroundedAction> beliefGAs = new ArrayList<GroundedAction>(
					mdpGAs.size());
			for (GroundedAction mga : mdpGAs) {

				if (mga instanceof AbstractObjectParameterizedGroundedAction) {
					beliefGAs.add(new ObjectParameterizedGroundedBeliefAction(
							this, mga));
				} else {
		...
		public List<TransitionProbability> getTransitions(State s,
			...
			if (!(s instanceof BeliefState)
				...
				throw new RuntimeException(
						"getTransitions for Belief MDP actions must operate on EnumerableBeliefState instances, but was requested to be operated on a "
								+ s.getClass().getName() + " instance.");
			}

			if (!this.poDomain.getObservationFunction()
					.canEnumerateObservations()) {
				throw new RuntimeException(
						"BeliefAction cannot return the full BeliefMDP transition dynamics distribution, because"
			...
			List<State> observations = BeliefMDPGenerator.this.podomain
					.getObservationFunction().getAllPossibleObservations();
			...
			for (State observation : observations) {
				...
				if (p > 0) {
					BeliefState nbs = bs.getUpdatedBeliefState(observation,
							mdpGA);
			...
			List<TransitionProbability> collapsed = this
					.collapseTransitionProbabilityDuplicates(tps);
		 ...
		 * @param observation
		 *            the observation that will be observed
		 * @param ga
		 *            the pomdp action that would be selected in the previous
		 *            belief state
		 * @return the probability of observing the given observation
		 */
		protected double probObservation(BeliefState bs, State observation,
				GroundedAction ga) {

			ObservationFunction of = this.poDomain.getObservationFunction();
			double sum = 0.;
			List<EnumerableBeliefState.StateBelief> beliefs = ((EnumerableBeliefState) bs)
			...
			for (EnumerableBeliefState.StateBelief sb : beliefs) {
				...
				for (TransitionProbability tp : mdpTps) {
					double op = of.getObservationProbability(observation, tp.s,
							ga);
	...
	/**
	 * A class for turning a POMDP reward function into a Belief MDP reward
	 * function. This class requires that the input {@link State} objects are
	 * classes that implement
	 * {@link burlap.oomdp.singleagent.pomdp.beliefstate.BeliefState} and
	 * {@link burlap.oomdp.singleagent.pomdp.beliefstate.EnumerableBeliefState}.
	 * If the POMDP reward function does not depend on the next state, then this
	 * can be declared with the srcRFIsNextStateIndependent flag in the
	 * {@link burlap.oomdp.singleagent.pomdp.BeliefMDPGenerator.BeliefRF#BeliefRF(PODomain, burlap.oomdp.singleagent.RewardFunction, boolean)}
	 * constructor, which will decrease the computational demands since the next
	 * states do not have to be marginalized over.
	 */
	public static class BeliefRF implements RewardFunction {
		...
		/**
		 * The source POMDP reward function to turn into a belief MDP reward
		 * function
		 */
		protected RewardFunction pomdpRF;

		/**
		 * A boolean flag indicating whether the POMDP reward function is
		 * independent of the next state transition.
		 */
		protected boolean srcRFIsNextStateIndependent;

		/**
		 * Initializes. Class will take the safe (but more computationally
		 * demanding) assumption that the POMDP reward function depends on the
		 * next state transition.
		 * 
		 * @param podomain
		 *            the source POMDP domain
		 * @param pomdpRF
		 *            the source POMDP reward function
		 */
		 ...
		 * @param pomdpRF
		 *            the source POMDP reward function
		 * @param srcRFIsNextStateIndependent
		 *            a boolean flag indicating whether the POMDP reward
		 *            function is independent of the next state transition.
		 */
		public BeliefRF(PODomain podomain, RewardFunction pomdpRF,
		...
		/**
		 * Returns the belief MDP reward when the POMDP reward function is
		 * independent from the next state transition.
		 * 
		 * @param s
		 *            the input belief state
		 * @param a
		 *            the action selected.
		...
		protected double saOnlyReward(State s, GroundedAction a) {
			...
			if (!(s instanceof BeliefState)
					|| !(s instanceof EnumerableBeliefState)) {
				throw new RuntimeException(
						"Belief MDP reward function must operate on EnumerableBeliefState instances, but was requested to be operated on a "
								+ s.getClass().getName() + " instance.");
			}

			EnumerableBeliefState bs = (EnumerableBeliefState) s;

			List<EnumerableBeliefState.StateBelief> beliefs = bs
					.getStatesAndBeliefsWithNonZeroProbability();
			double sum = 0.;
			for (EnumerableBeliefState.StateBelief sb : beliefs) {
		...
		/**
		 * Returns the belief MDP reward when the POMDP reward function is
		 * dependent on the next state transition. Requires marginalizing over
