...
/**
 * A data structure for keeping track of elapsed and average time. Use
 * {@link #start()} to start the timer (or set it to start in the constructor
 * {@link #MyTimer(boolean)}. Use {@link #peekAtTime()} to check the current
 * elapsed time since it was started. Use {@link #stop()} to stop the timer and
 * {@link #getTime()} to get the elapsed time between the last time the timer
 * was started and stopped. If you start and stop the timer multiple times, you
 * can get the average time for all start-stops using the {@link #getAvgTime()}
 * method and the total time of all start-stops with {@link #getTotalTime()}. If
 * you want to check if the timer is running, use {@link #isRunning()}.
 * 
...
public class MyTimer {
	 ...
	 * 
	 * @param start
	 *            if true, then start the timer; if false then don't start the
	 *            timer.
	 */
	 ...
	 * 
	 * @return returns true if the timer is started; false if it is already
	 ...
	 * 
	 * @return returns true if stops the timer; false if the timer is not
