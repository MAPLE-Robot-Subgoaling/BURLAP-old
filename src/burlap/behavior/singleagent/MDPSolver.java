 ...
 * both planning and learning algorithms. It implements the
 * {@link burlap.behavior.singleagent.MDPSolverInterface} and provides the
 * common data members and method implementations that most all algorithms will
 * need to use and provides methods for manipulating them that are common.
 * 
...
public abstract class MDPSolver implements MDPSolverInterface {
	...
	/**
	 * The list of actions this solver can use. May include non-domain specified
	 * actions like {@link burlap.behavior.singleagent.options.Option}s.
	 */
	protected List<Action> actions;

	/**
	 * A mapping to internal stored hashed states (
	 * {@link burlap.oomdp.statehashing.HashableState}) that are stored. Useful
	 * since two identical states may have different object instance name
	 * identifiers that can affect the parameters in GroundedActions.
	 */
	protected Map<HashableState, HashableState> mapToStateIndex;

	// /**
	// * Indicates whether the action set for this valueFunction includes
	// object-parametrized actions that are object identifier independent
	// */
	...
	/**
	 * Takes a source GroundedAction and a matching between object instances of
	 * two different states and returns a GroundedAction with parameters using
	 * the matched parameters if the GroundedAction is an instance of
	 * {@link burlap.oomdp.core.AbstractObjectParameterizedGroundedAction}. This
	 * method is useful a stored state and action pair in the valueFunction data
	 * structure has different object name identifiers than a query state that
	 * is otherwise identical. The matching is from the state in which the
	 * source action is applied to some target state that is not provided to
	 * this method.
	 * 
	 * @param a
	 ...
	 *            names.
	 * @return and new GroundedAction with object parametrization that follow
	 *         from the matching
	 */
