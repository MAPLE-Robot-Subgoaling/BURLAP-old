...
/**
 * An option with a defined policy, initiation state set and termination state
 * set. This option has deterministic termination conditions in any state where
 * the policy is undefined, or in any state that is in the termination set. In
 * general, this method should be used instead of the
 * {@link PolicyDefinedSubgoalOption} when the client needs more control over in
 * which states the option can be initiated and in which states it terminates.
 * One other advantage of this option is that it can be passed a planner from
 * which its policy will be created. The planner will be called from each
 * initiation state provided to the option constructor.
 * 
...
public class DeterminisitcTerminationOption extends Option {
     ...
     *            the option's policy
     * @param init
     *            the initiation states of the option
     * @param terminationStates
     *            the deterministic termination states of the option.
     */
    public DeterminisitcTerminationOption(String name, Policy p,
	    StateConditionTest init, StateConditionTest terminationStates) {
	this.name = name;
	this.policy = p;
     ...
     * The planner is called repeatedly on each state in the initiation state
     * set (which needs to be a
     * {@link burlap.behavior.singleagent.planning.StateConditionTestIterable})
     * and then sets this options policy to the planner derived policy that is
     * provided.
     * 
     * @param name
     *            the name of the option
     ...
     * @param planner
     *            the planner to be used to create the policy for this option
     * @param p
     *            the planner derived policy to use after planning from each
     *            initial state is performed.
     */
    public DeterminisitcTerminationOption(String name,
	    StateConditionTestIterable init,
	    StateConditionTest terminaitonStates, OOMDPPlanner planner,
	...
	// now construct the policy using the planner from each possible
	// initiation state
	for (State si : init) {
    ...
    /**
     * Initializes the option by creating the policy uses some provided option.
     * The planner is called repeatedly on each state in the the list
     * <code>seedStatesForPlanning</code> and then sets this options policy to
     * the planner derived policy that is provided.
     * 
     * @param name
     *            the name of the option
     * @param init
     *            the initiation conditions of the option
     * @param terminationStates
     *            the termination states of the option
     * @param seedStatesForPlanning
     *            the states that should be used as initial states for the
     *            planner
     * @param planner
     *            the planner that is used to create the policy for this option
     * @param p
     *            the planner derived policy to use after planning from each
     *            initial state is performed.
     */
    public DeterminisitcTerminationOption(String name, StateConditionTest init,
	    StateConditionTest terminationStates,
	    List<State> seedStatesForPlanning, OOMDPPlanner planner,
	    PlannerDerivedPolicy p) {
