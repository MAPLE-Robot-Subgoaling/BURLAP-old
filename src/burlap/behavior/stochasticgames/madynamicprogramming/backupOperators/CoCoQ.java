 ...
 * <p/>
 * 1. Sodomka, Eric, et al.
 * "Coco-Q: Learning in Stochastic Games with Side Payments." Proceedings of the
...
public class CoCoQ implements SGBackupOperator {
	...
	public double performBackup(State s, String forAgent,
		...
		double[][] minMaxPayout = new double[forAgentGSAs.size()][otherAgentGSAs
				.size()];
		...
		double[] forAgentStrat = MinMaxSolver
				.getRowPlayersStrategy(minMaxPayout);
		double[] otherAgentStrat = MinMaxSolver
				.getColPlayersStrategy(GeneralBimatrixSolverTools
						.getNegatedMatrix(minMaxPayout));

		double minmaxQ = GeneralBimatrixSolverTools.expectedPayoffs(
				minMaxPayout,
				GeneralBimatrixSolverTools.getNegatedMatrix(minMaxPayout),
				forAgentStrat, otherAgentStrat)[0];
