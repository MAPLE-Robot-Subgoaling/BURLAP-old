...
/**
 * This class provides methods for computing, sampling, and working with
 * Boltzmann distributions/Softmax. The input is a set of "preference" values.
 * The output is a probability distribution over those preferences with elements
 * that had higher preferences having higher probability. Computation is
 * performed using a log sum exponential re-expression trick that avoids
 * overflow errors.
 * 
...
public class BoltzmannDistribution {
	...
	/**
	 * The temperature value. Lower values produce more greedy deterministic
	 * probability outputs while higher values produce more uniform probability
	 * outputs
	 */
	...
	/**
	 * Initializes the distribution with the preference values that are to be
	 * turned into a soft max probability distribution.
	 * 
	 * @param preferences
	 *            the preference values to turn into a probability distribution
	 */
	...
	/**
	 * Initializes the distribution with the preference values that are to be
	 * turned into a soft max probability distribution and a temperature value
	 * to control how deterministic the probability output is. Lower temperature
	 * values result in a more deterministic distribution; higher values produce
