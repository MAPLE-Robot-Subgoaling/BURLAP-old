...
public class LunarLanderDomain implements DomainGenerator {
    ...
    public static final String PFTOUCHSURFACE = "touchingSurface"; // either
								   // horizontally
								   // or landed
								   // on
								   // obstacle
	...
	PropositionalFunction onpad = new OnPadPF(PFONPAD, domain,
		new String[] { AGENTCLASS, PADCLASS });
	PropositionalFunction touchpad = new TouchPadPF(PFTPAD, domain,
		new String[] { AGENTCLASS, PADCLASS });
	PropositionalFunction touchsur = new TouchSurfacePF(PFTOUCHSURFACE,
		domain, new String[] { AGENTCLASS, OBSTACLECLASS });
	PropositionalFunction touchgrd = new TouchGroundPF(PFONGROUND, domain,
		new String[] { AGENTCLASS });
	...
	ObjectInstance pad = new ObjectInstance(
		domain.getObjectClass(PADCLASS), PADCLASS + "0");
	s.addObject(pad);

	for (int i = 0; i < no; i++) {
	    ObjectInstance obst = new ObjectInstance(
    ...
    /*
     * Returns true if the agent is not only touching the landing pad, but has
