...
/**
 * Like the {@link DiscreteStateHashFactory} (which this class extends) this
 * class computes unique hash codes for states based on attributes that the
 * client specifies to use. However, unlike the superclass, this class also
 * performs state equality checks using only the attributes specified for each
 * class. This is often a useful abstraction for components like options whose
 * policies are invariant to attributes specifying properties like ultimate task
 * goal locations.
 * 
...
public class DiscreteMaskHashingFactory extends DiscreteStateHashFactory {
    ...
    // this is here for future implementation so that the client can specify
    // different attributes for hashing versus equality checks
    ...
    /**
     * Initializes this hashing factory to compute hash codes and equality
     * checks with all attributes of all object classes.
     */
    ...
    /**
     * Initializes this hashing factory to hash and equality check on only the
     * attributes for the specified classes in the provided map
     * 
     * @param attributesForHashCode
     *            a map from object class names to the attributes that should be
     *            used in the hash calculation and equality check for those
     *            object classes.
     */
    public DiscreteMaskHashingFactory(
	...
	public boolean equals(Object other) {
		...
		// then the user has specified no special attributes to use for
		// equality as we must use it all
		...
		List<ObjectInstance> theseObjects = this.s
			.getObjectsOfTrueClass(cname);
		List<ObjectInstance> thoseObjects = that.s
			.getObjectsOfTrueClass(cname);
