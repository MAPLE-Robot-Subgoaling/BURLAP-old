...
/**
 * A {@link WorldObserver} that visualizes each transition with a fixed refresh
 * delay between when the transition is rendered and when control is returned to
 * the client.
 * 
...
public class VisualWorldObserver extends JFrame implements WorldObserver {
	 ...
	 * control to the world. Default is 17ms (about 60fps)
	 */
	 ...
	 * 
	 * @param domain
	 *            the stochastic games domain to be visualized
	 * @param v
	 ...
	 *            the stochastic games domain to be visualized
	 * @param v
	 *            the visualize to use
	 * @param cWidth
	 *            the canvas width of the visuzlier
	 * @param cHeight
	 *            the canvas height of the visualizer
	 */
	public VisualWorldObserver(SGDomain domain, Visualizer v, int cWidth,
			int cHeight) {
		this.domain = domain;
		this.painter = v;
		this.cWidth = cWidth;
	...
	/**
	 * Causes the visualizer to be replayed for the given {@link GameAnalysis}
	 * object. The initial state of the provided game is first rendered for the
	 * given refresh delay of this object, and then each joint action is played.
	 * 
	 * @param ga
	 *            the game analysis object to be replayed.
	 */
