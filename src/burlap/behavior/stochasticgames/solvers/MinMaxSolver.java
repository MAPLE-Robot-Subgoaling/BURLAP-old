...
public class MinMaxSolver {
	...
	public static double[] getColPlayersStrategy(double[][] payoffMatrix) {
		...
		// get positive matrix (finds the minimum value and adds -min + 1 to all
		// elements)
		double[][] G = GeneralBimatrixSolverTools
				.getPositiveMatrix(payoffMatrix);

		LinearProgram lp = new LinearProgram(
				GeneralBimatrixSolverTools.constantDoubleArray(1., G[0].length));

		int cCount = 0;

		// add payoff matrix constraints
		for (int i = 0; i < G.length; i++) {
			lp.addConstraint(new LinearBiggerThanEqualsConstraint(G[i], 1., "c"
					+ cCount));
			cCount++;
		}
		...
		for (int i = 0; i < G[0].length; i++) {
			lp.addConstraint(new LinearBiggerThanEqualsConstraint(
					GeneralBimatrixSolverTools.zero1Array(i, G[0].length), 0.,
					"c" + cCount));
			cCount++;
		}
