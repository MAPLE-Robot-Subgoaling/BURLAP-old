 ...
 * agent's action. The command format is "agentName::action parameter1 pameter2"
 * and so on for as many parameters as there may be (or none if an action takes
 * no parameters).
 * <p/>
 ...
 * <p/>
 * The command ##reset## causes the state to reset to the initial state provided
 * to the explorer.
 * 
...
public class SGTerminalExplorer {
	...
	while (true) {
		...
		if (line.equals("##reset##")) {
		    ...
		    if (this.rf != null) {
			Map<String, Double> reward = rf.reward(s,
				curJointAction, ns);
			...
			if (sa.isApplicableInState(s, agentName, params)) {
			    ...
			    System.out
				    .println("Cannot apply this action in this state: "
