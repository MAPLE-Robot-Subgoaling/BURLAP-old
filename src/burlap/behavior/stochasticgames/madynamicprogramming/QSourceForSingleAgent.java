...
/**
 * And interface for objects that can store and retrive Q-value objects for a
 * specific agent and state and joint action query. The {@link JAQValue}
 * returned can have their q datamember directly modified to modify the Q-values
 * stored and retreived in this object. A hash-backed implementation is also
 * provided in this interface definition.
 * 
...
public interface QSourceForSingleAgent {
	 ...
	 * Q-values backed by hashmaps. Specifically, a map from hashed stats to a
	 * map from joint actions to stored {@link JAQValue} objects. This object
	 * also takes as {@link ValueFunctionInitialization} object so that Q-values
	 * for state-jointaction pairs not previously stored can be intitialized.
	 * 
	 * @author James MacGlashan
	 * 
	 */
	public class HashBackedQSource implements QSourceForSingleAgent {
		...
		/**
		 * Initializes with a given state hashing factory and value funciton
		 * initialization for Q-values.
		 * 
		 * @param hashingFactory
		 *            the state hashing factory used to index states
		 * @param qInit
		 *            the value function intitliazaiton to use for previously
		 *            unqueried states
		 */
		public HashBackedQSource(HashableStateFactory hashingFactory,
		...
		public JAQValue getQValueFor(State s, JointAction ja) {
			...
			// first make sure it's not an object identifier difference between
			// states that is causing a failure of matching
			if (ja.isParameterized()) {
				// is there a matching joint action after we translate?
				for (JAQValue sq : jaQS.values()) {
					JointAction translated = (JointAction) ja
							.translateParameters(s, sq.s);
