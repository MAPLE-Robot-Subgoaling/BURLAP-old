 ...
 * is a Best-first planner in which the f-score is g(s) + h(s), where g(s) is
 * the cost to state s, and h(s) is the heuristic score for s. Best-first search
 * requires checking if the open queue already has a search node stored in it
 * and possibly modifying its priority and back pointers if a better path to it
 * has been found. To efficiently provide that functionality, this class makes
 * use of a custom hash-backed priority queue (heap) which performs contains
 * tests using a hash map.
 * <p/>
		...
		for (GroundedAction ga : gas) {
			...
			if (F <= closedPSN.priority) {
			    continue; // no need to reopen because this is a
				      // worse path to an already explored node
			}
