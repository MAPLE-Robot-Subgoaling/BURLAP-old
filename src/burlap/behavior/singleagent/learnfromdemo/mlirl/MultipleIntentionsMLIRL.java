 ...
 * Reinforcement Learning [1]. This algorithm takes as input a set of expert
 * trajectories, a number of clusters, and a differentiable reward function
 * model; and clusters the trajectories assigning each cluster its own reward
 * function parameter values. The algorithm uses EM to find the reward function
 * parameter values for each cluster and uses
 * {@link burlap.behavior.singleagent.learnfromdemo.mlirl.MLIRL} to perform the
 * maximization step of the parameter values. EM is run for a specified number
 * of iterations.
 * <p/>
 * At initialization, the reward function parameters for each behavior cluster
 * will be randomly assigned values between -1 and 1. If you want to change this
 * behavior, subclass this object and override the
...
public class MultipleIntentionsMLIRL {
	...
	/**
	 * The {@link burlap.behavior.singleagent.learnfromdemo.mlirl.MLIRL}
	 * instance used to perform the maximization step for each clusters reward
	...
	/**
	 * A random object used for initializing each cluster's RF parameters
	 * randomly.
	 */
	protected Random rand = RandomFactory.getMapped(0);

	/**
	 * Initializes. Reward function parameters for each cluster will be
	 * initialized randomly between -1 and 1.
	 * 
	 * @param request
	 *            the request that defines the problem.
	 * @param emIterations
	 *            the number of EM iterations to perform.
	 * @param mlIRLLearningRate
	 *            the learning rate of the underlying
	 *            {@link burlap.behavior.singleagent.learnfromdemo.mlirl.MLIRL}
	 *            instance.
	 * @param maxMLIRLLikelihoodChange
	 *            the likelihood change threshold that causes
	 *            {@link burlap.behavior.singleagent.learnfromdemo.mlirl.MLIRL}
	 *            gradient ascent to stop.
	 * @param maxMLIRLSteps
	 *            the maximum number of gradient ascent steps allowd by the
	 *            underlying
	 *            {@link burlap.behavior.singleagent.learnfromdemo.mlirl.MLIRLRequest}
	 *            gradient ascent.
	 */
	public MultipleIntentionsMLIRL(MultipleIntentionsMLIRLRequest request,
			int emIterations, double mlIRLLearningRate,
			double maxMLIRLLikelihoodChange, int maxMLIRLSteps) {

		if (!request.isValid()) {
			throw new RuntimeException(
					"Provided MultipleIntentionsMLIRLRequest object is not valid.");
		}

		this.request = request;
		this.initializeClusters(this.request.getK(),
				this.request.getPlannerFactory());

		this.numEMIterations = emIterations;
		this.mlirlInstance = new MLIRL(request, mlIRLLearningRate,
				maxMLIRLLikelihoodChange, maxMLIRLSteps);
	...
	/**
	 * Computes the probability of each trajectory being generated by each
	 * cluster and returns it in a matrix. The prior probability of each cluster
	 * prior is also updated to maximize these values. The returned matrix has
	 * clusters along the rows and trajectories along the columns. These values
	 * are used to weight the contribution of each trajectory for the MLIRL
	 * performed to maxmize each cluster RF parameters.
	 * 
	 * @return the probability of each trajectory being generated by each
	 *         cluster
	 */
	protected double[][] computePerClusterMLIRLWeights() {
		...
		// first do pass computing log prior weighted likelihood of each
		// trajectory
		for (int i = 0; i < k; i++) {
			...
			for (int j = 0; j < n; j++) {
				double trajectLogLikelihood = this.mlirlInstance
						.logLikelihoodOfTrajectory(this.request
								.getExpertEpisodes().get(j), 1.);
		...
		// now pass through normalizing in log space, and then exponentiate to
		// get back probability
		// also maintain sum of entire matrix to normalize for new cluster
		// priors
		double matrixSum = 0.;
		for (int j = 0; j < n; j++) {
			double columnDenom = this
					.computeClusterTrajectoryLoggedNormalization(j, newWeights);
			for (int i = 0; i < k; i++) {
	...
	/**
	 * Initializes cluster data; i.e., it initializes RF parameters, cluster
	 * prior parameters (to uniform), and creates
	 * {@link burlap.behavior.singleagent.learnfromdemo.mlirl.MLIRLRequest}
	 ...
	 *            to use to generate a valueFunction for each cluster.
	 */
	protected void initializeClusters(int k,
		...
		for (int i = 0; i < k; i++) {
			...
			MLIRLRequest nRequest = new MLIRLRequest(this.request.getDomain(),
					null, this.request.getExpertEpisodes(), rfs.get(i));

			nRequest.setGamma(this.request.getGamma());
			nRequest.setBoltzmannBeta(this.request.getBoltzmannBeta());
			nRequest.setPlanner((Planner) plannerFactory
					.generateDifferentiablePlannerForRequest(nRequest));
