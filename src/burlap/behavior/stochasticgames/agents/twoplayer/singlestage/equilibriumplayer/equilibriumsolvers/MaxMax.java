...
/**
 * A class for finding a stategy that maxmizes the player's payoff under the
 * assumption that their "opponent" is friendly and will try to do the same.
 * This amounts to finding the joint aciton that has the maximum payoff for the
 * query player and playing the corresponding action. For example, if
 * rowPayoff[i][j] is the maximum element in the matrix, the
 * {@link #computeRowStrategy(double[][], double[][])} method will return a
 * strategy that always plays aciton i. If there are ties for the maximum with
 * different row actions, then the strategy returns is uniformly random among
 * the ties. The {@link #computeColStrategy(double[][], double[][])} does the
 * same, except for the colPayoff matrix and for column actions.
 * 
