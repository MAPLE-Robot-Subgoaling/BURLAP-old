 ...
 * 
 * This class is designed to run the merge operations for the policy blocks
 * algorithm
 * 
 * @author Tenji Tembo
 * 
...
public class TrajectoryGenerator {
	 ...
	 * 
	 * Note:the printing of the merged policies can be cleaned up, because the
	 * are visualized in showEpisodes() + also needs to be cleaned up as well
	 * 
	 * Description: Main class for generating and running the policies needed
	 * for visualization purposes. The Episode Anaylsis Trajectories only
	 * contain the best possible path afer being generated from a number of
	 * random policy goal states
	 * 
	 * @param num
	 *            - the number of policies to perform merging with
	 */
	...
	public void runSim(int num) {
		...
		for (int i = 0; i < ((ArrayList) (output.get(0))).size(); i++) {
			...
			System.out.print("\n"
					...
					+ "\n  Score: "
					+ Integer.toString((Integer) ((ArrayList) output.get(2))
							.get(i)));
		...
		System.out.println(((int) (Math.pow(2, number)) - 1)
				+ " possible policies\n" + ((ArrayList) output.get(0)).size()
				+ " resulting policies");

		System.out.print("\n"
				+ ((ArrayList) (output.get(1))).get(((ArrayList) output.get(2))
						.indexOf(max))
				+ "\n  Score: "
				+ Integer.toString((Integer) ((ArrayList) output.get(2))
						.get(((ArrayList) output.get(2)).indexOf(max))));
		visualize((EpisodeAnalysis) (((ArrayList) (output.get(0)))
				.get(((ArrayList) output.get(2)).indexOf(max))));
	 ...
	 * 
	 * @param outputPath
	 *            - the string file path where the trajectories are saved.
	 */
	...
	/**
	 * generatePolicies() - Generates "number" iterations which contains 100
	 * policies run via Q-Learning
	 * 
	 * @param number
	 *            - takes the number of desired trajectories stored in the
	 *            object and generates them.
	 */
	 ...
	 * 
	 * @param merged
	 *            - the EA obj to be visualized.
	 * 
	 *            Note: this can go or be cleaned up. Each of the merged
	 *            policies are able to be visualized.
	 */
	public void visualize(EpisodeAnalysis merged) {
		...
		for (int i = 0; i < merged.stateSequence.size() - 1; i++) {
			matrix[merged.stateSequence.get(i).getObservableObjectAt(1)
					.getValues().get(0).getDiscVal()][merged.stateSequence
					.get(i).getObservableObjectAt(1).getValues().get(1)
					.getDiscVal()] = (merged.actionSequence.get(i).toString()
					.charAt(0));
		}
		// displays map
		System.out.print("\n");
	...
	/*
	 * Merge() - Merges two policies This is the merge function. Currently all
	 * it does is collect the first two merged generated by Q-Learning. Then it
	 * iterates state by state, checking to see if the agent visits the same
	 * state in both policies.
	 * 
	 * If so, it takes the state-action-reward set and writes it to the episode
	 * analysis object.
	 */
	public EpisodeAnalysis merge(EpisodeAnalysis e0, EpisodeAnalysis e1) {
		...
		for (int i = 0; i < e0.stateSequence.size() - 1; i++) {
			Object s = e0.stateSequence.get(i).getObservableObjectAt(1)
					.getValues(); // collect the first state
			for (int j = 0; j < e1.stateSequence.size() - 1; j++) {
				Object p = e1.stateSequence.get(j).getObservableObjectAt(1)
						.getValues(); // collect the second state
				if (s.equals(p)
						&& e0.actionSequence.get(i).toString()
								.equals(e1.actionSequence.get(j).toString())) { // do
																				// we
																				// have
																				// a
																				// match

					// if you can figure out a better way to do this, by all
					// means!!!
	 ...
	 * 
	 * @param set
	 *            - the array set of all the merged trajectory objects needed to
	 *            merge
	...
	public ArrayList<Object> unionSet(EpisodeAnalysis[] set) {
		...
		// this loop seeks to exhaustively find the union set of the policies
		// provided
		// the variable "i" is treated as though it were in binary with each bit
		// representing whether a specific policy is part of the merged policy
		// or not
		// "i" is split into the leading boolean non-zero bit and the other
		// bits; the former is used to find the proper policy and the latter is
		// used to find the already merged other policies (within the result
		// array)
		// n is used to identify the leading bit easily
		// for example: when "i" is 11, this can be seen as 1011; the 1
		// indicates the obtain the fourth policy in the provided list and the
		// 011 indicate the merge policy 4 with policy 3 (011 in decimal) in the
		// resulting array

		// variable to store the number of binary digits used to represent
		// previous merges
		int n = 1;
		...
		System.out.println("Merges complete:         (of "
				+ ((int) (max / 100000) + " * 100K)"));
		...
		for (int i = 1; i < max; i++) {
			...
			if (i - Math.pow(2, n - 1) == 0) {
				...
				scores.add(result.get(label.indexOf(i)).stateSequence.size());
			} else if (label.indexOf(((int) (i - Math.pow(2, n - 1)))) > -1/*
																			 * &&
																			 * depth
																			 * .
																			 * get
																			 ...
																			 */) // toggling
																					...
																					// policies
			{
				temp = merge(set[n - 1], result.get((int) (label
						.indexOf((int) (i - Math.pow(2, n - 1))))));
				if (temp.stateSequence.size() > 0) {
					label.add(i);
					result.add(merge(set[n - 1], result.get((int) (label
							.indexOf((int) (i - Math.pow(2, n - 1)))))));
					names.add(names.get((int) (label.indexOf((int) (i - Math
							.pow(2, n - 1))))) + "+" + Integer.toString(n));
					depth.add(depth.get((int) (label.indexOf((int) (i - Math
							.pow(2, n - 1))))) + 1);
					scores.add(result.get(label.indexOf(i)).stateSequence
							.size() * depth.get(label.indexOf(i)));
				}
			}
	...
	/**
	 * writeTrajectories() - writes the trajectories merged to the file output
	 * path
	 */
	...
	/**
	 * createOptions() - takes the merged set of policies, and converts them
	 * into Policy Based Options
	 * 
	...
	public ArrayList<Option> createOptions() {
		...
		for (Policy p : policies) {
			options.add(new PolicyDefinedSubgoalOption("option-" + i, p,
					new PolicyStateCheck(p)));
			i++;
		}
