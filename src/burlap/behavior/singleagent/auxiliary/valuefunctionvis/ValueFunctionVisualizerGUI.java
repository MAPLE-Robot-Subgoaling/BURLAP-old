...
/**
 * Provides a GUI for a value function and policy visualizer. Provides a toggle
 * button for rendering the policy if the policy is provided.
 * 
...
public class ValueFunctionVisualizerGUI extends JFrame implements ItemListener {
	 ...
	 * visualization. The value of states will be represented by colored cells
	 * from red (lowest value) to blue (highest value). North-south-east-west
	 * actions will be rendered with arrows using
	 ...
	 * {@link #initGUI()} on the returned object to start it.
	 * 
	 * @param states
	 *            the states whose value should be rendered.
	 * @param valueFunction
	 *            the valueFunction that can return the state values.
	 * @param p
	...
	public static ValueFunctionVisualizerGUI createGridWorldBasedValueFunctionVisualizerGUI(
		...
		PolicyGlyphPainter2D spp = ArrowActionGlyph.getNSEWPolicyGlyphPainter(
				classWithPositionAtts, xAttName, yAttName, northActionName,
				southActionName, eastActionName, westActionName);

		ValueFunctionVisualizerGUI gui = new ValueFunctionVisualizerGUI(states,
				svp, valueFunction);
		gui.setSpp(spp);
		gui.setPolicy(p);
	 ...
	 *            the valueFunction that can return the state values.
	 */
	public ValueFunctionVisualizerGUI(List<State> states,
			StateValuePainter svp, ValueFunction valueFunction) {
		this.statesToVisualize = states;
		this.svp = svp;
		this.visualizer = new MultiLayerRenderer();
		this.vfLayer = new ValueFunctionRenderLayer(statesToVisualize, svp,
				valueFunction);
	...
	/**
	 * Returns the {@link burlap.oomdp.visualizer.MultiLayerRenderer} used in
	 * this GUI.
	 * 
	 * @return the {@link burlap.oomdp.visualizer.MultiLayerRenderer} used in
	 *         this GUI.
	 */
