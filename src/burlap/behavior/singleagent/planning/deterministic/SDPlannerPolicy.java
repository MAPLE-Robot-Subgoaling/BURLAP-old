...
/**
 * This is a static deterministic planner policy, which means if the source
 * deterministic planner has not already computed and cached the plan for a
 * query state, then this policy is undefined for that state and will cause the
 * policy to throw a corresponding
 * {@link burlap.behavior.singleagent.Policy.PolicyUndefinedException} exception
 * object.
 * 
...
public class SDPlannerPolicy extends Policy implements PlannerDerivedPolicy {
	...
	if (this.dp == null) {
	    throw new RuntimeException(
		    "The planner used by this Policy is not defined; therefore, the policy is undefined.");
	}
