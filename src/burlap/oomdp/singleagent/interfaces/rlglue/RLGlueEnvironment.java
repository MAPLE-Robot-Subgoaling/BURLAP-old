 ...
 * and turn it into an RLGlue environment with which other RLGlue agents can
 * interact. Because of the nature of RLGlue there are a few limitations: <br/>
 * (1) the same actions available in one state must be available everywhere<br/>
 * (2) the environment cannot represent object identifier independence and will
 * fill in RLGlue feature vectors by object class and in the order the objects
 * appear for each class;<br/>
 * (3) while single target relational domains can be used, multi-target
 * relational domains cannot.
 * <p/>
 * Because a fixed number of objects for each class is assumed, action
 * parameterization is supported by multiplying out all the possible
 * parameterizations. In order for action parameterization and relational
 * domains to work consistently for RLGlue, the state generator should always
 * add objects of each class to the state object in the same order. For
 * instance, in a grid world, the state generator should always add the agent
 * object instance to the state first and then all location objects (or always
 * do it in the reverse order). Object instance names, however, can vary between
 * generated states. <br/>
 * <br/>
 * Note that RLGlue does not support observations of terminal states; it only
 * gives the final reward upon entering a terminal state. Therefore, this class
 * will not terminate in a terminal state indicated by the provided
 * {@link burlap.oomdp.core.TerminalFunction}. Instead, it will allow one more
 * transition from the terminal state, which will transition back to itself with
 * reward zero, which is mathematically equivalent to transitioning to terminal
 * state and observing it.
 * 
...
public class RLGlueEnvironment implements EnvironmentInterface {
	...
	/**
	 * Indicates the number of times a terminal state has been visited by the
	 * agent within the same episode. This variable is used because RLGLue does
	 * not support observations into terminal states and so a terminal flag will
	 * only be set once the agent has taken one action in the terminal state,
	 * which will transition back to itself.
	 */
	...
	/**
	 * The number of objects of each object class that will appear in all
	 * states.
	 */
	...
	/**
	 * A mapping from action index identifiers (that RLGlue will use) to BURLAP
	 * actions and their parametrization specified as the index of objects in a
	 * state.
	 */
	...
	/**
	 * Whether the state generated from the state generator to gather auxiliary
	 * information (like the number of objects of each class) has yet be used as
	 * a starting state for an RLGlue episode. When this value is false, the
	 * state generated in the constructor will be passed as the initial state of
	 * a new episodes. After that, this value is set to true and the states used
	 * for each RLGlue episode are generated fresh from the state generator.
	 */
	protected boolean usedConstructorState = false;

	/**
	 * Constructs with all the BURLAP information necessary for generating an
	 * RLGlue Environment.
	 * 
	 * @param domain
	 *            the BURLAP domain
	 * @param stateGenerator
	 *            a generated for generating states at the start of each
	 *            episode.
	 * @param rf
	 *            the reward function
	 * @param tf
	 *            the terminal funciton
	 * @param rewardRange
	 *            the reward function value range
	 * @param isEpisodic
	 *            whether the task is episodic or continuing
	 * @param discount
	 *            the discount factor to use for the task
	 */
	public RLGlueEnvironment(Domain domain, StateGenerator stateGenerator,
			RewardFunction rf, TerminalFunction tf, DoubleRange rewardRange,
			boolean isEpisodic, double discount) {
		...
		for (burlap.oomdp.singleagent.Action a : this.domain.getActions()) {
			List<GroundedAction> gas = a
					.getAllApplicableGroundedActions(exampleState);
			for (GroundedAction ga : gas) {
				// ActionIndexParameterization ap = new
				// ActionIndexParameterization(ga, exampleState);
				this.actionMap.put(actionInd, ga);
				actionInd++;
			}
		}

		List<List<ObjectInstance>> obsByClass = exampleState
	 ...
	 * 
	 * @param theTaskSpecObject
	 *            the RLGlue task specification
	 * @param att
	 *            the BURLAP attribute to add to the spec
	 */
	protected void addAttribute(TaskSpecVRLGLUE3 theTaskSpecObject,
			Attribute att) {
		Attribute.AttributeType type = att.type;
		if (type == AttributeType.DISC || type == AttributeType.BOOLEAN
				|| type == AttributeType.INT) {
			theTaskSpecObject.addDiscreteObservation(new IntRange(
					(int) att.lowerLim, (int) att.upperLim));
			this.numDiscreteAtts++;
		} else if (type == AttributeType.RELATIONAL) {
			theTaskSpecObject.addDiscreteObservation(new IntRange(0,
					this.numObjects - 1));
			this.numDiscreteAtts++;
		} else if (type == AttributeType.REAL
				|| type == AttributeType.REALUNBOUND) {
			theTaskSpecObject.addContinuousObservation(new DoubleRange(
					att.lowerLim, att.upperLim));
			this.numContinuousAtts++;
		} else {
			throw new RuntimeException(
					"Cannot create RLGlue Attribute for BURLAP att type: "
							+ type);
		}
		...
		for (Map.Entry<String, Integer> e : this.numObjectsOfEachClass
			...
			for (int i = 0; i < obs.size(); i++) {
				...
				for (Attribute att : atts) {
					if (att.type == AttributeType.DISC
							|| att.type == AttributeType.INT
							|| att.type == AttributeType.BOOLEAN) {
						o.setInt(discCounter,
								oi.getIntValForAttribute(att.name));
						discCounter++;
					} else if (att.type == AttributeType.REAL
							|| att.type == AttributeType.REALUNBOUND) {
						o.setDouble(contCounter,
								oi.getRealValForAttribute(att.name));
						contCounter++;
					} else if (att.type == AttributeType.RELATIONAL) {
						o.setDouble(discCounter,
								this.objectIndex(s, oi.getName()));
						discCounter++;
					}
