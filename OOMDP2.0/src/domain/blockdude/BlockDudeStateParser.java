...
public class BlockDudeStateParser implements StateParser {
	...
	public String stateToString(State s) {
		...
		ObjectInstance a = s.getObjectsOfTrueClass(BlockDudeDomain.CLASSAGENT).get(0);
		ObjectInstance e = s.getObjectsOfTrueClass(BlockDudeDomain.CLASSEXIT).get(0);
		List<ObjectInstance> blocks = s.getObjectsOfTrueClass(BlockDudeDomain.CLASSBLOCK);
		List<ObjectInstance> platforms = s.getObjectsOfTrueClass(BlockDudeDomain.CLASSPLATFORM);
	...
	public State stringToState(String str) {
		...
		State s = BlockDudeDomain.getCleanState(this.domain, xpcomps, hpcomps, nb);
		
		//parse agent
		String [] ascomps = obcomps[2].split(" ");
		BlockDudeDomain.setAgent(s, Integer.parseInt(ascomps[0]), Integer.parseInt(ascomps[1]), Integer.parseInt(ascomps[2]), Integer.parseInt(ascomps[3]));
		
		//parse exit
		String [] escomps = obcomps[3].split(" ");
		BlockDudeDomain.setExit(s, Integer.parseInt(escomps[0]), Integer.parseInt(escomps[1]));
		
		//parse each block
		for(int i = 4; i < 4+nb; i++){
			String [] bscomps = obcomps[i].split(" ");
			BlockDudeDomain.setBlock(s, i-4, Integer.parseInt(bscomps[0]), Integer.parseInt(bscomps[1]));
		}
