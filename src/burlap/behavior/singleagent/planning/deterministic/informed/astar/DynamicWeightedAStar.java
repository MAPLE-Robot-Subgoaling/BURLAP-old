 ...
 * depth of the current search tree and based on an expected depth of the
 * search. Specifically, f(n) = g(n) + (1 + \epsilon * w(n))*h(n),
 * 
 * where epsilon is a parameter > 1 indicating greediness (the larger the more
 * greedy) and
 * 
 * w(n) = { 1 - d(n)/N if d(n) <= N { 0 if d(n) > N,
 * 
 * where d(n) is the depth of the search and N is the expected depth of the
 * search. This algorithm has the effect of becoming less greedy as the search
 * continues, which allows it to find a decent solution quickly but avoid
 * returning extremely sub-optimal solutions.
 * 
 * <p/>
 ...
 * Intelligence (IJCAI-73) 3. California, USA. pp. 11-17.
 * 
 * 
...
public class DynamicWeightedAStar extends AStar {
	...
	/**
	 * parameter > 1 indicating the maximum amount of greediness; the larger the
	 * more greedy.
	 */
	 ...
	 * @param epsilon
	 *            parameter > 1 indicating greediness; the larger the value the
	 *            more greedy.
	 * @param expectedDepth
	 *            the expected depth of the plan
	 */
	public DynamicWeightedAStar(Domain domain, RewardFunction rf,
			StateConditionTest gc, HashableStateFactory hashingFactory,
			Heuristic heuristic, double epsilon, int expectedDepth) {
	 ...
	 * . <br/>
	 * This method overrides AStar's implementation so that it avoids reopening
	 * closed states that are not actually better due to the dynamic h weight,
	 * the reopen check needs to be based on the g score, note the f score
	 * 
	 * @param initialState
			...
			for (Action a : actions) {
				...
				for (GroundedAction ga : gas) {
					...
					if (closedPSN != null) {
						...
						if (lastComputedCumR <= cumulatedRewardMap
								.get(closedPSN.s)) {
					...
					} else if (lastComputedCumR > cumulatedRewardMap
							.get(openPSN.s)) {
