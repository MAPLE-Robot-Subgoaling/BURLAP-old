 ...
 * provided a FeatureDatabase object that will be used to store retrieve state
 * features. For every feature returned by the feature database, this class will
 * automatically create a weight associated with it. The returned approximated
 * value for any state is the linear combination of state features and weights.
 * 
...
public class LinearVFA implements ValueFunctionApproximation {
	...
	/**
	 * Initializes with a feature database; the default weight value will be
	 * zero
	 * 
	...
	public LinearVFA(FeatureDatabase featureDatabase) {
		...
		if (featureDatabase.numberOfFeatures() > 0) {
			this.weights = new HashMap<Integer, FunctionWeight>(
					featureDatabase.numberOfFeatures());
		} else {
			this.weights = new HashMap<Integer, FunctionWeight>();
		}
	 ...
	 *            the feature database to use
	 * @param defaultWeight
	 *            the default feature weight to initialize feature weights to
	 */
	...
	/**
	 * Computes the linear function over the given features and the stored
	 * feature weights.
	 * 
	 * @param features
	 *            List of the {@link StateFeature} obejcts defining the state
	 *            features of this approximator.
	 * @return the linear function over the given features and the stored
	 *         feature weights stored in a {@link ApproximationResult}.
	 */
	protected ApproximationResult getApproximationResultFrom(
			List<StateFeature> features) {

		List<FunctionWeight> activedWeights = new ArrayList<FunctionWeight>(
				features.size());
		...
		ApproximationResult result = new ApproximationResult(predictedValue,
				features, activedWeights);
