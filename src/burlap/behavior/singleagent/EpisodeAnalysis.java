...
/**
 * This class is used to keep track of all events that occur in an episode. This
 * class should be created by either calling the constructor with the initial
 * state of the episode, or by calling the default constructor and then calling
 * the {@link #initializeEpisideWithInitialState(State)} method to set the
 * initial state of the episode, before recording any transitions. It is then
 * advised that transitions are recorded with the
 * {@link #recordTransitionTo(GroundedAction, State, double)} method, which
 * takes as input the next state to which the agent transitioned, the action
 * applied in the last recorded state, and the reward received fro the
 * transition.
 * <p/>
 * When querying about the state, action, and reward sequences, use the
 * {@link #getState(int)}, {@link #getAction(int)}, and {@link #getReward(int)}
 * methods. These methods take as input the time step of the element you want.
 * Note that t = 0 refers to the initial state step so calling getState(0) and
 * getAction(0) will return the initial state and the action taken in the
 * initial state, respectively. However, rewards are always received in the next
 * time step from the state and action that produced them. Therefore,
 * getReward(0) is undefined. Instead, the first reward received will be at time
 * step 1: getReward(1).
 * 
 * 
...
public class EpisodeAnalysis {
	...
	/**
	 * The sequence of rewards received. Note the reward stored at index i is
	 * the reward received at time step i+1.
	 */
	 ...
	 * {@link #recordTransitionTo(GroundedAction, State, double)} method is used
	...
	/**
	 * Records an transition event where the agent applied the usingAction
	 * action in the last state in this object's state sequence, transitioned to
	 * state nextState, and received reward r,.
	 * 
	 * @param usingAction
	 *            the action the agent used that caused the transition
	 * @param nextState
	 ...
	 *            the reward the agent received for this transition.
	 */
	public void recordTransitionTo(GroundedAction usingAction, State nextState,
			double r) {
	...
	/**
	 * Returns the state observed at time step t. t=0 refers to the initial
	 * state.
	 * 
	 * @param t
	 *            the time step of the episode
	...
	/**
	 * Returns the action taken in the state at time step t. t=0 refers to the
	 * action taken in the initial state.
	 * 
	 * @param t
	 *            the time step of the episode
	...
	/**
	 * Returns the reward received at timestep t. Note that the fist received
	 * reward will be at time step 1, which is the reward received after taking
	 * the first action in the initial state.
	 * 
	 * @param t
	 *            the time step of the episode
	...
	public double getReward(int t) {
		if (t == 0) {
			throw new RuntimeException(
					"Cannot return the reward received at time step 0; the first received reward occurs after the initial state at time step 1");
		}
		if (t > rewardSequence.size()) {
			throw new RuntimeException(
					"There are only "
							+ this.rewardSequence.size()
							+ " rewards recorded; cannot return the reward for time step "
							+ t);
		}
	...
	public void appendAndMergeEpisodeAnalysis(EpisodeAnalysis e) {
		for (int i = 0; i < e.numTimeSteps() - 1; i++) {
			this.recordTransitionTo(e.getAction(i), e.getState(i + 1),
					e.getReward(i + 1));
		}
	}
	 ...
	 * {@link burlap.behavior.singleagent.EpisodeAnalysis} objects and writes
	 * them to a directory. The format of the file names will be
	 * "baseFileName{index}.episode" where {index} represents the index of the
	 * episode in the list. States will be serialized with a
	 * {@link burlap.oomdp.stateserialization.simple.SimpleSerializableStateFactory}
	 * 
	 * @param episodes
	 *            the list of episodes to write to disk
	 * @param directoryPath
	 *            the directory path in which the episodes will be written
	 * @param baseFileName
	 *            the base file name to use for the episode files
	 */
	public static void writeEpisodesToDisk(List<EpisodeAnalysis> episodes,
			String directoryPath, String baseFileName) {
	...
	/**
	 * Writes this episode to a file. If the the directory for the specified
	 * file path do not exist, then they will be created. If the file extension
	 * is not ".episode" will automatically be added. States will be serialized
	 * with a
	 * {@link burlap.oomdp.stateserialization.simple.SimpleSerializableStateFactory}
	 * .
	 * 
	 ...
	 *            that defines how states are to be serialized.
	 */
	public void writeToFile(String path,
			SerializableStateFactory serializableStateFactory) {
	...
	/**
	 * Takes a path to a directory containing .episode files and reads them all
	 * into a {@link java.util.List} of
	 * {@link burlap.behavior.singleagent.EpisodeAnalysis} objects.
	 * 
	 * @param directoryPath
	 *            the path to the directory containing the episode files
	 * @param d
	 *            the domain to which the episode states and actions belong
	 * @return a {@link java.util.List} of
	 *         {@link burlap.behavior.singleagent.EpisodeAnalysis} objects.
	 */
	public static List<EpisodeAnalysis> parseFilesIntoEAList(
			String directoryPath, Domain d) {
		...
		private class StateYamlRepresent implements Represent {
			...
			public Node representData(Object o) {
				try {
					...
					throw new RuntimeException(
							"EpisodeAnalysis could not serialize one of the states. Got this error:\n"
									+ e.getMessage());
				}
			}
		}
	...
	public static EpisodeAnalysis parseEpisode(Domain domain,
		...
		private class EpisodeAnalysisConstruct extends
			...
			protected Object constructJavaBean2ndStep(MappingNode node,
				...
				if (type.equals(EpisodeAnalysis.class)) {
					...
					List<GroundedAction> actionList = (List<GroundedAction>) map
							.get("actionSequence");
					List<Double> rewardList = (List<Double>) map
							.get("rewardSequence");
					List<SerializableState> stateList = (List<SerializableState>) map
							.get("stateSequence");
					EpisodeAnalysis ea = new EpisodeAnalysis();
					ea.actionSequence = actionList;
					ea.rewardSequence = rewardList;
					ea.stateSequence = SerializableState.deserializeStates(
							stateList, domain);
					return ea;
				} else {
	 ...
	 *         {@link burlap.behavior.singleagent.EpisodeAnalysis} objects.
	 */
	public static List<EpisodeAnalysis> legacyParseFilesIntoEAList(
			String directoryPath, Domain d, StateParser sp) {
	...
	/**
	 * Parses a legacy BURLAP 1.0 string representation from a file and turns it
	 * into an EpisodeAnalysis object.
	 * 
	 * @param path
	 *            the path to the episode file.
	 * @param d
	 ...
	 * @return an EpisodeAnalysis object.
	 */
	public static EpisodeAnalysis legacyParseFileIntoEA(String path, Domain d,
			StateParser sp) {
	...
	/**
	 * Parses a legacy BURLAP 1.0 string representation and turns it into an
	 * EpisodeAnalysis object.
	 * 
	 * @param str
	 ...
	 * @return an EpisodeAnalysis object.
	 */
	public static EpisodeAnalysis legacyParseStringIntoEA(String str, Domain d,
			StateParser sp) {
		...
		for (int i = 1; i < elComps.length; i++) {
			...
			if (i < elComps.length - 1) {
				...
				ea.recordTransitionTo(getGAFromSpaceDelimGAString(d, ars[0]),
						s, Double.parseDouble(ars[1]));
			} else {
				ea.addState(s);
			}
		}
	...
	private static GroundedAction getGAFromSpaceDelimGAString(Domain d,
		...
		if (str.startsWith("*")) {
			...
			GroundedAction primitivePart = getGAFromSpaceDelimGAString(d,
					annotatedParts[1]);
			String annotation = annotatedParts[0].substring(1);
			Policy.GroundedAnnotatedAction ga = new Policy.GroundedAnnotatedAction(
					annotation, primitivePart);
			return ga;
		} else {
			...
			if (a == null) {
				// the domain does not have a reference, so create a null action
				// in its place
	...
	public static void main(String[] args) {
		...
		EpisodeAnalysis ea = p.evaluateBehavior(s, new NullRewardFunction(),
				new NullTermination(), 30);
