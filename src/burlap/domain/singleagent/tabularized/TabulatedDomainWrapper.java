 ...
 * factored state representations or the BURLAP State Hashing. However, some
 * algorithms may be limited to working with states that have explicit
 * enumerated values and with an entire state space already defined. In
 * particular, if you are interfacing with code external to BURLAP this may be
 * required. This domain generator can take any input domain and turns it into a
 * domain in which states are represented by a single int attribute and are
 * fully enumerated. The state space used must be enumerated before calling the
 * {@link #generateDomain()} method and is performed using a BURLAP state
 * enumerator. In particular, seed states must be passed to this object and it
 * will find all reachable states from the seed state and enumerate them.
 * 
...
public class TabulatedDomainWrapper implements DomainGenerator {
	 ...
	 * 
	 * @param inputDomain
	 *            the input domain to be wrapped
	 * @param hashingFactory
	 *            the hashing factory used to enumerate states from the input
	 *            domain
	 */
	public TabulatedDomainWrapper(Domain inputDomain,
	...
	/**
	 * Enumerates all reachable states from the input state to include in this
	 * tabularized domain's state space.
	 * 
	 * @param from
	 *            the souce state from which to find and enumerate all reachable
	 *            states
	 */
	...
	public Domain generateDomain() {
		...
		Attribute att = new Attribute(this.tabDomain, ATTSTATE,
				AttributeType.INT);
	...
	public State getTabularizedState(State s) {
		...
		ObjectInstance o = new MutableObjectInstance(
				this.tabDomain.getObjectClass(CLASSSTATE), "state");
