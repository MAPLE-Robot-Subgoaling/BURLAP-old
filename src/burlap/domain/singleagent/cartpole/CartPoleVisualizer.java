...
public class CartPoleVisualizer {
	...
	public static class CartPoleObjectPainter implements ObjectPainter {
		...
		public void paintObject(Graphics2D g2, State s, ObjectInstance ob,
			...
			Attribute xatt = ob.getObjectClass().getAttribute(
					CartPoleDomain.ATTX);
			...
			g2.fill(new Rectangle2D.Float(scx - cartRadius, scy - cartRadius,
					2 * cartRadius, 2 * cartRadius));

			// manage pole rendering
			float poleLength = 0.5f * cHeight;
			float poleTipX = poleLength * (float) Math.sin(a) + scx;
			float poleTipY = -poleLength * (float) Math.cos(a)
					+ (scy - cartRadius);
