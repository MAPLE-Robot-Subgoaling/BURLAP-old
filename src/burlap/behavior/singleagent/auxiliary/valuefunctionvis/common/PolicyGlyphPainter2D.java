 ...
 * different actions. There are three different ways to paint the policy: (1)
 * just painting the glyph for the most likely action (and all that tie as the
 * most likely); (2) glyphs for all actions whose likelihood is within some
 * threshold of the most likely action; (3) glyphs for all actions with the
 * glyphs scaled by how likely the action is.
 * 
 * 
...
public class PolicyGlyphPainter2D implements StatePolicyPainter {
	...
	/**
	 * MAXACTION paints only glphys for only those actions that have the highest
	 * likelihood <br/>
	 * MAXACTIONSOFTTIE paints the glyphs for all actions whose likelihood is
	 * within some threshold of the most likely action <br/>
	 * DISTSCALED paints glyphs for all actions and scales them by the
	...
	public enum PolicyGlyphRenderStyle {
		...
		MAXACTION(0), MAXACTIONSOFTTIE(1), DISTSCALED(2);
	...
	/**
	 * The name of the attribute that is used for determining the x-position on
	 * the canvas
	 */
	protected String xAttName;

	/**
	 * The name of the attribute that is used for determining the y-position on
	 * the canvas
	 */
	protected String yAttName;

	/**
	 * The name of the class that holds the x-attribute used for determining the
	 * x-posiition on the canvas
	 */
	protected String xClassName;

	/**
	 * The name of the class that holds the y-attribute used for determining the
	 * y-posiition on the canvas
	 */
	protected String yClassName;

	/**
	 * The name of the object that holds the x-attribute used for determining
	 * the x-posiition on the canvas
	 */
	protected String xObjectName;

	/**
	 * The name of the object that holds the y-attribute used for determining
	 * the y-posiition on the canvas
	 */
	protected String yObjectName;

	/**
	 * Option used for specifying the number of possible states that will be
	 * rendered in a row (i.e., across the x-axis). -1 causes the attribute
	 * categorical size to be used instead.
	 */
	protected int numXCells = -1;

	/**
	 * Option used for specifying the number of possible states that will be
	 * rendered in a column (i.e., across the y-axis). -1 causes the attribute
	 * categorical size to be used instead.
	 */
	...
	/**
	 * The max probability difference from the most likely action for which an
	 * action that is not the most likely will still be rendered under the
	...
	/**
	 * Returns the soft difference between max actions to determine ties when
	 * the MAXACTIONSOFSOFTTIE render style is used.
	 * 
	...
	@Override
	public void paintStatePolicy(Graphics2D g2, State s, Policy policy,
		...
		xval = ((float) (xOb.getNumericValForAttribute(xAttName) - xAtt.lowerLim))
				* width;
		...
		yval = cHeight
				- height
				- ((float) (yOb.getNumericValForAttribute(yAttName) - yAtt.lowerLim))
				* height;
		...
		if (this.renderStyle != PolicyGlyphRenderStyle.DISTSCALED) {
			...
			for (ActionProb ap : pdist) {
				if (ap.pSelection >= maxp) {
					ActionGlyphPainter agp = this.actionNameToGlyphPainter
							.get(ap.ga.actionName());
					if (agp != null) {
			...
			for (ActionProb ap : pdist) {
				float[] scaledRect = this.rescaleRect(xval, yval, width,
						height, (float) (ap.pSelection / maxp));
				ActionGlyphPainter agp = this.actionNameToGlyphPainter
						.get(ap.ga.actionName());
				if (agp != null) {
					agp.paintGlyph(g2, scaledRect[0], scaledRect[1],
							scaledRect[2], scaledRect[3]);
				}
			}
		}
	...
	/**
	 * Takes in a rectangle specification and scales it equally along each
	 * direction by a scale factor.
	 * 
	 * @param x
	 ...
	 * @param scale
	 *            the scale factor
	 * @return a double array with the new x, y, width, and height values of the
	 *         scaled rectangle.
	 */
	protected float[] rescaleRect(float x, float y, float width, float height,
			float scale) {
	...
	/**
	 * Returns the object instance in a state that holds the x-position
	 * information.
	 * 
	 * @param s
	 *            the state for which to get the x-position
	 * @return the object instance in a state that holds the x-position
	 *         information.
	 */
	...
	/**
	 * Returns the object instance in a state that holds the y-position
	 * information.
	 * 
	 * @param s
	 *            the state for which to get the y-position
	 * @return the object instance in a state that holds the y-position
	 *         information.
	 */
