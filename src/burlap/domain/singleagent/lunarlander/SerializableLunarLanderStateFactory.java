 ...
 * {@link burlap.domain.singleagent.lunarlander.LunarLanderDomain} states.
 * 
...
public class SerializableLunarLanderStateFactory implements
	...
	public static String stateToString(State s) {
		...
		ObjectInstance agent = s
				.getObjectsOfClass(LunarLanderDomain.AGENTCLASS).get(0);
		ObjectInstance pad = s.getObjectsOfClass(LunarLanderDomain.PADCLASS)
				.get(0);
		List<ObjectInstance> obsts = s
				.getObjectsOfClass(LunarLanderDomain.OBSTACLECLASS);

		// write agent
		buf.append(agent.getRealValForAttribute(LunarLanderDomain.AATTNAME))
				.append(" ");
		buf.append(agent.getRealValForAttribute(LunarLanderDomain.XATTNAME))
				.append(" ");
		buf.append(agent.getRealValForAttribute(LunarLanderDomain.YATTNAME))
				.append(" ");
		buf.append(agent.getRealValForAttribute(LunarLanderDomain.VXATTNAME))
				.append(" ");
		buf.append(agent.getRealValForAttribute(LunarLanderDomain.VYATTNAME))
				.append("\n");

		// write pad
		buf.append(pad.getRealValForAttribute(LunarLanderDomain.LATTNAME))
				.append(" ");
		buf.append(pad.getRealValForAttribute(LunarLanderDomain.RATTNAME))
				.append(" ");
		buf.append(pad.getRealValForAttribute(LunarLanderDomain.BATTNAME))
				.append(" ");
		...
		for (ObjectInstance ob : obsts) {
			buf.append("\n")
					.append(ob
							.getRealValForAttribute(LunarLanderDomain.LATTNAME))
					.append(" ");
			buf.append(ob.getRealValForAttribute(LunarLanderDomain.RATTNAME))
					.append(" ");
			buf.append(ob.getRealValForAttribute(LunarLanderDomain.BATTNAME))
					.append(" ");
	...
	public static State stringToState(Domain domain, String str) {
		...
		LunarLanderDomain.setAgent(s, Double.parseDouble(aComps[0]),
				Double.parseDouble(aComps[1]), Double.parseDouble(aComps[2]),
				Double.parseDouble(aComps[3]), Double.parseDouble(aComps[4]));
		LunarLanderDomain.setPad(s, Double.parseDouble(pComps[0]),
				Double.parseDouble(pComps[1]), Double.parseDouble(pComps[2]),
				Double.parseDouble(pComps[3]));

		for (int i = 2; i < lineComps.length; i++) {
			String[] oComps = lineComps[i].split(" ");
			LunarLanderDomain.setObstacle(s, i - 2,
					Double.parseDouble(oComps[0]),
					Double.parseDouble(oComps[1]),
					Double.parseDouble(oComps[2]),
					Double.parseDouble(oComps[3]));
		}
