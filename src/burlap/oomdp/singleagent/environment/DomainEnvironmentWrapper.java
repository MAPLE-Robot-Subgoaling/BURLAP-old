...
/**
 * If a problem is best described by an Environment class, this domain wrapper
 * takes a domain object that specifies the state representation and action set,
 * and creates a new domain object in which actions of the same name and
 * parameterization will make calls to the provided Environment class and wait
 * for it to return the resulting state before returning themselves.
 * 
...
public class DomainEnvironmentWrapper implements DomainGenerator {
    ...
    /**
     * Initializes this class with the source domain used by an environment
     * object
     * 
     * @param srcDomain
     *            the source domain used by an environment
     * @param env
     *            the environment that this domain generator will wrap
     */
	...
	PropositionalFunction pf = new IndirectPF(src.getName(), d,
		src.getParameterClasses(), src.getParameterOrderGroups(),
		src.getClassName(), src);
	...
	public IndirectPF(String name, Domain domain,
	    ...
	    super(name, domain, parameterClasses, parameterOrderGroup,
		    pfClassName);
	...
	public EnvironmentExecutingAction(Domain d, Action src) {
	    this.init(src.getName(), d, src.getParameterClasses(),
		    src.getParameterOrderGroups());
	}
