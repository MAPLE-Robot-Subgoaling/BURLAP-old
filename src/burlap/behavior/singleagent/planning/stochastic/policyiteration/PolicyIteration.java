...
public class PolicyIteration extends ValueFunctionPlanner {
     ...
     *            when the number of policy evaluation iterations exceeds this
     *            value, policy evaluation will terminate.
    ...
    public PolicyIteration(Domain domain, RewardFunction rf,
	    TerminalFunction tf, double gamma, StateHashFactory hashingFactory,
	    double maxPIDelta, double maxEvalDelta,
    ...
    /**
     * Sets which kind of policy to use whenever the policy is updated. The
     * default is a deterministic greedy policy (
     * {@link burlap.behavior.singleagent.planning.commonpolicies.GreedyDeterministicQPolicy}
     * .
     * 
     * @param p
     *            the policy to use when updating to the new evaluated value
     *            function.
     */
	...
	if (this.performReachabilityFrom(initialState)) {
	    ...
	    double delta;
	    do {
		StaticVFPlanner lastValueFunction = new StaticVFPlanner(
			this.domain, this.rf, this.gamma, this.hashingFactory,
			this.actions, this.valueFunction);
    ...
    /**
     * This is class is merely a wrapper for the value function computed from
     * the previous policy evaluation that can be referenced by Policy objects
     * to derive a policy.
     * 
	...
	public StaticVFPlanner(Domain domain, RewardFunction rf, double gamma,
		StateHashFactory hashingFactory, List<Action> allActions,
		Map<StateHashTuple, Double> srcValueFunction) {
	    this.VFPInit(domain, rf, new NullTermination(), gamma,
		    hashingFactory);
	    ...
	    for (Map.Entry<StateHashTuple, Double> e : srcValueFunction
		    .entrySet()) {
