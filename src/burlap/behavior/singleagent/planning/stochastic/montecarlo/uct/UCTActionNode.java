...
public class UCTActionNode {
	...
	/**
	 * The possible successor states. Stores a list of nodes for the same
	 * outcome state since options may reach the same outcome state after a
	 * different number steps causing a further depth in the tree.
	 */
	public Map<HashableState, List<UCTStateNode>> successorStates;

	/**
	 * Generates a new action node for a given action. All statistics are
	 * initialized to 0.
	 * 
	 * @param a
	 *            the action this node wraps.
	 */
	...
	public void addSuccessor(UCTStateNode node) {
		...
		List<UCTStateNode> succesorsMatchingState = successorStates
				.get(node.state);
	...
	/**
	 * Returns whether this action node has a observed a given successor state
	 * node in the past
	 * 
	 * @param node
	 *            the node which is checked to be in the current successor
	 *            states
	 * @return true if this node contains in its observed successors the input
	 *         state node
	 */
