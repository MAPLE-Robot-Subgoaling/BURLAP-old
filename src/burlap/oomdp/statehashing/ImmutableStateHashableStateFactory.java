 ...
 * ImmutableStateInterface. It caches the value by default, and also will check
 * object equality through the == comparison, since ImmutableStates may share
 * the same objects.
 * 
 * @author brawner
 * 
...
public class ImmutableStateHashableStateFactory extends
	...
	public HashableState hashState(State s) {
		...
		if (!(s instanceof ImmutableStateInterface)) {
			throw new RuntimeException(s.getClass().toString()
					+ " is not an immutable state type.");
		}

		ImmutableStateInterface immState = (ImmutableStateInterface) s;
		List<ImmutableObjectInstance> hashed = new ArrayList<ImmutableObjectInstance>(
				s.numTotalObjects());
		TIntArrayList hashes = new TIntArrayList(s.numTotalObjects());

		for (ImmutableObjectInstance obj : immState) {
			ImmutableHashableObject hashedObj = this.objectHashingFactory
					.hashObject(obj);
		...
		ImmutableList<ImmutableObjectInstance> immList = ImmutableList
				.copyOf(hashed);
		hashes.sort();
		return new ImmutableHashableState(immState.replaceAndHash(immList,
				hashes.hashCode()));
	}
	...
	protected Set<ObjectInstance> prepareSet(ImmutableStateInterface s) {
		...
		if (this.maskedObjects.isEmpty()) {
			List<List<ObjectInstance>> objectsByClass = s
