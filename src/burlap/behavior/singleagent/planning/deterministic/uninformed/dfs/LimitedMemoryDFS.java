 ...
 * state that is in its memory will be pruned from the search tree. This planner
 * gives some of the reduced searching power gained when using DFS with a closed
 * with, but with a constant memory space cost that is traded for less pruning
 * power. If there is an imposed search depth and a potential path will explore
 * a state in the memory, but will explore it at an earlier depth, the planner
 * will explore it anyway, since it's possible it will lead to a goal given that
 * it can expand further than the previous exploration.
 * 
 * <p/>
...
public class LimitedMemoryDFS extends DFS {
     ...
     * nodes the planner will remember.
     */
    ...
    public LimitedMemoryDFS(Domain domain, StateConditionTest gc,
	    StateHashFactory hashingFactory, int maxDepth,
