...
public class UniversalStateParser implements StateParser {
    ...
    /**
     * This parser only requires that the source domain for the states is
     * provided.
     * 
     * @param domain
     *            the domain for which the states will be converted.
     */
	...
	for (int i = 1; i < obcomps.length; i++) { // first comp will be empty
						   // so skip it
	    ...
	    ObjectInstance o = new ObjectInstance(
		    domain.getObjectClass(ocname), oname);
