...
public class Sokoban2Visualizer {
	...
	if (agentImagePath.length == 0) {
	    v.addObjectClassPainter(Sokoban2Domain.CLASSAGENT,
		    new AgentPainter());
	} else {
	    v.addObjectClassPainter(Sokoban2Domain.CLASSAGENT,
		    new AgentPainterWithImages(agentImagePath[0]));
	}
	try {
	    v.addObjectClassPainter(Sokoban2Domain.CLASSBLOCK,
		    new BlockPainter(agentImagePath[0]));
	} catch (Exception e) {
    ...
    public static Visualizer getVisualizer(int maxX, int maxY,
	    String... agentImagePath) {

	Visualizer v = new Visualizer();

	v.addObjectClassPainter(Sokoban2Domain.CLASSROOM, new RoomPainter(maxX,
		maxY));
	v.addObjectClassPainter(Sokoban2Domain.CLASSDOOR, new DoorPainter(maxX,
		maxY));
	if (agentImagePath.length == 0) {
	    v.addObjectClassPainter(Sokoban2Domain.CLASSAGENT,
		    new AgentPainter());
	    ...
	    v.addObjectClassPainter(Sokoban2Domain.CLASSAGENT,
		    new AgentPainterWithImages(agentImagePath[0], maxX, maxY));
	}
	v.addObjectClassPainter(Sokoban2Domain.CLASSBLOCK, new BlockPainter(
		maxX, maxY, agentImagePath[0]));
	...
	public void paintObject(Graphics2D g2, State s, ObjectInstance ob,
	    ...
	    Color rcol = colorForName(ob
		    .getStringValForAttribute(Sokoban2Domain.ATTCOLOR));
    ...
    public static class AgentPainterWithImages implements ObjectPainter,
	    ImageObserver {
	...
	public AgentPainterWithImages(String pathToImageDir) {
	    ...
	    try {
		dirToImage.put("north", ImageIO.read(new File(pathToImageDir
			+ "robotNorth.png")));
		dirToImage.put("south", ImageIO.read(new File(pathToImageDir
			+ "robotSouth.png")));
		dirToImage.put("east", ImageIO.read(new File(pathToImageDir
			+ "robotEast.png")));
		dirToImage.put("west", ImageIO.read(new File(pathToImageDir
			+ "robotWest.png")));
	    } catch (IOException e) {
		e.printStackTrace();
	    }
	...
	public void paintObject(Graphics2D g2, State s, ObjectInstance ob,
	    ...
	    String dir = null;
	    Attribute dirAtt = ob.getObjectClass().getAttribute(
		    Sokoban2Domain.ATTDIR);
	    if (dirAtt != null) {
	    ...
	    g2.drawImage(img, (int) rx, (int) ry, (int) width, (int) height,
		    this);
	...
	protected void initImages(String pathToImageDir) {
		...
		for (String colName : Sokoban2Domain.COLORS) {
		    ...
		    String path = pathToImageDir + shapeName + "/" + key
			    + ".png";
		    try {
	...
	public void paintObject(Graphics2D g2, State s, ObjectInstance ob,
	    ...
	    String colName = ob
		    .getStringValForAttribute(Sokoban2Domain.ATTCOLOR);
	    String shapeName = ob
		    .getStringValForAttribute(Sokoban2Domain.ATTSHAPE);
	    String key = this.shapeKey(shapeName, colName);
	    BufferedImage img = this.shapeAndColToImages.get(key);

	    if (img == null) {
		Color col = colorForName(
			ob.getStringValForAttribute(Sokoban2Domain.ATTCOLOR))
			.darker();
		...
		g2.drawImage(img, (int) rx, (int) ry, (int) width,
			(int) height, this);
	    }
