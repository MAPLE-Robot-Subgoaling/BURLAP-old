...
public class StateValuePainter2D extends StateValuePainter {
	...
	/**
	 * The object to use for returning the color with which to fill the state
	 * cell given its value.
	 */
	...
	/**
	 * Whether the numeric string for the value of the state should be rendered
	 * in its cell or not.
	 */
	...
	/**
	 * A value between 0 and 1 indicating how far from the left of a value cell
	 * the value string should start being rendered. 0 indicates stating at the
	 * left of the cell; 1 the right;
	 */
	protected float vsOffsetFromLeft = 0f;

	/**
	 * A value between 0 and 1 indicating how from from the top of a value cell
	 * the value string should start be rendered. 0 indicates stating at the top
	 * of the cell; 1 the bottom;
	 */
	...
	/**
	 * Initializes using a
	 * {@link burlap.behavior.singleagent.auxiliary.valuefunctionvis.common.LandmarkColorBlendInterpolation}
	 ...
	 * 
	 * @param colorBlend
	 *            the object to use for returning the color with which to fill
	 *            the state cell given its value.
	 */
	...
	public void paintStateValue(Graphics2D g2, State s, double value,
		...
		float normX = (float) (xOb.getNumericValForAttribute(xAttName) - xAtt.lowerLim)
				/ domainXScale;
		xval = normX * cWidth;
		...
		float normY = (float) (yOb.getNumericValForAttribute(yAttName) - yAtt.lowerLim)
				/ domainYScale;
		...
		if (this.renderValueString) {
			...
			String fstring = String
					.format("%." + this.vsPrecision + "f", value);
