 ...
 * agent must jump between different ice blocks. Each time the agent jumps on an
 * ice block, it adds a layer to an igloo that is being built and "activates"
 * all ice blocks on the same row. Jumping on an activated ice block does not ad
 * a layer to the igloo. Once all rows of ice blocks are activated, they reset
 * and can be activated by jumping on them again. Once the igloo is built, the
 * agent can go to it to win the game. If the agent jumps or walks into the
 * water, the game is over. <br/>
 * <br/>
 * If you run the main method of this class, it will launch of a visual explorer
 * that you can play. They keys w,s,a,d,x correspond to the actions jump north,
 * jump south, move west, move east, do nothing. If you win or lose the visual
 * explorer will automatically terminate. If you want it to keep running, you
 * can set this class' public static {@link #visualizingDomain} data member to
 * false.
 * 
...
public class FrostbiteDomain implements DomainGenerator {
	...
	/**
	 * Main function to test the domain. Note: The termination conditions are
	 * not checked when testing the domain this way, which means it is
	 * impossible to win or die and might trigger bugs. To enable them,
	 * uncomment the code in the "update" function.
	 * 
	 * @param args
	 */
	 ...
	 *            the y position of the agent
	 * @param h
	 *            the height of the agent (0 is ground)
	 */
	 ...
	 * 
	 * @param s
	 *            the state in which the platform should be set
	 * @param i
	 *            specifies the ith platform object to be set to these values
	 * @param x
	 ...
	 * @param ss
	 *            the platform size
	 * @param activatedStatus
	 *            the platform status
	 */
	public static void setPlatform(State s, int i, int x, int y, int ss,
	 ...
	 *            domain
	 * @param s
	 *            the state to initialize the platforms in
	 * @param row
	 *            the row id (starts at 0). Defines the direction platforms are
	 *            going to move.
	 */
	private static void setPlatformRow(Domain d, State s, int row) {
		for (int i = 0; i < numberPlatformCol; i++) {
			ObjectInstance platform = new MutableObjectInstance(
					d.getObjectClass(PLATFORMCLASS), PLATFORMCLASS
							+ (i + row * numberPlatformCol));
			s.addObject(platform);

			platform.setValue(XATTNAME, spaceBetweenPlatforms * i
					+ ((row % 2 == 0) ? 0 : gameWidth / 3));
			platform.setValue(YATTNAME, gameIceHeight + jumpSize / 2
					- platformSize / 2 + agentSize / 2 + jumpSize * row);
	...
	public Domain generateDomain() {
		...
		Attribute hatt = new Attribute(domain, HEIGHTATTNAME,
				Attribute.AttributeType.INT);
		hatt.setLims(-83, 83);

		Attribute satt = new Attribute(domain, SIZEATTNAME,
				Attribute.AttributeType.INT);
		satt.setLims(0, gameWidth);

		Attribute batt = new Attribute(domain, BUILDINGATTNAME,
				Attribute.AttributeType.INT);
		satt.setLims(0, 256); // It's an Atari game, it should crash at some
								// point!

		Attribute aatt = new Attribute(domain, ACTIVATEDATTNAME,
	 ...
	 * platforms and screen borders
	 * 
	 * @param s
	 *            the current state
	 * @param xd
	 *            the attempted X position increment of the agent
	 * @param yd
	...
	protected void move(State s, int xd, int yd) {
		...
		if (leftToJump == 0 && yd != 0) {
			// Player can only jump when on a platform (except last line), or
			// when hitting down on the top part
			if ((platformSpeedOnAgent != 0 && ay + yd * jumpSize < gameHeight
					- agentSize)
					|| (platformSpeedOnAgent == 0 && yd > 0)) {
		...
		if (nx < 0 || nx >= gameWidth - agentSize || ny < 0
				|| ny >= gameHeight - agentSize) {
			nx = ax;
			ny = ay;
		}
	...
	/**
	 * Collision check between a point (player center) and a platform, including
	 ...
	 *            point Y coordinate
	 * @param x
	 *            platform top left corner X coordinate
	 * @param y
	 *            platform top left corner Y coordinate
	 * @param s
	 *            platform size
	...
	private boolean pointInPlatform(int px, int py, int x, int y, int s) {
		if (pointInPlatformHelper(px, py, x, y, s))
			return true;
		if (x + s > FrostbiteDomain.gameWidth
				&& pointInPlatformHelper(px, py, x - gameWidth, y, s))
			return true;
