 ...
 * and does not respond to the other player's actions. The policy object that
 * determines actions can leave the actingAgent field empty/null, because this
 * the {@link #getAction(State)} method will automatically rpelace it with
 * whatever this agent's name is.
 * 
...
public class SetStrategyAgent extends Agent {
    ...
    /**
     * Initializes for the given domain in which the agent will play and the
     * strategy that they will follow.
     * 
     * @param domain
     *            the domain in which the agent will play
     * @param policy
     *            the strategy that the agent will follow
     */
	...
	GroundedSingleAction actSelection = (GroundedSingleAction) this.policy
		.getAction(s);
