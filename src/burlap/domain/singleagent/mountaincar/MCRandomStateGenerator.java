 ...
 * specified range and the velocity between some specified range. By default,
 * the ranges are the full ranges supported by the domain, but they can be
...
public class MCRandomStateGenerator implements StateGenerator {
	...
	/**
	 * Initializes for the {@link MountainCar} {@link Domain} object for which
	 * states will be generated. By default, the random x and velocity ranges
	 * will be the full range used by the domain.
	 * 
	 * @param domain
	 *            the {@link MountainCar} {@link Domain} object for which states
	 *            will be generated
	 */
	 ...
	 */
	public MCRandomStateGenerator(Domain domain, double xmin, double xmax,
	...
	public State generateState() {
		...
		ObjectInstance agent = new MutableObjectInstance(
				this.domain.getObjectClass(MountainCar.CLASSAGENT),
				MountainCar.CLASSAGENT);
		s.addObject(agent);

		double rx = this.rand.nextDouble() * (this.xmax - this.xmin)
				+ this.xmin;
		double rv = this.rand.nextDouble() * (this.vmax - this.vmin)
				+ this.vmin;
