...
public class FrostbiteVisualizer {
	...
	public static class IglooPainter implements ObjectPainter {
		...
		public void paintObject(Graphics2D g2, State s, ObjectInstance ob,
			...
			int building = ob
					.getIntValForAttribute(FrostbiteDomain.BUILDINGATTNAME);
			...
			for (; layer < Math.min(maxLayer, building) - 1; layer++) {
				...
				g2.fill(new Rectangle2D.Double(iglooOffsetx + 3
						* FrostbiteDomain.gameWidth / 4, iglooOffsety
						+ FrostbiteDomain.gameHeight / 5 - brickHeight * layer,
			...
			if (building >= maxLayer) {
				...
				g2.fill(new Rectangle2D.Double(3 * FrostbiteDomain.gameWidth
						/ 4 + FrostbiteDomain.gameWidth / 12 - doorWidth / 2,
						FrostbiteDomain.gameHeight / 5 - doorHeight / 2,
