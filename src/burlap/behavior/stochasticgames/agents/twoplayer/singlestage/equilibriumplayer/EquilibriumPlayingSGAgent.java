...
/**
 * This agent plays an equilibrium solution for two player games based on the
 * immediate joint rewards received for the given state, as if it is a single
 * stage game. By default, the solution concept used will be {@link MaxMax} -
 * assuming the other agent will choose actions that maximize your reward.
 * Different solution concepts can be used by providing a different
 * {@link BimatrixEquilibriumSolver} object in the constructor.
 * 
...
public class EquilibriumPlayingSGAgent extends SGAgent {
	...
	/**
	 * Initializes with strategies formed usign the solution concept generated
	 * by the given solver.
	 * 
	 * @param solver
	 *            the solver to use for a given solution concept.
	 */
	...
	/**
	 * Constructs a bimatrix game from the possible joint rewards of the given
	 * state. The other agent and their action set is determined by retreiving
	 * the corresponding agent object from the world. Similarly for the joint
	 * action model. If this agent has an internal reward function, they use
	 * that; otherwise the world reward function is used.
	 * 
	 * @param s
	 *            the state from which the joint rewards are based
	 * @param myActions
	 *            the set of
	 *            {@link burlap.oomdp.stochasticgames.agentactions.GroundedSGAgentAction}
	 *            s the agent can taken in s.
	 * @return a {@link BimatrixTuple} for the joint reward function.
	 */
	protected BimatrixTuple constructBimatrix(State s,
			List<GroundedSGAgentAction> myActions) {
		...
		List<GroundedSGAgentAction> opponentActions = SGAgentAction
				.getAllApplicableGroundedActionsFromActionList(s,
						opponent.getAgentName(),
						opponent.getAgentType().actions);

		BimatrixTuple bimatrix = new BimatrixTuple(myActions.size(),
				opponentActions.size());
		for (int i = 0; i < bimatrix.nRows(); i++) {
			...
			for (int j = 0; j < bimatrix.nCols(); j++) {
				...
				bimatrix.setPayoff(i, j, r.get(this.worldAgentName),
						r.get(opponent.getAgentName()));
	...
	/**
	 * Returns the {@link burlap.oomdp.stochasticgames.SGAgent} object in the
	 * world for the opponent.
	 * 
	 * @return the {@link burlap.oomdp.stochasticgames.SGAgent} object in the
	 *         world for the opponent.
	 */
	protected SGAgent getOpponent() {
		...
		if (agents.size() != 2) {
			throw new RuntimeException(
					"EquilibriumPlayingAgent is only defined for two player games and there are "
							+ agents.size() + " players in the world.");
		}
