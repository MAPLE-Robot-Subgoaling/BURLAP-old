 ...
 * RLGlue {@link org.rlcommunity.rlglue.codec.taskspec.TaskSpec}. The
 * representation consists of up to two objects. One object contains all RLGlue
 * discrete attributes and the other all Real (double) attributes. The domain
 * can only support RLGlue problems that have discrete actions. The created
 * BURLAP {@link burlap.oomdp.singleagent.Action} objects that correspond to the
 * RLGlue actions cannot be applied to states since RLGlue does not provide
 * action transition dynamics; as a consequence a runtime exception will be
 * thrown is the action
 ...
 * method is called. Instead, only the
...
public class RLGlueDomain implements DomainGenerator {
	...
	/**
	 * The object class name for the object that holds the RLGlue real-valued
	 * (double) attributes
	 */
	public static final String REALCLASS = "real";

	/**
	 * The base name of a discrete attribute. The ith discrete attribute will be
	 * named DISCATTi
	 */
	public static final String DISCATT = "disc";

	/**
	 * The base name of a real (double) attribute. The ith real attribute will
	 * be named REALATTi
	 */
	...
	public Domain generateDomain() {
		...
		for (int i = 0; i < theTaskSpec.getNumDiscreteObsDims(); i++) {
			Attribute a = new Attribute(domain, DISCATT + i,
		...
		for (int i = 0; i < theTaskSpec.getNumContinuousObsDims(); i++) {
			Attribute a = new Attribute(domain, REALATT + i,
		...
		if (theTaskSpec.getNumDiscreteActionDims() != 1
				|| theTaskSpec.getNumContinuousActionDims() > 0) {
			throw new RuntimeException(
					"Can only create domains with one discrete action dimension");
		}

		for (int i = 0; i < theTaskSpec.getDiscreteActionRange(0)
				.getRangeSize(); i++) {
