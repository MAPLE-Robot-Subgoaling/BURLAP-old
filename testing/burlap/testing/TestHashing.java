...
public class TestHashing {
	...
	public void testSimpleHashFactory() {
		...
		Set<HashableState> hashedStates = this.getReachableHashedStates(
				startState, domain, factory);
	...
	public void testImmutableSimpleHashFactoryIdentifierDependentCached() {
		...
		HashableStateFactory factory = new SimpleHashableStateFactory(true,
				true);
		Set<HashableState> hashedStates = this.getReachableHashedStates(
				new ImmutableState(startState), domain, factory);
		assert (hashedStates.size() == 104);
	}
	...
	public void testSimpleHashFactoryLargeState(HashableStateFactory factory,
			int width, int numRandomStates, boolean moveLocObjects) {
		...
		Set<HashableState> hashedStates = this.generateRandomStates(domain,
				startState, factory, width, numRandomStates, moveLocObjects);
		Set<Integer> hashes = new HashSet<Integer>();
		for (HashableState hs : hashedStates) {
			hashes.add(hs.hashCode());
		}
		System.out.println("Hashed states: " + hashedStates.size()
				+ ", hashes: " + hashes.size());
	}
	...
	public void testSimpleHashFactoryLargeStateIdentifierDependent() {
		...
		System.err.println("Hashed states: " + hashedStates.size()
				+ ", hashes: " + hashes.size());
	...
	public Set<HashableState> generateStates(SADomain domain, State state,
			HashableStateFactory factory, int width) {
		Set<HashableState> hashedStates = new HashSet<HashableState>();
		for (int i = 0; i < width; ++i) {
			for (int j = 0; j < width; ++j) {
				State copy = state.copy();
				copy.getObjectsOfClass(GridWorldDomain.CLASSAGENT).get(0)
						.setValue(GridWorldDomain.ATTX, i);
				copy.getObjectsOfClass(GridWorldDomain.CLASSAGENT).get(0)
						.setValue(GridWorldDomain.ATTY, j);
	...
	public Set<HashableState> generateRandomStates(SADomain domain,
			State state, HashableStateFactory factory, int width,
			int numStates, boolean moveLocations) {
		Set<HashableState> hashedStates = new HashSet<HashableState>();
		...
		while (hashedStates.size() < numStates) {
			...
			State copy = state.copy();
			copy.getObjectsOfClass(GridWorldDomain.CLASSAGENT).get(0)
					.setValue(GridWorldDomain.ATTX, random.nextInt(width));
			copy.getObjectsOfClass(GridWorldDomain.CLASSAGENT).get(0)
					.setValue(GridWorldDomain.ATTY, random.nextInt(width));

			if (moveLocations) {
				for (ObjectInstance loc : copy
	...
	public Set<HashableState> getReachableHashedStates(State from,
		...
		while (openList.size() > 0) {
			...
			// sh.s.getAllGroundedActionsFor(actions);
			...
			for (GroundedAction ga : gas) {
				...
				for (TransitionProbability tp : tps) {
					...
					for (HashableState hashedState : hashedStates) {
						...
						boolean hashEquals = (hashedState.hashCode() == nsh
								.hashCode());
						if (sameObject || valueEquals) {
							assert (hashEquals); // Same state, hashes need to
													// be equal
						}
						if (!hashEquals) {
