 ...
 * objects that computes hash codes and test for state equality after
 * discretizing any real values. Discretizing is performed by flooring real
 * values to the nearest user-defined multiple. For example, if the multiple is
 * set to 0.5, then 9.73 would become 9.5, and 102.3 would become 102. Note
 * using a multiple value of 1.0 is equivalent to floor values to their
 * corresponding int value. Large multiple values result in course
 * discretization and small multiple values result in a fine discretization. <br/>
 * <br/>
 * The multiple used can be specified for individual attributes so that
 * different attributes have different degrees of discretization. To set the
 * multiple used for an specific attribute, use the
 * {@link #addFloorDiscretizingMultipleFor(String, double)} method. When a
 * continuous attribute is to be hashed or compared, it is first checked if
 * there has been a specific multiple value set for it. If so, that multiple is
 * used for discretization. If not, the default multiple is used. The default
 * multiple may be set though the constructors or by using the
 * {@link #setDefaultFloorDiscretizingMultiple(double)} method. <br/>
 * <br/>
 * This class extends
 ...
 */
public class DiscretizingHashableStateFactory extends
	 ...
	 */
	public DiscretizingHashableStateFactory(boolean identifierIndependent,
			boolean useCached, double defaultMultiple) {
