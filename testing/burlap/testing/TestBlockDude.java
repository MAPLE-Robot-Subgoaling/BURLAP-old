...
public class TestBlockDude {
	...
	public void testDude(State s) {
		...
		AStar astar = new AStar(domain, rf, sc,
				new SimpleHashableStateFactory(), new NullHeuristic());
		...
		/*
		 * BlockDude constructor = new BlockDude(); Domain d =
		 ...
		 * 
		 * State o = BlockDude.getCleanState(d, px, ph, 6); o =
		 * BlockDude.setAgent(o, 9, 3, 1, 0); o = BlockDude.setExit(o, 1, 0);
		 * 
		 * o = BlockDude.setBlock(o, 0, 5, 1); o = BlockDude.setBlock(o, 1, 6,
		 * 1); o = BlockDude.setBlock(o, 2, 14, 3); o = BlockDude.setBlock(o, 3,
		 * 16, 4); o = BlockDude.setBlock(o, 4, 17, 4); o =
		 * BlockDude.setBlock(o, 5, 17, 5);
		 * 
		 * TerminalFunction tf = new
		 * SinglePFTF(d.getPropFunction(BlockDude.PFATEXIT)); StateConditionTest
		 * sc = new SinglePFSCT(d.getPropFunction(BlockDude.PFATEXIT));
		 * 
		 * RewardFunction rf = new UniformCostRF();
		 * 
		 * AStar astar = new AStar(d, rf, sc, new DiscreteStateHashFactory(),
		 * new NullHeuristic()); astar.toggleDebugPrinting(false);
		 * astar.planFromState(o);
		 * 
		 * Policy p = new SDPlannerPolicy(astar); EpisodeAnalysis ea =
		 * p.evaluateBehavior(o, rf, tf, 100);
		 * 
		 * State lastState = ea.stateSequence.get(ea.stateSequence.size() - 1);
		 * Assert.assertEquals(true, tf.isTerminal(lastState));
		 * Assert.assertEquals(true, sc.satisfies(lastState));
