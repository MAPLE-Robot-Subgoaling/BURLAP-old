...
/**
 * This abstract class provides the interface and comment mechanisms for
 * defining a stochastic game's transition dynamics. The
 * {@link #performJointAction(State, JointAction)} method first makes a copy of
 * a state and passes it to the {@link #actionHelper(State, JointAction)}
 * method, which is what should be overridden by subclasses. The
 * {@link #transitionProbsFor(State, JointAction)} method defines the transition
 * probabilities and should also be overridden by subclasses if this model is to
 * be used with planning algorithms that require it.
 * 
...
public abstract class JointActionModel {
    ...
    /**
     * Performs {@link JointAction} ja in {@link burlap.oomdp.core.State} s and
     * returns the result. The input state is not modified by this operation.
     * 
     * @param s
	...
	// first make sure that every action satisfies the necessary
	// preconditions
	for (GroundedSingleAction gsa : ja) {
	    if (!gsa.action.isApplicableInState(s, gsa.actingAgent, gsa.params)) {
		throw new RuntimeException("The action " + gsa.toString()
     ...
     * {@link JointAction} action in the given state. Transition probabilities
     ...
     * @return a list of state {@link burlap.oomdp.core.TransitionProbability}
     *         objects.
     */
     ...
     * executed in {@link burlap.oomdp.core.State} s. The input state should be
     * directly modified.
     * 
     * @param s
     *            the state in which the joint action is performed.
     ...
     * probability 1 and whose state is determined by querying the {@link
     * performJointAction(State, JointAction)} method.
     * 
     * @param s
