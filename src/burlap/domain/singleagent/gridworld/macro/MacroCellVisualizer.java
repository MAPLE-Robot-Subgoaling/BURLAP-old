...
/**
 * A class for visualizing the reward weights assigned to a Macro-cell in a
 * Macro-cell grid world. Reward weights provided are first normalized to lie
 * between 0 and 1. Macro cells are filled in with color that ranges from white
 * if the reward weight is 0, to blue if the reward weight 1. The agent is
 * rendered as a grey circle.
 * 
...
public class MacroCellVisualizer extends GridWorldVisualizer {
	...
	public static class MacroCellRewardWeightPainter implements StaticPainter {
		...
		public MacroCellRewardWeightPainter(int[][] map,
				MacroCellGridWorld.InMacroCellPF[] propFunctions,
				Map<String, Double> rewardMap) {
				...
				for (int j = 0; j < this.rewardMap[0].length; j++) {
					for (int k = 0; k < propFunctions.length; k++) {
						if (propFunctions[k].isTrue(i, j)) {
							this.rewardMap[i][j] += rewardMap
									.get(propFunctions[k].getName());
						}
					}
				}
			}
