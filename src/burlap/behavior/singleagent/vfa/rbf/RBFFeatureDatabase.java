 ...
 * approximation. This formalization for RBFs is general to any kind of state
 * distance measure and can therefore potentially exploit the OO-MDP structure
 * of states. However, if you plan on using Gaussian RBF units with standard
 * Euclidean distance measures, it is recommended that you use the
 * {@link burlap.behavior.singleagent.vfa.rbf.FVRBFFeatureDatabase} instead, as
 * it will be more computationally efficient.
 * 
 * @author Anubhav Malhotra and Daniel Fernandez and Spandan Dutta; modified by
 *         James MacGlashan
 * 
 */
	...
	protected int getActionMultiplier(GroundedAction ga) {
		...
		if (ga instanceof AbstractObjectParameterizedGroundedAction) {
			throw new RuntimeException(
					"RBF Feature Database does not support actions with AbstractObjectParameterizedGroundedActions.");
		}
