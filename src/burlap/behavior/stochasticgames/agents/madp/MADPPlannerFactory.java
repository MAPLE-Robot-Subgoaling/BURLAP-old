 ...
 * objects. This is useful for the {@link MADPPlanAgentFactory} since it allows
 * planning objects to be deployed with the generation of agents. A factory that
 * returns a constant object reference is supplied as well as one for generating
 * new value iteration instances.
 * 
...
public interface MADPPlannerFactory {
	...
	/**
	 * {@link burlap.behavior.stochasticgames.madynamicprogramming.MADynamicProgramming}
	 * factory that always returns the same object instance, unless the
	 * reference is chaned with a mutator.
	 * 
	 * @author James MacGlashan
	 * 
	 */
	public static class ConstantMADPPlannerFactory implements
			MADPPlannerFactory {
		 ...
		 * 
		 * @param plannerRefernece
		 *            the valueFunction reference to return
		 */
	...
	public static class MAVIPlannerFactory implements MADPPlannerFactory {
		...
		/**
		 * The state hashing factory used to query the value function for
		 * individual states
		 */
		 ...
		 *            the backup operator that defines the solution concept
		 *            being solved
		 * @param maxDelta
		 *            the threshold that causes VI to terminate when the max
		 *            Q-value change is less than it
		 * @param maxIterations
		 *            the maximum number of iterations allowed
		 */
		public MAVIPlannerFactory(SGDomain domain,
				JointActionModel jointActionModel, JointReward jointReward,
				TerminalFunction terminalFunction, double discount,
				HashableStateFactory hashingFactory, double qInit,
				SGBackupOperator backupOperator, double maxDelta,
				int maxIterations) {
		...
		@Override
		public MADynamicProgramming getPlannerInstance() {
			return new MAValueIteration(domain, agentDefinitions, jointReward,
					terminalFunction, discount, hashingFactory, qInit,
