...
/**
 * Computes the correlated equilibrium strategy. The default constructor will
 * cause this object to compute the correlated equilibrium for the
 * {@link CorrelatedEquilibriumObjective#UTILITARIAN} objective. A different
 * objective may be set using the corresponding constructor.
 * 
...
public class CorrelatedEquilibrium extends BimatrixEquilibriumSolver {
	...
	public double[] computeColStrategy(double[][] rowPayoff,
		...
		if (this.objective != CorrelatedEquilibriumObjective.LIBERTARIAN) {
			jointStrategy = CorrelatedEquilibriumSolver
					.getCorrelatedEQJointStrategy(this.objective, rowPayoff,
							colPayoff);
			return GeneralBimatrixSolverTools
					.marginalizeColPlayerStrategy(jointStrategy);
		} else {
			// libertarian assumes row player request, so transpose matrices
			jointStrategy = CorrelatedEquilibriumSolver
					.getCorrelatedEQJointStrategy(this.objective,
							GeneralBimatrixSolverTools
									.transposeMatrix(rowPayoff),
							GeneralBimatrixSolverTools
									.transposeMatrix(colPayoff));
