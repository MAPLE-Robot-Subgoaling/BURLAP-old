 ...
 * There exists a single table and any number of blocks that can be stacked on
 * each other. Blocks can be specified to have the color red, green, or blue.
 * Because this is a relational domain, when performing planning, the
 * {@link burlap.oomdp.statehashing.HashableStateFactory} should be object
 * identifier dependent. For example, if using a
 * {@link burlap.oomdp.statehashing.SimpleHashableStateFactory}, in its
 * constructor specify identifierIndependent=false,
 * 
...
public class BlocksWorld implements DomainGenerator {
	...
	public Domain generateDomain() {
		...
		Attribute attonblock = new Attribute(domain, ATTONBLOCK,
				Attribute.AttributeType.RELATIONAL);

		Attribute attontable = new Attribute(domain, ATTONTABLE,
				Attribute.AttributeType.BOOLEAN);

		Attribute attclear = new Attribute(domain, ATTCLEAR,
				Attribute.AttributeType.BOOLEAN);

		Attribute attcolor = new Attribute(domain, ATTCOLOR,
	...
	/**
	 * Creates a new state with nBlocks block objects in it. Values for created
	 * objects are unset, so be sure to set them after call thing method.
	 * 
	 * @param d
	 *            the blocks world domain
	 * @param nBlocks
	 *            the number of block objects to create
	 ...
	 * 
	 * @param s
	 *            the state in which the block object exists
	 * @param blockInd
	 *            the index of the block object whose color value should be set
	 * @param color
	 *            the categorical color value (either "red", "green", or "blue")
	 */
	 ...
	 * 
	 * @param s
	 *            the state in which the block object should be set
	 * @param blockInd
	 *            the index of the block object whose values should be set
	 * @param onBlock
	 *            the relational on block value
	 * @param onTable
	 *            the binary on table value
	 * @param clear
	 *            the binary clear value
	 * @param color
	 *            the categorical color value (either "red", "green", or "blue")
	 */
	public static void setBlock(State s, int blockInd, String onBlock,
			int onTable, int clear, String color) {
		setBlock(s.getAllObjects().get(blockInd), onBlock, onTable, clear,
				color);
	}
	 ...
	 *            the state in which the block object should be set
	 * @param blockName
	 *            the name identifier of the block object whose values are to be
	 *            set.
	 * @param onBlock
	 *            the relational on block value
	 * @param onTable
	 ...
	 *            the categorical color value (either "red", "green", or "blue")
	 */
	public static void setBlock(State s, String blockName, String onBlock,
			int onTable, int clear, String color) {
	 ...
	 * 
	 * @param block
	 *            the block object instance to whose values should be set
	 * @param onBlock
	 *            the relational on block value
	 * @param onTable
	 *            the binary on table value
	 ...
	 *            the categorical color value (either "red", "green", or "blue")
	 */
	public static void setBlock(ObjectInstance block, String onBlock,
			int onTable, int clear, String color) {
	...
	/**
	 * Action class for stacking one block onto another. The both blocks must be
	 * clear for one to be stacked on the other.
	 * 
	 * @author James MacGlashan
	 * 
	 */
	public class StackAction extends ObjectParameterizedAction implements
			FullActionModel {
		...
		public boolean applicableInState(State st, GroundedAction groundedAction) {
			...
			String[] params = ((AbstractObjectParameterizedGroundedAction) groundedAction)
					.getObjectParameters();
		...
		protected State performActionHelper(State st,
			...
			src.setValue(ATTONTABLE, 0); // may not have been on table to start,
											// but make sure it's not set now
	...
	/**
	 * Action class for unstacking a block off of another block and putting it
	 * on the table. The block to be unstacked must be clear and must be on
	 * another block to be unstacked.
	 * 
	 * @author James MacGlashan
	 * 
	 */
	public class UnstackAction extends ObjectParameterizedAction implements
			FullActionModel {
	...
	/**
	 * Propositional function class for evaluating whether one block is on
	 * another.
	 * 
	 * @author James MacGlashan
	 * 
	...
	public class OnBlockPF extends PropositionalFunction {
		...
		public boolean isTrue(State st, String... params) {
			ObjectInstance src = st.getObject(params[0]);
			ObjectInstance target = st.getObject(params[1]);
			if (src.getStringValForAttribute(ATTONBLOCK).equals(
					target.getName())) {
				return true;
			}
			return false;
	...
	/**
	 * Propositional function class for evaluating whether one block is on a
	 * table.
	 * 
	 * @author James MacGlashan
	 * 
	...
	/**
	 * Propositional function class for evaluating whether one block is clear
	 * (has no block stacked on top of it).
	 * 
	 * @author James MacGlashan
	 * 
	 */
	...
	/**
	 * Propositional function class for evaluating whether a block is a certain
	 * color. This class will be instantiated for each possible color of a
	 * block.
	 * 
	 * @author James MacGlashan
	 * 
	...
	/**
	 * Main method for exploring the domain. The initial state will have 3 red
	 * blocks starting on the table. By default this method will launch the
	 * visual explorer. Pass a "t" argument to use the terminal explorer.
	 * 
	 * @param args
	 */
	public static void main(String[] args) {
		...
		if (expMode == 0) {
			...
			VisualExplorer exp = new VisualExplorer(domain,
					BlocksWorldVisualizer.getVisualizer(24), s);
