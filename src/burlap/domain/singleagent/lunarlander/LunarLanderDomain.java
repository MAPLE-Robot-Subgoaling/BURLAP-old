...
/**
 * This domain generator is used for producing a Lunar Lander domain, based on
 * the classic Lunar Lander Atari game (1979). The agent pilots a lunar lander
 * that has a variety of thrust actions and can rotate clockwise or counter
 * clockwise. The agent may also do nothing and just drift. Using a thrust
 * action exerts force in the opposite direction the lander is oriented. Gravity
 * acts on the agent so the agent must find the right balance of thrust to take
 * off and reach some designated landing position.
 * <p/>
 * The agent object class is made up of 5 attributes, x-y position, x-y
 * velocity, and the angle or orientation (in radians from the default vertical
 * orientation). Two other object classes are defined for landing pads and
 * obstacles. Both are objects are defined as a rectangular region with a left,
 * right, bottom, and top attribute defining the space. Typically, the goal is
 * for the agent to land on the land pad while avoiding the obstacles.
 * <p/>
 * The domain generator allows the client to specify the physics and action
 * definitions for the lunar lander. In particular the client can add as many
 * thrust actions (each with their own thrust force) as desired, the force of
 * gravity can be changed, the size of the domain (that is, the x space and y
 * space in which the agent can fly before hitting a "wall") can be changed, the
 * maximum lander rotation can be set, and the rotate increment size that
 * results from applying a clockwise/counterclockwise rotate action can also be
 * set. There is also a method to set the domain to a standard set of physics
 * and actions.
 * <p/>
 * If the domain generator physics parameters are changed after a domain has
 * been generated, the previously generated domain will remain unaffected,
 * allowing you to reuse the same domain generator to produce different versions
 * of the domain without conflict.
 * 
...
public class LunarLanderDomain implements DomainGenerator {
	...
	/**
	 * Constant for the name of the left boundary attribute for rectangular
	 * obstacles and landing pads
	 */
	public static final String LATTNAME = "lAtt";

	/**
	 * Constant for the name of the right boundary attribute for rectangular
	 * obstacles and landing pads
	 */
	public static final String RATTNAME = "rAtt";

	/**
	 * Constant for the name of the bottom boundary attribute for rectangular
	 * obstacles and landing pads
	 */
	public static final String BATTNAME = "bAtt";

	/**
	 * Constant for the name of the top boundary attribute for rectangular
	 * obstacles and landing pads
	 */
	...
	/**
	 * Constant for the base name of thrust actions. Each thrust action will
	 * have a number appended to this name to indicate which thrust action it
	 * is.
	 */
	public static final String ACTIONTHRUST = "thrust";

	/**
	 * Constant for the name of the idle action which causes the agent to do
	 * nothing by drift for a time step
	 */
	public static final String ACTIONIDLE = "idle";

	/**
	 * Constant for the name of the propositional function that indicates
	 * whether the agent/lander is on a landing pad
	 */
	public static final String PFONPAD = "onLandingPad";

	/**
	 * Constant for the name of the propositional function that indicates
	 * whether the agent/lander is *touching* a landing pad. Note that the agent
	 * may be touching the pad on the side of its boundaries and therefore not
	 * be landed on it. However if the agent is landed on the pad, then they
	 * will also be touching it.
	 */
	public static final String PFTPAD = "touchingLandingPad";

	/**
	 * Constant for the name of the propositional function that indicates
	 * whether the agent/lander is touching an obstacle surface.
	 */
	public static final String PFTOUCHSURFACE = "touchingSurface";

	/**
	 * Constant for the name of the propositional function that indicates
	 * whether the agent/lander is on the ground
	 */
	...
	/**
	 * Sets the agent/lander position/orientation and zeros out the lander
	 * velocity.
	 * 
	 * @param s
	 *            the state in which to set the agent
	 * @param a
	 ...
	 *            the angle of the lander
	 * @param x
	 *            the x position of the lander
	 * @param y
	 *            the y position of the lander
	 * @param vx
	 *            the x velocity component of the lander
	 * @param vy
	 *            the y velocity component of the lander
	 */
	public static void setAgent(State s, double a, double x, double y,
			double vx, double vy) {
		ObjectInstance agent = s.getObjectsOfClass(AGENTCLASS).get(0);
	 ...
	 * 
	 * @param s
	 *            the state in which the obstacle should be set
	 * @param i
	 *            specifies the ith landing pad object to be set to these values
	 * @param l
	 *            the left boundary
	 * @param r
	 *            the right boundary
	 ...
	 *            the top boundary
	 */
	public static void setObstacle(State s, int i, double l, double r,
			double b, double t) {
	 ...
	 *            the top boundary
	 */
	public static void setPad(State s, int i, double l, double r, double b,
			double t) {
	...
	/**
	 * Updates the position of the agent/lander given the provided thrust force
	 * that has been exerted
	 * 
	 * @param s
	 *            the state in which the agent/lander should be modified
	 * @param thrust
	 *            the amount of thrust force exerted by the lander.
	 */
	protected static void updateMotion(State s, double thrust,
			LLPhysicsParams physParams) {
