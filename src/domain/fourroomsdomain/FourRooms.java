...
/**
 * A Domain implementing a Four Rooms, Grid-World style setup complete with four
 * doorways for the agent to traverse through. It holds two attributes
 * [ATTX,ATTY] to declare the position of the respective Object Classes
 * [CLASSAGENT, CLASSGOAL]. The agent is restricted to 4 possible motions
 * [ACTIONNORTH, ACTIONSOUTH, ACTIONEAST, ACTIONWEST] and one goal oriented
 * propositional function [PFATGOAL].
 * 
...
public class FourRooms implements DomainGenerator {
    ...
    public static final ArrayList<String> colors = new ArrayList<String>(
	    Arrays.asList(new String[] { "white", "blue", "red" }));
	...
	if (expMode == 0) {
	    ...
	    p.evaluateBehavior(s, rf, tf).writeToFile("output/OOMDP_Planner",
		    parser);
	...
	Action east = new PrimitiveOption(
		new EastAction(ACTIONEAST, DOMAIN, ""));
	Action west = new PrimitiveOption(
		new WestAction(ACTIONWEST, DOMAIN, ""));
	...
	Q = new QLearning(DOMAIN, rf, tf, FourRooms.DISCOUNTFACTOR,
		hashFactory, 0.2, FourRooms.LEARNINGRATE, Integer.MAX_VALUE);
	S = new SarsaLam(DOMAIN, rf, tf, FourRooms.DISCOUNTFACTOR, hashFactory,
		0.2, FourRooms.LEARNINGRATE, FourRooms.LAMBDA);
	planner = new ValueIteration(DOMAIN, rf, tf, FourRooms.DISCOUNTFACTOR,
		hashFactory, 0.001, 100);
	...
	// the order of the objects determines the order in which they are drawn
	// -Richard
	...
	public boolean satisfies(State s) {
	    ...
	    int aSx = agentState.getDiscValForAttribute(ATTX); // X&Y for state
							       // passed in
	    int aSy = agentState.getDiscValForAttribute(ATTY);

	    int aCx = agentCondition.getDiscValForAttribute(ATTX); // X&Y for
								   // the
								   // Condition
	    int aCy = agentCondition.getDiscValForAttribute(ATTY);

	    if ((aSx == aCx) && (aSy == aCy)) // Only Checks the Agent's
					      // relative position
		return true;
	    return false;
	}
