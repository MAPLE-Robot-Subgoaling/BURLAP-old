 ...
 * failures. The domain consists of only object classes: an agent class and a
 * location class, each of which is defined by and x and y position. Walls are
 * not considered objects; instead walls are considered part of the transition
 * dynamics. There are five propositional functions supported: atLocation(agent,
...
public class GridWorldDomain implements DomainGenerator {
	...
	xatt.setDiscValuesForRange(0, this.width - 1, 1); // -1 due to
							  // inclusivity vs
							  // exclusivity

	Attribute yatt = new Attribute(DOMAIN, ATTY,
		Attribute.AttributeType.DISC);
	yatt.setDiscValuesForRange(0, this.height - 1, 1); // -1 due to
							   // inclusivity vs
							   // exclusivity
	...
	Action north = new MovementAction(ACTIONNORTH, DOMAIN,
		this.transitionDynamics[0]);
	Action south = new MovementAction(ACTIONSOUTH, DOMAIN,
		this.transitionDynamics[1]);
	Action east = new MovementAction(ACTIONEAST, DOMAIN,
		this.transitionDynamics[2]);
	Action west = new MovementAction(ACTIONWEST, DOMAIN,
		this.transitionDynamics[3]);

	PropositionalFunction atLocationPF = new AtLocationPF(PFATLOCATION,
		DOMAIN, new String[] { CLASSAGENT, CLASSLOCATION });

	PropositionalFunction wallToNorthPF = new WallToPF(PFWALLNORTH, DOMAIN,
		new String[] { CLASSAGENT }, 0);
	PropositionalFunction wallToSouthPF = new WallToPF(PFWALLSOUTH, DOMAIN,
		new String[] { CLASSAGENT }, 1);
	PropositionalFunction wallToEastPF = new WallToPF(PFWALLEAST, DOMAIN,
		new String[] { CLASSAGENT }, 2);
	PropositionalFunction wallToWestPF = new WallToPF(PFWALLWEST, DOMAIN,
		new String[] { CLASSAGENT }, 3);
	...
	s.addObject(new ObjectInstance(d.getObjectClass(CLASSLOCATION),
		CLASSLOCATION + 0));
	s.addObject(new ObjectInstance(d.getObjectClass(CLASSAGENT),
		CLASSAGENT + 0));
	...
	public boolean isTrue(State st, String[] params) {
	    ...
	    if (cx < 0 || cx >= GridWorldDomain.this.width || cy < 0
