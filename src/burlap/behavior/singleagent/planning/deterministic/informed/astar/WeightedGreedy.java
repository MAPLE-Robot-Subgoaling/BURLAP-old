...
/**
 * A weighted greedy search in which the g(n) function (the cost to the current
 * node) is weighted by a fraction 0 <= w <= 1. When w = 0, the search is fully
 * greedy. When w = 1, the search is optimal and equivalent to A*.
 * 
 * <p/>
...
public class WeightedGreedy extends AStar {
	 ...
	 * @param costWeight
	 *            a fraction 0 <= w <= 1. When w = 0, the search is fully
	 *            greedy. When w = 1, the search is optimal and equivalent to
	 *            A*.
	 */
	public WeightedGreedy(Domain domain, RewardFunction rf,
			StateConditionTest gc, HashableStateFactory hashingFactory,
			Heuristic heuristic, double costWeight) {
