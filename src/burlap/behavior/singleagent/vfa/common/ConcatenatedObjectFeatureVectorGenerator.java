...
/**
 * This class is used to produce a state feature vector that is the
 * concatenation of the observable attributes of objects belonging to a
 * specified sequence of object classes. The feature vector attribtues are
 * ordered first by the class of object and then by the order of the objects
 * that appear for each object of that class. Values in the feature vector can
 * optionally be specfied to be the normalized object values, which uses the
 * attributes lowerlims and upperlims attribute to determine normalization.
 * 
...
public class ConcatenatedObjectFeatureVectorGenerator implements
	...
	/**
	 * The order of object classes and their order to follow when concatenting
	 * objects into a single state feature vector.
	 */
	 ...
	 *            whether the values in the feature vector should be normalized.
	 * @param objectClassOrder
	 *            the sequence of object classes to use when constructing a
	 *            state feature vector.
	 */
	public ConcatenatedObjectFeatureVectorGenerator(boolean normalizeValues,
			String... objectClassOrder) {
