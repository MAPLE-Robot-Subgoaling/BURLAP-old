...
public class BlocksWorldVisualizer {
	 ...
	 * object identifier will be painted on the block and the color of the
	...
	public static class BlockPainter implements ObjectPainter {
		...
		/**
		 * Initializes with a font size for rendering the name of the block
		 * object.
		 * 
		 * @param fontSize
		 *            a font size for rendering the name of the block object.
		 */
		...
		public void paintObject(Graphics2D g2, State s, ObjectInstance ob,
			...
			g2.setColor(this.getColorForString(ob
					.getStringValForAttribute(BlocksWorld.ATTCOLOR)));

			float rx = ind * blockWidth;
			float ry = cHeight - blockHeight - this.getHeight(s, ob)
					* blockHeight;

			g2.fill(new Rectangle2D.Float(rx + (hGap), ry, blockWidth - 2
					* hGap, blockHeight));
			...
			int stringLen = (int) g2.getFontMetrics()
					.getStringBounds(valueString, g2).getWidth();
			int stringHeight = (int) g2.getFontMetrics()
					.getStringBounds(valueString, g2).getHeight();
