...
public class TabularModel extends Model {
     ...
     *            model's prediction.
     */
    public TabularModel(Domain sourceDomain, StateHashFactory hashingFactory,
	    int nConfident) {
    ...
    /**
     * Returns the {@link StateActionNode} object associated with the given
     * hashed state and action. If there is not an associated
     * {@link StateActionNode} object, then null is returned.
     * 
     * @param sh
     *            the hashed state
     * @param ga
     *            the grounded action
     * @return the associated {@link StateActionNode} or null if it does not
     *         exist.
     */
    protected StateActionNode getStateActionNode(StateHashTuple sh,
	    GroundedAction ga) {
     ...
     * Returns the {@link StateActionNode} object associated with the given
     * hashed state and action. If there is not an associated
     * {@link StateActionNode} object, then one will be created.
     * 
     * @param sh
     *            the hashed state
     * @param ga
     ...
     */
    protected StateActionNode getOrCreateActionNode(StateHashTuple sh,
	    GroundedAction ga) {
	...
	if (sn == null) {
	    ...
	    List<GroundedAction> allActions = sh.s
		    .getAllGroundedActionsFor(this.sourceDomain.getActions());
	 ...
	 */
	public StateActionNode(GroundedAction ga, double r,
		StateHashTuple sprime) {
	    this.ga = ga;
	    this.sumR = r;
	    this.nTries = 1;
