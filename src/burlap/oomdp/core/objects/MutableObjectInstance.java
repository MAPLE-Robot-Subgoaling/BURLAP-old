 ...
 * instance has a name identifier that is unique from any other object instances
 ...
 * 
 */
public class MutableObjectInstance extends OOMDPObjectInstance implements
		ObjectInstance {

	/**
	 * The OO-MDP {@link burlap.oomdp.core.ObjectClass} of which this object is
	 * an instance.
	 */
	...
	/**
	 * Returns the boolean value of the attribute (only defined for boolean
	 * attributes, int, and disc values).
	 * 
	 * @param attName
	 *            the name of the attribute whose value should be returned
	...
	/**
	 * Returns a double vector of all the values in this object instance.
	 * Discrete values have their int stored valued converted to a double for
	 * this array. This method will throw a runtime exception if the object
	 * instance includes relational values. This method may be useful if objects
	 * need to be indexed in data structures like kd-trees.
	 * 
	...
	/**
	 * Returns a normalized double vector of all the values in this object
	 * instance. This method relies on the lowerlims and upperlims being set for
	 * the corresponding attribute. Furthermore, this method will throw a
	 * runtime exception if the object instance includes attributes that are
	 * *not* type REAL or INT.
	 * 
	 * @return a normalized double vector of all the observable values in this
	 *         object instance.
	 */
	@Override
		...
		for (int i = 0; i < obsFeatureVec.length; i++) {
			...
			if (a.type != AttributeType.REAL && a.type != AttributeType.INT) {
				throw new RuntimeException(
						"Cannot get a normalized numeric value for attribute "
								+ a.name
								+ " because it is not a REAL or INT type.");
			}
