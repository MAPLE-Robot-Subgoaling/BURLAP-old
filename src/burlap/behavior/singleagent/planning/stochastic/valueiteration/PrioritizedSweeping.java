 ...
 * described by Li and Littman [1]. This class will perform Bellman updates on
 * states according to their position in a Priority queue. The priority of any
 * state is updated with respect to the change in the Bellman error of a state
 * to which it transitions. This means that there is greater memory utilization
 * in this algorithm than standard VI because the backwards transition dynamics
 * must be stored. The priority queue takes C*lg(N) time to manage at each step,
 * where C is the number of backpointers per state, but if large gains can be
 * achieved by the ordeing of the states, then this cost may be worth it.
 * 
 * 
 * 1. Li, Lihong, Michael L. Littman, and L. Littman. Prioritized sweeping
 * converges to the optimal value function. Tech. Rep. DCS-TR-631, 2008.
 * 
...
public class PrioritizedSweeping extends ValueIteration {
	 ...
	 * @param maxBackups
	 *            the maximum number of Bellman backups. If set to -1, then
	 *            there is no hard limit.
	 */
	public PrioritizedSweeping(Domain domain, RewardFunction rf,
			TerminalFunction tf, double gamma,
			HashableStateFactory hashingFactory, double maxDelta, int maxBackups) {
		super(domain, rf, tf, gamma, hashingFactory, maxDelta, 0);
		this.priorityNodes = new HashIndexedHeap<PrioritizedSweeping.BPTRNode>(
				new BPTRNodeComparator());
	...
	public boolean performReachabilityFrom(State si) {
		...
		while (openList.size() > 0) {
			...
			if (this.tf.isTerminal(node.sh.s)
