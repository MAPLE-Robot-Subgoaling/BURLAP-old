 ...
 * Javadoc test
 * 
 * @author Richard Adjogah This is the visualizer for the Sokoban domain.
 * 
 */
public class SokobanVisualizer {
	...
	v.addObjectClassPainter(SokobanDomain.AGENTCLASS,
		rv.new AgentPainter(d));
	v.addObjectClassPainter(SokobanDomain.BLOCKCLASS,
		rv.new BlockPainter(d));

	return v;
    }

    /**
     * Note: Each individual object type should have its own painter class that
     * determines what color to draw and where
     * 
     * @author richard
     * 
     */
	...
	public void paintObject(Graphics2D g2, State s, ObjectInstance ob,
	    ...
	    // determines which java Color to use from a String. Defaults to
	    // Dark Gray if the string isn't a valid color
	    String color = ob
		    .getStringValForAttribute(SokobanDomain.COLORATTNAME);
	    ...
	    float bottomX = ob
		    .getDiscValForAttribute(SokobanDomain.BOTTOMXATTNAME);
	    float topY = ob.getDiscValForAttribute(SokobanDomain.TOPYATTNAME);
	    float bottomY = ob
		    .getDiscValForAttribute(SokobanDomain.BOTTOMYATTNAME);
	...
	public void paintObject(Graphics2D g2, State s, ObjectInstance ob,
	    ...
	    float rx = ob.getDiscValForAttribute(SokobanDomain.XATTNAME)
		    * width;
	    float ry = cHeight - height
		    - ob.getDiscValForAttribute(SokobanDomain.YATTNAME)
		    * height;
