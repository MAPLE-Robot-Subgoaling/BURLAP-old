...
/**
 * A class for producing {@link burlap.oomdp.statehashing.HashableState} objects
 * that computes hash codes and tests for {@link burlap.oomdp.core.states.State}
 * equality by discretizing real-valued attributes and by masking (ignoring)
 * either {@link burlap.oomdp.core.Attribute}s and/or
 * {@link burlap.oomdp.core.ObjectClass}es. For more information on how
 * discretization is performed, see the {@link DiscretizingHashableStateFactory}
 * class documentation and for more information on how
 * {@link burlap.oomdp.core.Attribute}/{@link burlap.oomdp.core.ObjectClass}
 * masking is performed see the {@link MaskedHashableStateFactory} class
 * documentation. <br/>
 * <br/>
 * This class extends
 * {@link burlap.oomdp.statehashing.SimpleHashableStateFactory}, which means it
 * can be toggled to to be object identifier/name independent or dependent and
 * can be set to use {@link burlap.oomdp.statehashing.HashableState} instances
 * that hash their hash code or not. See the
 * {@link burlap.oomdp.statehashing.SimpleHashableStateFactory} class
 * documentation for more information on those features.
 * 
 * @author James MacGlashan.
 */
public class DiscretizingMaskedHashableStateFactory extends
	...
	/**
	 * The default multiple to use for any continuous attributes that have not
	 * been specifically set.
	 */
	protected double defaultMultiple;

	/**
	 * Initializes with object identifier independence, no hash code caching and
	 * object class or attribute masks.
	 * 
	 * @param defaultMultiple
	 *            The default multiple to use for any continuous attributes that
	 *            have not been specifically set. The default is 1.0, which
	...
	/**
	 * Initializes with non hash code caching and object class or attribute
	 * masks
	 * 
	 ...
	 *            corresponds to integer floor discretization.
	 */
	public DiscretizingMaskedHashableStateFactory(
			boolean identifierIndependent, double defaultMultiple) {
	 ...
	 * @param defaultMultiple
	 *            The default multiple to use for any continuous attributes that
	 *            have not been specifically set. corresponds to integer floor
	 *            discretization.
	 */
	public DiscretizingMaskedHashableStateFactory(
			boolean identifierIndependent, boolean useCached,
	 ...
	 * @param maskNamesAreForAttributes
	 *            whether the specified masks are masks for attributes or object
	 *            classes. True for attributes, false for object classes.
	 * @param masks
	 *            the names of the {@link burlap.oomdp.core.Attribute}s or
	 *            {@link burlap.oomdp.core.ObjectClass} that will be masks
	 *            (ignored from state hashing and equality checks)
	 */
	public DiscretizingMaskedHashableStateFactory(
			boolean identifierIndependent, boolean useCached,
			double defaultMultiple, boolean maskNamesAreForAttributes,
			String... masks) {
		super(identifierIndependent, useCached, maskNamesAreForAttributes,
				masks);
		this.defaultMultiple = defaultMultiple;
	}
	 ...
	 * the greatest value that is a multiple of the multiple given and less than
	 * or equal to the true value.
	 * 
	 * @param attributeName
	 *            the name of the attribute whose discretization multiple is
	 *            being set.
	 * @param nearestMultipleValue
	 *            the multiple to which values are floored.
	 */
	...
	protected void appendHashcodeForValue(HashCodeBuilder hashCodeBuilder,
		...
		if (attClass == AttClass.DOUBLE) {
			...
			if (mult != null) {
				...
				hashCodeBuilder.append(intMultiples(this.defaultMultiple,
						v.getRealVal()));
			}
		} else if (attClass == AttClass.DOUBLEARRAY) {
			Double multPointer = attributeWiseMultiples.get(v.attName());
			double mult = multPointer == null ? this.defaultMultiple
					: multPointer;
	...
	/**
	 * Returns whether two values are equal. If the values are real-valued, they
	 * are discretized before comparison.
	 * 
	 * @param v1
	 *            the first value to compare
	 * @param v2
	 *            the second value to compare
	 * @return true if v1 = v2 after accounting for discretization; false
	 *         otherwise.
	 */
	@Override
	protected boolean valuesEqual(Value v1, Value v2) {
		...
		if (attClass == AttClass.DOUBLE) {
			...
			return intMultiples(mult, v1.getRealVal()) == intMultiples(mult,
					v2.getRealVal());
		} else if (attClass == AttClass.DOUBLEARRAY) {
			Double multP = attributeWiseMultiples.get(v1.attName());
			double mult = multP == null ? this.defaultMultiple : multP;
			...
			for (int i = 0; i < array1.length; i++) {
				if (intMultiples(mult, array1[i]) != intMultiples(mult,
						array2[i])) {
					return false;
				}
			}
