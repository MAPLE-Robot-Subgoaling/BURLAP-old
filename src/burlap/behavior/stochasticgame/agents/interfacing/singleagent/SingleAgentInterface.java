...
/**
 * For a number of reasons outside the scope of this class description, BURLAP
 * single agent learning algorithms use a different interface for interacting
 * with the world than stochastic games agents do. Specifically, single agent
 * learning algorithms make calls to actions that modify the world, whereas in
 * stochastic games, the world requests actions from the agent and then
 * subsequently tells the agent about the results. This stochastic games agent
 * class provides an interface so that any BURALP single agent learning
 * algorithm can be used in a stochastic games world. Specifically, this class
 * works by running the single agent learning algorithm in a separate thread and
 * synchronizing action selection and state outcomes between the two paradigms.
 * The only information that neeeds to be provided to this class is the
 * stochastic games domain in which this agent will play and a special single
 * agent learning algorithm factory to produce the single agent learning
...
public class SingleAgentInterface extends Agent {
    ...
    /**
     * whether a new single agent learning episode needs to be started for the
     * next action request
     */
    ...
    /**
     * Initializes for a given stochastic games domain and a factory to produce
     * the single agent learning object
     * 
     * @param sgDomain
     *            the source stochastic games domain to be played
     * @param saAgentFactory
     *            the single learning agent factory to use to perform learning
	...
	if (this.saAgent == null) {
	    SGToSADomain dg = new SGToSADomain(this.domain, this.agentType,
		    this);
	    this.saDomain = (SADomain) dg.generateDomain();
	    this.saAgent = this.saAgentFactory.generateAgentForRLDomain(
		    this.saDomain, new SARFWrapper(), new SATFWrapper());
	}
	...
	if (this.needsToStartEpisode == true) {
		...
		@Override
		public void run() {
		    SingleAgentInterface.this.saAgent
			    .runLearningEpisodeFrom(fs);
		}
	    });
	    saThread.start();
    ...
    /**
     * A method that receives calls from the single agent domain actions to
     * inform this stochastic games agent which action to take next when
     * requested by the world.
     * 
     * @param ga
     *            the single agent grounded aciton selection
     * @return the state that will be the result of the agent applying the
     *         corresponding action in the stochastic games world.
     */
    public State receiveSAAction(GroundedAction ga) {

	SingleAction sgaction = this.domain.getSingleAction(ga.actionName());
	GroundedSingleAction gsa = new GroundedSingleAction(
		this.worldAgentName, sgaction, ga.params);
    ...
    /**
     * A termianl function that returns true when the last RLGlue state was
     * terminal.
     * 
     * @author James MacGlashan
     * 
