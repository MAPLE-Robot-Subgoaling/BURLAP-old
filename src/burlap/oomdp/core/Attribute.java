 ...
 * Currently, there are attributes to support discrete values, real values, and
 * relational values. Discrete values are represented by an integer, but these
 * integers can be specified to correspond to categorical values represented as
 * string. Discrete attributes require a finite domain (that is, it must be
 * specified how many different discrete values there are). There are two
 * different kinds of real attributed; bounded and unbounded. A a bounded real
 * attribute should have a lower limit value and an upper limit value. Unbounded
 * real attributes may span any range. Both attributes' values are represented
 * by double data types. There are also two kinds of relational attributes:
 * single target and multi-target. A single target relational attribute can only
 * be linked to one other object, whereas a multi-target relational attribute
 * can be linked to set of other objects. In both cases, the relational
 * attribute can be unset to any target. The single-target relational attribute
 * values are represented by strings the specify the name reference of the
 * object to which they are connected. Multi-target relational attribute values
 * are represented by an ordered set of string values indicating the targets to
 * which the attribute is connected.
 * 
 * Attributes may also be specified as "hidden," which means that they should
 * not be used by the agent planning/learning algorithms when resolving the
 * state. Hidden attributes may be useful for defining POMDP domains or in
 * facilitating the generation of values for observable attributes.
 * 
...
public class Attribute {
     ...
     * not categorical) 6: BOOLEAN (0, or 1, discrete value for false, true)
     * 
     * @author James MacGlashan
     * 
     */
    ...
    /**
     * whether this value is part of the state representation or is hidden from
     * the agent
     */
     ...
     * "false" and "true" with a range from 0 to 1.
     * 
     * @param domain
     *            the domain that the attribute exists in
     * @param name
    ...
    /**
     * Sets the possible range of discrete values for the attribute. The
     * categorical values will be set to the the string representation of each
     * integer number.
     * 
     * @param low
	...
	} else if (this.type == AttributeType.REAL
		|| this.type == AttributeType.REALUNBOUND) {
