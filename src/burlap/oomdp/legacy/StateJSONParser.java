...
/**
 * A StateParser class that uses the JSON file format and can can convert states
 * to JSON strings (and back from them) for any possible input domain. Use of
 * this class requires the Jackson java library. This class also provides
 * methods for preparing an OO-MDP {@link State} into a datastructure that can
 * be easily parsed into an JSON representation and for taking a datastructure
 * preapred for a JSON representation and converting it into an OO-MDP
 * {@link State}.
 * 
...
public class StateJSONParser implements StateParser {
	...
	/**
	 * The domain holding the object class definitions that states represented
	 * in JSON strings will be converted to. This is not necessary if parsing
	 * will only go from states to strings and not strings to states.
	 */
	protected Domain domain;

	/**
	 * Initializes with a given domain object. The domain object is used for
	 * converting from strings to states. Specifically, the object instances
	 * will be assigned to the object class in the domain object with the given
	 * name in the state string.
	 * 
	 * @param domain
	 *            the domain object to which state objects will be associated.
	 */
	 ...
	 * 
	 * @param s
	 *            the input state to turn into a JSON prepared datastructure
			...
			for (Attribute a : o.getObjectClass().attributeList) {
				if (a.type == AttributeType.BOOLEAN) {
					Boolean bval = new Boolean(
							o.getIntValForAttribute(a.name) == 1);
				...
				} else if (a.type == AttributeType.REAL
						|| a.type == AttributeType.REALUNBOUND) {
				...
				} else if (a.type == AttributeType.STRING
						|| a.type == AttributeType.INTARRAY
						|| a.type == AttributeType.DOUBLEARRAY) {
					objectData.put(a.name, o.getStringValForAttribute(a.name));
				}
			}
	 ...
	 * maps. Each map represents an object instance which stores the objects
	 * name, name of the object's class, and the value for each attribute.
	 * 
	 * @param objects
	 *            the list of OO-MDP object instances
	...
	public State JSONPreparedToState(List<Map<String, Object>> objects) {
		...
		for (Map<String, Object> oMap : objects) {
			...
			ObjectInstance ob = new MutableObjectInstance(
					this.domain.getObjectClass(className), obName);
