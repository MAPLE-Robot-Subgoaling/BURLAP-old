 ...
 * you to set a mask, to only hash a subset of objects or object classes.
 * Because it only works for FixedSizeImmutableStates, it hashes objects
 * specifically in the order there exist in the state. It also iterates staight
 * through when doing the state equality comparison, allowing for significant
 * speedups for certain domains.
 * 
 * @author brawner
 * 
 */
public class FixedSizeStateHashableStateFactory extends
	...
	public void setObjectClassMask(boolean value, String... objectClassNames) {
		...
		for (String objectClassName : objectClassNames) {
			for (int i = 0; i < this.initialState.numTotalObjects(); i++) {
				if (this.initialState.getObject(i).getObjectClass().name
	...
	protected boolean statesEqual(State s1, State s2) {
		...
		if (!iS1.isHashed() || !iS2.isHashed()) {
			throw new RuntimeException(
					"These states should be hashed for this equality comparison");
		}
