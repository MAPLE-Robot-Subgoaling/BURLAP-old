...
/**
 * 
 * @author James The four rooms domains is a basic block world domain in which
 *         the agent has to navigate to the goal square. This class should be
 *         used as a basis for anyone interested in developing a more
 *         complicated grid world domain.
 */
public class FourRoomsDomain implements DomainGenerator {
    ...
    /*
     * A variable of type Domain The attributes of the Domain class are:private
     * List <ObjectClass> objectClasses; //list of object classesprivate Map
     * <String, ObjectClass> objectClassMap; //look up object classes by name
     * 
     * private List <Attribute> attributes; //list of attributesprivate Map
     * <String, Attribute> attributeMap; //lookup attributes by name
     * 
     * private List <PropositionalFunction> propFunctions; //list of
     * propositional functionsprivate Map <String, PropositionalFunction>
     * propFunctionMap; //lookup propositional functions by name
     * 
     * private List <Action> actions; //list of actionsprivate Map <String,
     * Action> actionMap; //lookup actions by name
     */
	...
	/*
	 * Creating an object called fourRoomsDomain() This object has no
	 * constructor nor any attributes that would differ from each object
	 */
	...
	s.addObject(new ObjectInstance(DOMAIN.getObjectClass(CLASSAGENT),
		CLASSAGENT + 0));
	s.addObject(new ObjectInstance(DOMAIN.getObjectClass(CLASSGOAL),
		CLASSGOAL + 0));
     ...
     *            the state to modify
     * @param x
     *            the new x coordinate of the goal
     * @param y
     *            the new y coordinate of the goal
     */
    ...
    /**
     * Makes a big box with the walls being set to the value of 1 So, (0,0) is
     * one point in th Map and since it is the main frame, the value is set to 1
     */
    ...
    /**
     * Creates horizontal and vertical walls within the bigger box created in
     * frameMap() see the documentation for horizontalWall() and verticalWall()
     ...
     * walls and blocks This is a helper function for the North, South, East,
     * and West actions.
     * 
     * @param the
     *            current state
    ...
    /**
     * 
     * @author James Propositional function for determining whether the agent is
     *         in the same position as the goal.
     */
