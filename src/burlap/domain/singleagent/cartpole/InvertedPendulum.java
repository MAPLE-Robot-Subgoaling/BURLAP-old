...
/**
 * A simplified version of the {@link CartPoleDomain} in which the movement of
 * the pole depends only on gravity and the force applied, and not the velocity
 * of the underlying cart. The track is also always assumed to be infinite.
 * Therefore, the state space for this domain is fully described by two
 * variables: the angle and angular velocity of the pole. However, there is also
 * noise included in the actions of this domain as well a noop action. This
 * version of the inverted pendulum is the version used in the original
...
public class InvertedPendulum implements DomainGenerator {
	...
	public static class IPPhysicsParams {
		 ...
		 * non-realistic at pi/2; task should terminate before then.
		 */
		...
		/**
		 * The force (magnitude) noise in any action, including the no force
		 * action.
		 */
		public double actionNoise = 10.;

		/**
		 * The maximum speed (manitude) of the change in angle. The default sets
		 * it to 1
		 */
		...
		public IPPhysicsParams(double angleRange, double gravity,
				double cartMass, double poleMass, double poleLength,
				double actionForce, double actionNoise, double maxAngleSpeed,
				double timeDelta) {
		...
		public IPPhysicsParams copy() {
			return new IPPhysicsParams(angleRange, gravity, cartMass, poleMass,
					poleLength, actionForce, actionNoise, maxAngleSpeed,
					timeDelta);
		}
	}
	 ...
	 *            the input state
	 * @param controlForce
	 *            the control force acted upon the cart.
	 * @param physParams
	 *            the
	 ...
	 */
	public static void updateState(State s, double controlForce,
		...
		double num = (physParams.gravity * sinA)
				- (alpha * physParams.poleMass * physParams.poleLength * av0
						* av0 * Math.sin(2. * a0) * 0.5)
				- (alpha * cosA * controlForce);

		double denom = ((4. / 3.) * physParams.poleLength) - alpha
				* physParams.poleMass * physParams.poleLength * cosA * cosA;
