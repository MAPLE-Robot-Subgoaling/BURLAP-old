...
/**
 * This domain generator is used to produce single agent domain version of a
 * stochastic games domain for an agent of a given type (specified by an
 * {@link burlap.oomdp.stochasticgames.SGAgentType} object or for a given list
 * of stochastic games agent actions (
 * {@link burlap.oomdp.stochasticgames.agentactions.SGAgentAction}). Each of the
 * stochastic game agent actions is converted into a single agent
 * {@link burlap.oomdp.singleagent.Action} object with the same action name and
 * parametrization. The created {@link burlap.oomdp.singleagent.SADomain}'s
 * {@link burlap.oomdp.singleagent.Action} objects maintain the action
 * specification of the input {@link burlap.oomdp.stochasticgames.SGDomain}'s
 * {@link burlap.oomdp.stochasticgames.agentactions.SGAgentAction} (that is,
 * their name and parameter types), but the
 ...
 * method is undefined since the transition dynamics would depend on the action
 * selection of other agents, which is unknown. Instead, actions can only be
 * executed through the
 * {@link burlap.oomdp.singleagent.Action#performInEnvironment(burlap.oomdp.singleagent.environment.Environment, burlap.oomdp.singleagent.GroundedAction)}
 * method only in which the specified
 * {@link burlap.oomdp.singleagent.environment.Environment} handles the
 * decisions of the other agents. For example, this domain can typically be
 * paired with the {@link LearningAgentToSGAgentInterface}, which will handle
 * these calls indirectly by simultaneously acting as a stochastic game
 * {@link burlap.oomdp.stochasticgames.SGAgent}.
 * 
...
public class SGToSADomain implements DomainGenerator {
	...
	/**
	 * A single agent action wrapper for a stochastic game action. Calling this
	 * action will cause it to call the corresponding single interface to inform
	 * it of the action selection. The constructed action will have the same
	 * name and object parametrization specification as the source stochastic
	 * game {@link burlap.oomdp.stochasticgames.agentactions.SGAgentAction}
	 * object.
	 * 
	 * @author James MacGlashan
	 * 
	...
	public static class SAActionWrapper extends Action {
		...
		public boolean applicableInState(State s, GroundedAction groundedAction) {
			return this.srcAction
					.applicableInState(
							s,
							((GroundedSAAActionWrapper) groundedAction).wrappedSGAction);
		}
