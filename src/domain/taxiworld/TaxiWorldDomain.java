...
public class TaxiWorldDomain implements DomainGenerator {
    ...
    public static final double DISCOUNTFACTOR = 0.995;
	...
	Attribute datt = new Attribute(DOMAIN, ATTDROPPED,
		Attribute.AttributeType.DISC);
	...
	Action pickUp = new PrimitiveOption(new PickUpAction(ACTIONPICKUP,
		DOMAIN, ""));
	Action dropOff = new PrimitiveOption(new DropOffAction(ACTIONDROPOFF,
		DOMAIN, ""));
	...
	ObjectInstance pass = s.getObjectsOfTrueClass(CLASSPASS).get(num - 1);
	...
	if (pass.getDiscValForAttribute(ATTCARRIED) == 1
		&& agent.getDiscValForAttribute(ATTCARRY) != 0 && ax == gx
		&& ay == gy) {
	    agent.setValue(ATTCARRY, 0);
