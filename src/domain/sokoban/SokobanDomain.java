...
/**
 * 
 * @author richard This domain is an implementation of the classic computer game
 *         Sokoban. See http://en.wikipedia.org/wiki/Sokoban for details. It's
 *         largely based off the four rooms domain, but with a number of
 *         improvements including support for reading trajectories from files, a
 *         fairly robust command line interface, and shape/color descriptors to
...
public class SokobanDomain implements DomainGenerator, StateParser {
    ...
    public static final ArrayList<String> colors = new ArrayList<String>(
	    Arrays.asList(new String[] { "black", "blue", "cyan", "darkGray",
		    "gray", "green", "lightGray", "magenta", "orange", "pink",
		    "red", "white", "yellow" }));

    public static final ArrayList<String> shapes = new ArrayList<String>(
	    Arrays.asList(new String[] { "star", "moon", "circle", "smiley",
		    "square" }));
	...
	/*
	 * if (args.length == 0 || args[0].equals("-help")){ System.out.println(
	 * "Usage: SokobanDomain [-c commandsfile] [-t] [-v] [-f statefile]");
	 * System.out.println("-c runs a set of commands from a text file");
	 * System
	 * .out.println("-d runs a set of commands from a directory of text files"
	 * ); System.out.println("-t runs the terminal domain explorer");
	 * System.out.println("-v runs the visual domain explorer");
	 * System.out.println(
	 * "-f will load a starting state from a text file rather than using the default one"
	 * ); System.out.println(
	 * "-w will prevent the creation of walls (warning: this may have unintented consequences)"
	 * ); System.exit(0); }
	 */
	// check for all flags
	...
	// option for setting the size of the domain; not very well tested
	// don't use
	if (sLocation != -1) {
	...
	if (fLocation != -1) {
		...
		System.out.println("Error: File \"" + args[fLocation + 1]
			+ "\" not found.");
		System.exit(1);
	    }
	    try {
	    ...
	    Map<String, Set<PropositionalFunction>> ms = domain
		    .getPropositionlFunctionsMap();

	    for (Map.Entry<String, Set<PropositionalFunction>> entry : ms
		    .entrySet()) {
	...
	// read trajectory files from directory
	else if (dLocation != -1) {
	    BufferedReader in = null;
	    ...
	    ArrayList<TrainingElement> elements = new ArrayList<TrainingElement>();

	    try {
		File directory = new File(args[dLocation + 1]);
		File[] listOfFiles = directory.listFiles();
		for (File file : listOfFiles) {
		    if (file.getPath().contains(".svn") != true) {
			...
			ArrayList<String> stringActions = new ArrayList<String>(
				Arrays.asList(trajectoryString.split(" ")));
			// perform actions from file
			for (int i = 0; i < stringActions.size(); i++) {
			    Action action = domain.getAction(stringActions
				    .get(i));
			    actions.add(new GroundedAction(action, ""));
			    st = action.performAction(st, "");
			}
			trajectory = new Trajectory(states, actions);
			TrainingElement element = new TrainingElement(command,
				trajectory);
			elements.add(element);
			System.out.println("File \"" + file.getName()
				+ "\" has been completed with command \""
				+ element.command + "\"");
		    }
		    // System.out.println(constructor.stateToString(element.trajectory.getState(element.trajectory.numStates()
		    // - 1)));
		}
	    } catch (FileNotFoundException e) {
		System.out.println("Error: Directory \"" + args[dLocation + 1]
			+ "\" not found.");
		System.exit(1);
	    } catch (IOException e) {
		e.printStackTrace();
	...
	for (ObjectInstance obj : st.getAllObjects()) {
	    ...
	    if ((numRooms == 1 && str.equals("room"))
		    || (numBlocks == 1 && str.equals("block"))
		    || (numAgents == 1 && str.equals("agent"))
		    || (numDoors == 1 && str.equals("door"))) {
			...
			for (int j = MINY; j <= MAXY; j++) {
				...
				for (int w = MINY; w <= MAXY; w++) {
					...
					String[] roomList = new String[] {
						...
						"" + i + ",", "" + j + ",",
						"" + k + ",", "" + w };
					totalRoomList.add(SokobanDomain
						.arrToString(roomList));
					...
					String[] agentList = new String[] {
						"agent" + ",", "" + k + ",",
						"" + w };
					totalAgentList.add(SokobanDomain
						.arrToString(agentList));
				    } else if (str.equals("door") && n == 0) {
					String[] doorList = new String[] {
						"door" + ",", "" + i + ",",
						"" + j + ",", "" + k + ",",
						"" + w };
					totalDoorList.add(SokobanDomain
						.arrToString(doorList));
					...
					for (int t = 0; t < shapes.size(); t++) {
					    String[] blockList = new String[] {
						    ...
						    "" + n + ",", "" + t + ",",
						    "" + k + ",", "" + w };
					    totalBlockList.add(SokobanDomain
						    .arrToString(blockList));
					}
				    }
				}
			    }
    ...
    /**
     * Creates the domain by assigning ranges to all discrete attributes,
     * associating them with object types, and associating all actions with the
     * domain
     * 
	...
	Q = new QLearning(SOKOBANDOMAIN, rf, tf, FourRooms.DISCOUNTFACTOR,
		hashFactory, 0.2, FourRooms.LEARNINGRATE, Integer.MAX_VALUE);
	...
	PropositionalFunction isCircle = new IsCirclePF(PFISCIRCLE,
		SOKOBANDOMAIN, new String[] { BLOCKCLASS });
	PropositionalFunction isStar = new IsStarPF(PFISSTAR, SOKOBANDOMAIN,
		new String[] { BLOCKCLASS });
	PropositionalFunction isMoon = new IsMoonPF(PFISMOON, SOKOBANDOMAIN,
		new String[] { BLOCKCLASS });
	PropositionalFunction isSmiley = new IsSmileyPF(PFISSMILEY,
		SOKOBANDOMAIN, new String[] { BLOCKCLASS });
	PropositionalFunction isSquare = new IsSquarePF(PFISSQUARE,
		SOKOBANDOMAIN, new String[] { BLOCKCLASS });
	PropositionalFunction inRoom = new InRoomPF(PFAGENTINROOM,
		SOKOBANDOMAIN, new String[] { AGENTCLASS, ROOMCLASS });
	PropositionalFunction inRoom2 = new InRoomPF(PFBLOCKINROOM,
		SOKOBANDOMAIN, new String[] { BLOCKCLASS, ROOMCLASS });
	PropositionalFunction isBlack = new IsBlackPF(PFISBLACK, SOKOBANDOMAIN,
		new String[] { BLOCKCLASS });
	PropositionalFunction isBlue = new IsBluePF(PFISBLUE, SOKOBANDOMAIN,
		new String[] { BLOCKCLASS });
	PropositionalFunction isCyan = new IsCyanPF(PFISCYAN, SOKOBANDOMAIN,
		new String[] { BLOCKCLASS });
	PropositionalFunction isDarkGray = new IsDarkGrayPF(PFISDARKGRAY,
		SOKOBANDOMAIN, new String[] { BLOCKCLASS });
	PropositionalFunction isGray = new IsGrayPF(PFISGRAY, SOKOBANDOMAIN,
		new String[] { BLOCKCLASS });
	PropositionalFunction isGreen = new IsGreenPF(PFISGREEN, SOKOBANDOMAIN,
		new String[] { BLOCKCLASS });
	PropositionalFunction isLightGray = new IsLightGrayPF(PFISLIGHTGRAY,
		SOKOBANDOMAIN, new String[] { BLOCKCLASS });
	PropositionalFunction isMagenta = new IsMagentaPF(PFISMAGENTA,
		SOKOBANDOMAIN, new String[] { BLOCKCLASS });
	PropositionalFunction isOrange = new IsOrangePF(PFISORANGE,
		SOKOBANDOMAIN, new String[] { BLOCKCLASS });
	PropositionalFunction isPink = new IsPinkPF(PFISPINK, SOKOBANDOMAIN,
		new String[] { BLOCKCLASS });
	PropositionalFunction isRed = new IsRedPF(PFISRED, SOKOBANDOMAIN,
		new String[] { BLOCKCLASS });
	PropositionalFunction isWhite = new IsWhitePF(PFISWHITE, SOKOBANDOMAIN,
		new String[] { BLOCKCLASS });
	PropositionalFunction isYellow = new IsYellowPF(PFISYELLOW,
		SOKOBANDOMAIN, new String[] { BLOCKCLASS });
	PropositionalFunction nextTo = new NextToPF(PFBLOCKNEXTTO,
		SOKOBANDOMAIN, new String[] { BLOCKCLASS, ROOMCLASS });
	PropositionalFunction nextTo2 = new NextToPF(PFAGENTNEXTTO,
		SOKOBANDOMAIN, new String[] { AGENTCLASS, ROOMCLASS });
	PropositionalFunction roomIsBlack = new IsBlackPF(PFROOMISBLACK,
		SOKOBANDOMAIN, new String[] { ROOMCLASS });
	PropositionalFunction roomIsBlue = new IsBluePF(PFROOMISBLUE,
		SOKOBANDOMAIN, new String[] { ROOMCLASS });
	PropositionalFunction roomIsCyan = new IsCyanPF(PFROOMISCYAN,
		SOKOBANDOMAIN, new String[] { ROOMCLASS });
	PropositionalFunction roomIsDarkGray = new IsDarkGrayPF(
		PFROOMISDARKGRAY, SOKOBANDOMAIN, new String[] { ROOMCLASS });
	PropositionalFunction roomIsGray = new IsGrayPF(PFROOMISGRAY,
		SOKOBANDOMAIN, new String[] { ROOMCLASS });
	PropositionalFunction roomIsGreen = new IsGreenPF(PFROOMISGREEN,
		SOKOBANDOMAIN, new String[] { ROOMCLASS });
	PropositionalFunction roomIsLightGray = new IsLightGrayPF(
		PFROOMISLIGHTGRAY, SOKOBANDOMAIN, new String[] { ROOMCLASS });
	PropositionalFunction roomIsMagenta = new IsMagentaPF(PFROOMISMAGENTA,
		SOKOBANDOMAIN, new String[] { ROOMCLASS });
	PropositionalFunction roomIsOrange = new IsOrangePF(PFROOMISORANGE,
		SOKOBANDOMAIN, new String[] { ROOMCLASS });
	PropositionalFunction roomIsPink = new IsPinkPF(PFROOMISPINK,
		SOKOBANDOMAIN, new String[] { ROOMCLASS });
	PropositionalFunction roomIsRed = new IsRedPF(PFROOMISRED,
		SOKOBANDOMAIN, new String[] { ROOMCLASS });
	PropositionalFunction roomIsWhite = new IsWhitePF(PFROOMISWHITE,
		SOKOBANDOMAIN, new String[] { ROOMCLASS });
	PropositionalFunction roomIsYellow = new IsYellowPF(PFROOMISYELLOW,
		SOKOBANDOMAIN, new String[] { ROOMCLASS });
	...
	isBlue.setClassName(PFCOLORCLASS);
	...
	roomIsRed.setClassName(PFCOLORCLASS);
	roomIsWhite.setClassName(PFCOLORCLASS);
	roomIsYellow.setClassName(PFCOLORCLASS);
    ...
    /**
     * Generates the first state by instantiating every object and adding it to
     * the state
     * 
	...
	ObjectInstance agent = new ObjectInstance(
		SOKOBANDOMAIN.getObjectClass(AGENTCLASS), AGENTCLASS + 0);
	ObjectInstance block = new ObjectInstance(
		SOKOBANDOMAIN.getObjectClass(BLOCKCLASS), BLOCKCLASS + 0);
	ObjectInstance block2 = new ObjectInstance(
		SOKOBANDOMAIN.getObjectClass(BLOCKCLASS), BLOCKCLASS + 1);
	ObjectInstance room = new ObjectInstance(
		SOKOBANDOMAIN.getObjectClass(ROOMCLASS), ROOMCLASS + 0);
	ObjectInstance room2 = new ObjectInstance(
		SOKOBANDOMAIN.getObjectClass(ROOMCLASS), ROOMCLASS + 1);
	ObjectInstance room3 = new ObjectInstance(
		SOKOBANDOMAIN.getObjectClass(ROOMCLASS), ROOMCLASS + 2);
	ObjectInstance door = new ObjectInstance(
		SOKOBANDOMAIN.getObjectClass(DOORCLASS), DOORCLASS + 0);
	ObjectInstance door2 = new ObjectInstance(
		SOKOBANDOMAIN.getObjectClass(DOORCLASS), DOORCLASS + 1);
    ...
    /**
     * Creates the walls using the coordinates of rooms. Each room is defined by
     * its top left and bottom right corners (including walls) Entrances are
     * allowed by door objects
     * 
     * @param st
     *            A state with defined rooms and doors from which we can
     *            generate walls
     */
     ...
     *            current state
     * @param the
     *            attempted new X position of the agent
     * @param the
     *            attempted new Y position of the agent
     */
    ...
    private static boolean moveBlock(State st, ObjectInstance block, int x,
	    int y) {
    ...
    // don't let the title deceive you: this propositional function should work
    // for both agents and blocks inside
	...
	public IsStarPF(String name, Domain domain, String parameterClasses) {
	    super(name, domain, parameterClasses);
	}
	...
	public boolean isTrue(State st, String[] params) {
	    ...
	    if (st.getObject(params[0]).getDiscValForAttribute(SHAPEATTNAME) == 0) {
		return true;
	    }
	    return false;
	}
	...
	public IsMoonPF(String name, Domain domain, String parameterClasses) {
	    super(name, domain, parameterClasses);
	}
	...
	public boolean isTrue(State st, String[] params) {
	    ...
	    if (st.getObject(params[0]).getDiscValForAttribute(SHAPEATTNAME) == 1) {
		return true;
	    }
	    return false;
	}
	...
	public IsCirclePF(String name, Domain domain, String parameterClasses) {
	    super(name, domain, parameterClasses);
	}
	...
	public boolean isTrue(State st, String[] params) {
	    ...
	    if (st.getObject(params[0]).getDiscValForAttribute(SHAPEATTNAME) == 2) {
		return true;
	    }
	    return false;
	}
	...
	public IsSmileyPF(String name, Domain domain, String parameterClasses) {
	    super(name, domain, parameterClasses);
	}
	...
	public boolean isTrue(State st, String[] params) {
	    ...
	    if (st.getObject(params[0]).getDiscValForAttribute(SHAPEATTNAME) == 3) {
		return true;
	    }
	    return false;
	}
	...
	public IsSquarePF(String name, Domain domain, String parameterClasses) {
	    super(name, domain, parameterClasses);
	}
	...
	public boolean isTrue(State st, String[] params) {
	    ...
	    if (st.getObject(params[0]).getDiscValForAttribute(SHAPEATTNAME) == 4) {
		return true;
	    }
	    return false;
	}
	...
	public IsBlackPF(String name, Domain domain, String parameterClasses) {
	    super(name, domain, parameterClasses);
	}
	...
	public boolean isTrue(State st, String[] params) {
	    ...
	    if (st.getObject(params[0]).getDiscValForAttribute(COLORATTNAME) == 0) {
		return true;
	    }
	    return false;
	}
	...
	public IsBluePF(String name, Domain domain, String parameterClasses) {
	    super(name, domain, parameterClasses);
	}
	...
	public boolean isTrue(State st, String[] params) {
	    ...
	    if (st.getObject(params[0]).getDiscValForAttribute(COLORATTNAME) == 1) {
		return true;
	    }
	    return false;
	}
	...
	public IsCyanPF(String name, Domain domain, String parameterClasses) {
	    super(name, domain, parameterClasses);
	}
	...
	public boolean isTrue(State st, String[] params) {
	    ...
	    if (st.getObject(params[0]).getDiscValForAttribute(COLORATTNAME) == 2) {
		return true;
	    }
	    return false;
	}
	...
	public IsDarkGrayPF(String name, Domain domain, String parameterClasses) {
	    super(name, domain, parameterClasses);
	}

	public IsDarkGrayPF(String name, Domain domain,
	...
	public boolean isTrue(State st, String[] params) {
	    ...
	    if (st.getObject(params[0]).getDiscValForAttribute(COLORATTNAME) == 3) {
		return true;
	    }
	    return false;
	}
	...
	public IsGrayPF(String name, Domain domain, String parameterClasses) {
	    super(name, domain, parameterClasses);
	}
	...
	public boolean isTrue(State st, String[] params) {
	    ...
	    if (st.getObject(params[0]).getDiscValForAttribute(COLORATTNAME) == 4) {
		return true;
	    }
	    return false;
	}
	...
	public IsGreenPF(String name, Domain domain, String parameterClasses) {
	    super(name, domain, parameterClasses);
	}
	...
	public boolean isTrue(State st, String[] params) {
	    ...
	    if (st.getObject(params[0]).getDiscValForAttribute(COLORATTNAME) == 5) {
		return true;
	    }
	    return false;
	}
	...
	public IsLightGrayPF(String name, Domain domain, String parameterClasses) {
	    super(name, domain, parameterClasses);
	}

	public IsLightGrayPF(String name, Domain domain,
		String[] parameterClasses) {
	    super(name, domain, parameterClasses);
	}
	...
	public boolean isTrue(State st, String[] params) {
	    ...
	    if (st.getObject(params[0]).getDiscValForAttribute(COLORATTNAME) == 6) {
		return true;
	    }
	    return false;
	}
	...
	public IsMagentaPF(String name, Domain domain, String parameterClasses) {
	    super(name, domain, parameterClasses);
	}
	...
	public boolean isTrue(State st, String[] params) {
	    ...
	    if (st.getObject(params[0]).getDiscValForAttribute(COLORATTNAME) == 7) {
		return true;
	    }
	    return false;
	}
	...
	public IsOrangePF(String name, Domain domain, String parameterClasses) {
	    super(name, domain, parameterClasses);
	}
	...
	public boolean isTrue(State st, String[] params) {
	    ...
	    if (st.getObject(params[0]).getDiscValForAttribute(COLORATTNAME) == 8) {
		return true;
	    }
	    return false;
	}
	...
	public IsPinkPF(String name, Domain domain, String parameterClasses) {
	    super(name, domain, parameterClasses);
	}
	...
	public boolean isTrue(State st, String[] params) {
	    ...
	    if (st.getObject(params[0]).getDiscValForAttribute(COLORATTNAME) == 9) {
		return true;
	    }
	    return false;
	}
	...
	public IsRedPF(String name, Domain domain, String parameterClasses) {
	    super(name, domain, parameterClasses);
	}
	...
	public boolean isTrue(State st, String[] params) {
	    ...
	    if (st.getObject(params[0]).getDiscValForAttribute(COLORATTNAME) == 10) {
		return true;
	    }
	    return false;
	}
	...
	public IsWhitePF(String name, Domain domain, String parameterClasses) {
	    super(name, domain, parameterClasses);
	}
	...
	public boolean isTrue(State st, String[] params) {
	    ...
	    if (st.getObject(params[0]).getDiscValForAttribute(COLORATTNAME) == 11) {
		return true;
	    }
	    return false;
	}
	...
	public IsYellowPF(String name, Domain domain, String parameterClasses) {
	    super(name, domain, parameterClasses);
	}
	...
	public boolean isTrue(State st, String[] params) {
	    ...
	    if (st.getObject(params[0]).getDiscValForAttribute(COLORATTNAME) == 12) {
		return true;
	    }
	    return false;
	}
	...
	public NextToPF(String name, Domain domain, String parameterClasses) {
	    super(name, domain, parameterClasses);
	}
	...
	public boolean isTrue(State st, String[] params) {
	    ...
	    int topX = st.getObject(params[1]).getDiscValForAttribute(
		    TOPXATTNAME);
	    int botX = st.getObject(params[1]).getDiscValForAttribute(
		    BOTTOMXATTNAME);
	    int topY = st.getObject(params[1]).getDiscValForAttribute(
		    TOPYATTNAME);
	    int botY = st.getObject(params[1]).getDiscValForAttribute(
		    BOTTOMYATTNAME);
	    int x = st.getObject(params[0]).getDiscValForAttribute(XATTNAME);
	    int y = st.getObject(params[0]).getDiscValForAttribute(YATTNAME);
	    if ((topX - 1) == x && y <= topY && y >= botY) {
		return true;
	    } else if ((botX + 1) == x && y <= topY && y >= botY) {
		return true;
	    } else if ((topY + 1) == y && x <= botX && x >= topX) {
		return true;
	    } else if ((botY - 1) == y && x <= botX && x >= topX) {
		return true;
	    }
	    return false;
	}
	...
	for (int i = 0; i < rooms.size(); i++) {
	    ...
	    output = output + rooms.get(i).getDiscValForAttribute(TOPYATTNAME)
		    + ",";
	    output = output
		    + rooms.get(i).getDiscValForAttribute(BOTTOMXATTNAME) + ",";
	    output = output
		    + rooms.get(i).getDiscValForAttribute(BOTTOMYATTNAME) + " ";
	}
	...
	for (int i = 0; i < doors.size(); i++) {
	    ...
	    output = output + doors.get(i).getDiscValForAttribute(TOPYATTNAME)
		    + ",";
	    output = output
		    + doors.get(i).getDiscValForAttribute(BOTTOMXATTNAME) + ",";
	    output = output
		    + doors.get(i).getDiscValForAttribute(BOTTOMYATTNAME) + " ";
	}

	for (int i = 0; i < blocks.size(); i++) {
	    output += "block,";
	    output = output
		    + blocks.get(i).getDiscValForAttribute(COLORATTNAME) + ",";
	    output = output
		    + blocks.get(i).getDiscValForAttribute(SHAPEATTNAME) + ",";
    ...
    /**
     * parses a string representation of a state into an actual State object The
     * state is described by a string in which an object is named, followed by
     * its parameters for example: room,2,0,0,10,10 rooms are defined by their
	...
	for (int i = 0; i < objects.length; i++) {
		...
		ObjectInstance room = new ObjectInstance(
			SOKOBANDOMAIN.getObjectClass(ROOMCLASS), ROOMCLASS
				+ rooms);
		room.setValue(COLORATTNAME,
			colors.get(Integer.parseInt(splitobject[1])));
		...
		ObjectInstance door = new ObjectInstance(
			SOKOBANDOMAIN.getObjectClass(DOORCLASS), DOORCLASS
				+ doors);
		...
		ObjectInstance block = new ObjectInstance(
			SOKOBANDOMAIN.getObjectClass(BLOCKCLASS), BLOCKCLASS
				+ blocks);
		block.setValue(COLORATTNAME,
			colors.get(Integer.parseInt(splitobject[1])));
		block.setValue(SHAPEATTNAME,
			shapes.get(Integer.parseInt(splitobject[2])));
		...
		ObjectInstance agent = new ObjectInstance(
			SOKOBANDOMAIN.getObjectClass(AGENTCLASS), AGENTCLASS
				+ agents);
		agent.setValue(XATTNAME, Integer.parseInt(splitobject[1]));
		agent.setValue(YATTNAME, Integer.parseInt(splitobject[2]));
		st.addObject(agent);
		agents++;
	    }
	}
    ...
    public static void runTrajectory(SokobanDomain constructor, Domain domain,
	    State st, String file) {
	BufferedReader in = null;
	String currentState = null;
	String trajectoryString = null;
	String command = null;
	...
	while (openList.size() > 0) {
		...
		// add to close list to avoid no-ops from re-adding this state
		// to the open list
		closedList.add(id);
	...
	// creates a unique ID for each state
	public static int getStateId(State s) {
	    int col = getMaxX(s);
	    int rows = getMaxY(s);
	    List<ObjectInstance> blocks = s.getObjectsOfTrueClass(BLOCKCLASS);
	    ObjectInstance agent = s.getObjectsOfTrueClass(AGENTCLASS).get(0);
	    int stateID = agent.getDiscValForAttribute(XATTNAME)
		    + agent.getDiscValForAttribute(YATTNAME) * col;
		...
		stateID += x * ((int) Math.pow(rows, i + 1))
			* ((int) Math.pow(col, i + 1));
		stateID += y * ((int) Math.pow(rows, i + 1))
			* ((int) Math.pow(col, i + 2));
	    }
	...
	public static State getStateFromId(int id, State referenceState) {
	    ...
	    List<ObjectInstance> blocks = referenceState
		    .getObjectsOfTrueClass(BLOCKCLASS);
		...
		if (i == 0) {
		    ...
		    ObjectInstance block = st.getObjectsOfTrueClass(BLOCKCLASS)
			    .get(i - 1);
