			...
			try {
				...
				for (File file : listOfFiles){
					if (file.getPath().contains(".svn") != true){
						...
						System.out.println("File \"" + file.getName() + "\" has been completed with command \"" + element.command + "\"");
					}
					//System.out.println(constructor.stateToString(element.trajectory.getState(element.trajectory.numStates() - 1)));
				}
			} catch (FileNotFoundException e) {
				System.out.println("Error: Directory \"" + args[dLocation + 1] + "\" not found.");
				System.exit(1);
			} catch (IOException e) {
				e.printStackTrace();
				System.exit(1);
