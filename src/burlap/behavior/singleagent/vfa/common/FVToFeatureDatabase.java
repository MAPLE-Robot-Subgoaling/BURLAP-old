 ...
 * {@link burlap.behavior.singleagent.vfa.StateToFeatureVectorGenerator} into a
 * {@link burlap.behavior.singleagent.vfa.FeatureDatabase}. One advantage of
 * this approach is that this can be used to automatically construct
 * state-action features. Specifically, the state features produced but the
 * {@link burlap.behavior.singleagent.vfa.StateToFeatureVectorGenerator} are
 * duplicated into unique features for each possible action. The duplication of
 * features for actions is lazy. That is, every time a previously unseen action
 * is queried in the
 * {@link #getActionFeaturesSets(burlap.oomdp.core.states.State, java.util.List)}
 * method, it gets new state features assigned to it.
 * 
...
public class FVToFeatureDatabase implements FeatureDatabase {
	...
	/**
	 * The dimensionality of the state features produced by the feature vector
	 * generator
	 */
	protected int dim;
	 ...
	 * action. Effectively this ensures a unique feature ID for each state
	 * feature for each action.
	 */
	 ...
	 *            The state feature vector generator
	 * @param dim
	 *            The dimensionality of the state features produced by the
	...
	public List<ActionFeaturesQuery> getActionFeaturesSets(State s,
		...
		List<ActionFeaturesQuery> afqs = new ArrayList<ActionFeaturesQuery>(
				actions.size());
