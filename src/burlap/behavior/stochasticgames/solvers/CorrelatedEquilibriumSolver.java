...
/**
 * This class provides static methods for solving correlated equilibrium
 * problems for Bimatrix games or values represented in a Bimatrix. Correlated
 * equilibrium provide joint strategy for the agents to follow such that there
 * would be no incentive for an agent to change their behavior if a referee
 * selected a joint action accroding to the joint strategy and informed each
 * agent of the action they should take, assuming all other agents would follow
 * their selected action.
 * <p/>
 * 4 different correlated equilibrium problems can be solved: utilitarian,
 * egalitarian, libertarian, and republican, as defined by Greenwald and Hall
 * [1]. The utilitarian objective maximizes the sum payoff of the found joint
 * strategy. Egalitarian maximizes the minimum player payoff. Republican
 * maximizes the maximum player payoff. Liberatarian maximizes the payoff for a
 * specific player. These different objectives can be specified with the
 * {@link CorrelatedEquilibriumObjective} enumeration.
 * 
 * <p/>
 * The primary method to use to get the equilibrium is the
 * {@link #getCorrelatedEQJointStrategy(CorrelatedEquilibriumObjective, double[][], double[][])}
 * method.
 * 
 * <p/>
...
public class CorrelatedEquilibriumSolver {
	...
	/**
	 * The four different equilibrium objectives that can be used: UTILITARIAN,
	 * EGALITARIAN, REPUBLICAN, and LIBERTARIAN. The utilitarian objective
	 * maximizes the sum payoff of the found joint strategy. Egalitarian
	 * maximizes the minimum player payoff. Republican maximizes the maximum
	 * player payoff. Liberatarian maximizes the payoff for a specific player.
	 * 
	 * @author James MacGlashan
	 * 
	 */
	 ...
	 * joint policy selection), the probability of all joint action variables
	 * must sum to 1, and all joint action variables are lower bound at 0.0.
	 * Note that depending on the objective, additional variables beyond the
	 * joint action variables may be part of the lp formulation. It is assumed
	 * that the first variables in the variable arrays are the sequnce of joint
	 * action variables and that any auxiliary variables that are necessary
	 * follow them. The parameter n is the total number of lp variables (joint
	 * actions and auxiliary).
	 * 
	 * @param lp
	 *            the lineary program to which the constraints will be added
	 * @param payoffRow
	 *            the payoffs for the row player
	 * @param payoffCol
	 *            the payoffs for the col player
	 * @param n
	 *            the total number of lp variables
	 * @param cCount
	 *            the number of constraints that have been added to the lp so
	 *            far
	 * @return the new number of constraints added to the lp
	 */
	protected static int addCorrelatedEquilibriumMainConstraints(
			LinearProgram lp, double[][] payoffRow, double[][] payoffCol,
			int n, int cCount) {
		...
		for (int a1 = 0; a1 < nRows; a1++) {
			...
			for (int a1prime = 0; a1prime < nRows; a1prime++) {
				...
				double[] ineq = GeneralBimatrixSolverTools.constantDoubleArray(
						0., n);
				...
				lp.addConstraint(new LinearBiggerThanEqualsConstraint(ineq, 0.,
						"c" + cCount));
				cCount++;
			}
		}
		...
		double[] eqConst = GeneralBimatrixSolverTools
				.constantDoubleArray(0., n);
		...
		// add lower bound constraints; one for each of the joint action
		// variables (not any auxiliary variables)
		for (int i = 0; i < nn; i++) {
			double[] lb = GeneralBimatrixSolverTools.zero1Array(i, n);
			lp.addConstraint(new LinearBiggerThanEqualsConstraint(lb, 0., "c"
					+ cCount));
			cCount++;
		}
	...
	/**
	 * Old code for solving the utilitarian equilibrium with JOptimizer.
	 * JOptmizer is often not able to find solutions to these problems because
	 * it uses an interior point method that requires an feasible solution on
	 * the interior and in the initial stages of stochastic games VI, this may
	 * not be the case. Therefore, this code is deprecatd, but is retained for
	 * reference.
	 * 
	 * @param payoffRow
	 *            the payoffs for the row player
	 ...
	 */
	@Deprecated
	protected static double[][] getCorrelatedEQJointStrategyUsingJOptimizer(
			double[][] payoffRow, double[][] payoffCol) {
		...
		double[] c = GeneralBimatrixSolverTools
				.getNegatedArray(getUtilitarianObjective(payoffRow, payoffCol));
		...
		// double [] lb = GeneralBimatrixSolverTools.constantDoubleArray(-1e-5,
		// n);
		...
		double[] h = GeneralBimatrixSolverTools.constantDoubleArray(0.,
				G.length);
		...
		double[] sol = roundNegativesToZero(opt.getOptimizationResponse()
				.getSolution());
	...
	/**
	 * Returns the egalitarian objective for the given payoffs for the row and
	 * column player.
	 * 
	 * @param payoffRow
	...
	/**
	 * Returns the republican/libertarian objective for the given player's
	 * payoffs that are to be maximized. The repubilican objective will use this
	 * method to solve an LP for each player. The libertarian will use this
	 * method to solve a single LP for the selected player.
	 * 
	 * @param payoffForQueryPlayer
	 *            the given player's player's payoffs that are to be maximized
	 * @return the objective function as a double array of the LP variable
	 *         coeffecients.
	 */
	public static double[] getRepublicanObjective(
			double[][] payoffForQueryPlayer) {
	...
	/**
	 * Takes an input 2D double matrix and returns a new matrix will all the all
	 * zero rows removed.
	 * 
	 * @param m
	 *            input 2D double matrix
