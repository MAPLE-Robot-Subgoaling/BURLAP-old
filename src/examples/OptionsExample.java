...
public class OptionsExample extends BasicBehavior {
	...
	// override initial state goal location to be on a hallway where options
	// can be most exploited
	...
	// creating the learning algorithm object; discount= 0.99;
	// initialQ=-90.0; learning rate=0.9
	LearningAgent agent = new QLearning(domain, rf, tf, 0.99,
		hashingFactory, -90.0, 0.9);
	...
	// creating the learning algorithm object; discount= 0.99;
	// initialQ=-90.0; learning rate=0.5; lambda=1.0 (online Monte carlo at
	// 1.0, one step at 0.0)
	LearningAgent agent = new SarsaLam(domain, rf, tf, 0.99,
		hashingFactory, -90.0, 0.5, 1.0);
	this.addRoomsOptionsToPlanner((OOMDPPlanner) agent);
	...
	planner.addNonDomainReferencedAction(this.getRoomOption("blt", 0, 4, 0,
		4, 1, 5));
	planner.addNonDomainReferencedAction(this.getRoomOption("blr", 0, 4, 0,
		4, 5, 1));

	planner.addNonDomainReferencedAction(this.getRoomOption("tlr", 0, 4, 6,
		10, 5, 8));
	planner.addNonDomainReferencedAction(this.getRoomOption("tlb", 0, 4, 6,
		10, 1, 5));

	planner.addNonDomainReferencedAction(this.getRoomOption("trb", 6, 10,
		5, 10, 8, 4));
	planner.addNonDomainReferencedAction(this.getRoomOption("trl", 6, 10,
		5, 10, 5, 8));

	planner.addNonDomainReferencedAction(this.getRoomOption("brt", 6, 10,
		0, 3, 8, 4));
	planner.addNonDomainReferencedAction(this.getRoomOption("brl", 6, 10,
		6, 3, 5, 1));
	...
	StateConditionTestIterable inRoom = new InRoomStateCheck(leftBound,
		rightBound, bottomBound, topBound);
	...
	// OOMDPPlanner planner = new ValueIteration(domain, new
	// LocalSubgoalRF(inRoom, atHallway, 0., 0., 1.), new
	// LocalSubgoalTF(inRoom, atHallway), 0.99, hashingFactory, 0.001, 50);
	OOMDPPlanner planner = new ValueIteration(domain, new LocalSubgoalRF(
		inRoom, atHallway), new LocalSubgoalTF(inRoom, atHallway),
		0.99, hashingFactory, 0.001, 50);
	...
	public InRoomStateCheck(int leftBound, int rightBound, int bottomBound,
		int topBound) {
	...
	public boolean satisfies(State s) {
	    ...
	    ObjectInstance agent = s.getObjectsOfTrueClass(
		    GridWorldDomain.CLASSAGENT).get(0); // get the agent object
	...
	public Iterator<State> iterator() {
		...
		public State next() {
		    ...
		    State s = GridWorldDomain.getOneAgentNLocationState(domain,
			    0);
