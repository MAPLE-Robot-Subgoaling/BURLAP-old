 ...
 * input a single agent
 * {@link burlap.behavior.singleagent.learning.LearningAgent} to handle
 * behavior. The interface from the single agent paradigm to the multi-agent
 * paradigm is handled by this class also implementing the
 * {@link burlap.oomdp.singleagent.environment.Environment} interface. When a
 * game starts, a new thread is launched in which the provided
 * {@link burlap.behavior.singleagent.learning.LearningAgent} interacts with
 * this class's {@link burlap.oomdp.singleagent.environment.Environment}
 * methods. <br/>
 * <br/>
 * When constructing a
 * {@link burlap.behavior.singleagent.learning.LearningAgent} to use with this
 * class, you should set its {@link burlap.oomdp.core.Domain} to null. Then,
 * when this class joins a world through the
 * {@link #joinWorld(burlap.oomdp.stochasticgames.World, burlap.oomdp.stochasticgames.SGAgentType)}
 * method, it will automatically use the
 * {@link burlap.behavior.stochasticgames.agents.interfacing.singleagent.SGToSADomain}
 * to create a {@link burlap.oomdp.singleagent.SADomain} and will then set then
 * {@link burlap.behavior.singleagent.learning.LearningAgent} to use it.
 * 
...
public class LearningAgentToSGAgentInterface extends SGAgent implements
	...
	/**
	 * The single agent
	 * {@link burlap.behavior.singleagent.learning.LearningAgent} that will be
	 * learning in this stochastic game as if the other players are part of the
	 * environment.
	 */
	 ...
	 *            {@link burlap.oomdp.stochasticgames.SGDomain} in which this
	 *            agent will interact.
	 * @param learningAgent
	 *            the {@link burlap.behavior.singleagent.learning.LearningAgent}
	 *            that will handle this
	 *            {@link burlap.oomdp.stochasticgames.SGAgent}'s control.
	 */
	...
	public GroundedSGAgentAction getAction(State s) {
		...
		if (this.saThread == null) {
			this.saThread = new Thread(new Runnable() {
				...
				public void run() {
					learningAgent
							.runLearningEpisode(LearningAgentToSGAgentInterface.this);
				}
			});
			this.saThread.start();
		}
