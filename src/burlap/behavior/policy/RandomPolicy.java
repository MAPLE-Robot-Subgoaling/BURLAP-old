 ...
 * between which it randomly selects by providing a domain (from which the
 * domains primitive actions are copied into an internal list) or from a list of
 * Action objects (from which the action references are copied into an internal
 * list). You may also add additional actions with the
 * {@link #addAction(burlap.oomdp.singleagent.Action)} method or remove or clear
 * the actions. <br/>
 * Upon action selection, all applicable grounded actions for the state are
 * generated and an action is selected uniformly randomly from them. The policy
 * is not defined if there are no applicable actions.
 */
public class RandomPolicy extends Policy {
	...
	/**
	 * Initializes by copying all the primitive actions references defined for
	 * the domain into an internal action list for this policy.
	 * 
	 * @param domain
	 *            the domain containing all the primitive actions.
	 */
	...
	/**
	 * Initializes by copying all the actions references defined in the provided
	 * list into an internal action list for this policy.
	 * 
	 * @param acitons
	 *            the actions to select between.
	 */
	 ...
	 * 
	 * @param action
	 *            an action to consider in selection
	 */
	...
	/**
	 * Clears the action list used in action selection. Note that if no actions
	 * are added to this policy after calling this method then the policy will
	 * be undefined everywhere.
	 */
	...
	public AbstractGroundedAction getAction(State s) {
		...
		GroundedAction selection = gas.get(this.rand.nextInt(this.actions
				.size()));
		return selection;
	}
