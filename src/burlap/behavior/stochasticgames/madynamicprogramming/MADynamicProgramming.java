...
/**
 * An abstract value function based planning algorithm base for sequential
 * stochastic games that require the computation of Q-values for each agent for
 * each joint action. Value function updates are performed using a Bellman-like
 * backup operator; however, planning for different solution concepts is
 * achieved by providing different backup operators via the
 * {@link SGBackupOperator} object.
 * <p/>
 * Note that the agent definitions can only be changed up until planning begins.
 * Once planning has begun, the agent definitions must remain fixed for
 * consistency of planning results. If the the client tries to change the agent
 * definitions after planning has already begun, then a runtime exception will
 * be thrown.
 * <p/>
 * Since value function planning algorithms compute multi-agent Q-values, this
 * object implements the {@link MultiAgentQSourceProvider} interface.
 * 
...
public abstract class MADynamicProgramming implements MultiAgentQSourceProvider {
	 ...
	 * function for an agent and produces Joint action Q-values by marginalizing
	 * over the transition dynamics the reward and discounted next state value.
	 * 
	 * @author James MacGlashan
	 * 
	 */
	protected class BackupBasedQSource implements QSourceForSingleAgent {
		 ...
		 * 
		 * @param agentName
		 *            the name of the agent for which the value function
		 *            corresponds.
		 */
		...
		public JAQValue getQValueFor(State s, JointAction ja) {
			...
			if (!MADynamicProgramming.this.terminalFunction.isTerminal(s)) {
				...
				for (int i = 0; i < jat.tps.size(); i++) {
					...
					double p = tp.p;
					HashableState sh = MADynamicProgramming.this.hashingFactory
							.hashState(tp.s);
					double r = jat.jrs.get(i).get(this.agentName);
					double vprime = this.getValue(sh);

					double contribution = r
							+ MADynamicProgramming.this.discount * vprime;
		...
		/**
		 * Returns the stored state value for hashed state sh, or creates an
		 * entry with an initial value corresponding to the
		 * {@link MADynamicProgramming} instance's value function initialization
		 * object and returns that value if the the quried state has never
		 * previously been indexed.
		 * 
		 * @param sh
		 *            the state for which the value is to be returned.
		 ...
		 * 
		 * @param sh
		 *            the hashed state for which the value is to be set
		 * @param v
