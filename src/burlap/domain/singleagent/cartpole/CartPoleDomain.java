...
/**
 * The classic cart pole balancing problem as described by Barto, Sutton, and
 * Anderson [2] with correct mechanics as described by Florian [1]. The agent
 * controls a cart that can apply horizontal force in either direction. Attached
 * to the cart is a pole on a hinge and the goal of the agent is to apply force
 * to the cart such that the pole stays vertically balanced. If the angle
 * between the pole and the vertical axis is greater than some threshold
 * (originally 12 degrees or about 0.2 radians), the agent fails. The track on
 * which the cart can move is also finite in size, and running into the end of
 * the track is also considered failure; however, the track can be set to
 * infinite by setting the {@link #physParams} isFiniteTrack parameter to false.
 * The infinite track is handled by never changing the position value of the
 * cart. All model/physics parameters are stored in the
 * {@link burlap.domain.singleagent.cartpole.CartPoleDomain.CPPhysicsParams}
 * object {@link #physParams}. Modifying this generator's model parameters will
 * not affected previously generated domains, so the same generator can be used
 * to generate different domains without affecting others.
 * <p/>
 * By default, this implementation will use the simulation described by Florian,
 * which corrects two problems in the classic Barto, Sutton, and Anderson paper.
 * The two problems were (1) gravity was specified as negative in the equations
 * when it should have been positive and (2) friction was not calculated
 * correctly. However, this domain may also be set to use the classic incorrect
 * mechanics or the classic mechanics with correct gravity for comparison
 * purposes. To do so, use the methods {@link #setToIncorrectClassicModel()} and
 * {@link #setToIncorrectClassicModelWithCorrectGravity()}. Note that when
 * incorrect gravity is used, the pole will "bounce" once it reaches about 90
 * degrees (though in most tasks the pole is never allowed to fall this far).
 * <p/>
 * This domain consists of a single object with 4 real valued attributes: the x
 * position of the cart, the x velocity of the cart, the angle between the pole
 * and the vertical axis, and the speed of the change in angle. Additionally, a
 * 5th hidden attribute is included when the corrected physics are used that
 * maintains the sign of the normal force in the last step. If the classic
 * mechanics are used instead, then this hidden attribute is not included. The
 * physics are simulated using a non-linear differential equation that is
 * estimated using Euler's Method. All system parameters are defaulted to those
 * used in the original paper, but they may modified as desired.
 * <p/>
 * Also included with this class are default classes for reward function and
 * terminal function for this domain.
 * <p/>
 * Running the main method of this class will launch and interactive visualizer
 * with the 'a' and 'd' keys controlling left and right movement force
 * respectively.
 * 
 * 
 * 1. Florian, Razvan V.
 * "Correct equations for the dynamics of the cart-pole system." Center for
 * Cognitive and Neural Studies (Coneural), Romania (2007). 2. Barto, Andrew G.,
 * Richard S. Sutton, and Charles W. Anderson.
...
public class CartPoleDomain implements DomainGenerator {
	...
	public static class CPPhysicsParams {
		...
		/**
		 * The maximum radius the pole can fall. Note, physics get weird and
		 * non-realisitc at pi/2; task should terminate before then.
		 */
		...
		/**
		 * The force magnatude that can be exterted in either direction on the
		 * cart
		 */
		...
		/**
		 * The maximum speed of the cart. A good place to set it to is somewhere
		 * around the max expected if the cart was pushed from one side of the
		 * track to the other. The default is 6.81 (which is an upperbound of
		 * that with default parameters)
		 */
		public double maxCartSpeed = 6.81;

		/**
		 * The maximum speed of the change in angle. The default sets it to the
		 * speed that would result in 12 degress for a sinlge time step of 0.02
		 * (which is about 10.47 radians/second), since 12 degrees is the
		 * default termination range.
		 */
		public double maxAngleSpeed = 10.47; // 12 degrees per time step of 0.02
												// seconds

		/**
		 * Whether the track is finite (true) or infinite (false). When the
		 * track is infinite, the position of the cart always remains the same.
		 */
		public boolean isFiniteTrack = true;

		/**
		 * Specifies whether the correct Cart Pole physical model should be used
		 * or the classic, but incorrect, Barto Sutton and Anderson model [1].
		 */
		...
		public CPPhysicsParams(double halfTrackLength, double angleRange,
				double gravity, double cartMass, double poleMass,
				double halfPoleLength, double cartFriction,
				double poleFriction, double movementForceMag, double timeDelta,
				double maxCartSpeed, double maxAngleSpeed,
				boolean isFiniteTrack, boolean useCorrectModel) {
		...
		public CPPhysicsParams copy() {
			return new CPPhysicsParams(halfTrackLength, angleRange, gravity,
					cartMass, poleMass, halfPoleLength, cartFriction,
					poleFriction, movementForceMag, timeDelta, maxCartSpeed,
					maxAngleSpeed, isFiniteTrack, useCorrectModel);
		}
	}
	...
	/**
	 * Returns the default initial state: the cart centered on the track, not
	 * moving, with the pole perfectly vertical.
	 * 
	 * @param domain
	...
	/**
	 * Returns an initial state with the given initial values for the cart and
	 * pole.
	 * 
	 * @param domain
	 *            the domain object to which the state will be associated.
	 ...
	 * @param av
	 *            the velocity of the angle
	 * @return the corresponding initial state object
	 */
	public static State getInitialState(Domain domain, double x, double xv,
			double a, double av) {
		ObjectInstance cartPole = new MutableObjectInstance(
				domain.getObjectClass(CLASSCARTPOLE), CLASSCARTPOLE);
	...
	/**
	 * Simulates the physics for one time step give the input state s, and the
	 * direction of force applied. The input state will be directly modified to
	 * be the next state. Physics simulated using one step of Euler's method on
	 * the non-linear differential equations provided by Barto Sutton, and
	 * Anderson [2]. <b>Note that this model is not physically correct [1] but
	 * is left in code for historical comparisons</b>. Optionally a correct
	 * model can be used instead.
	 * 
	 * @param s
	 *            the current state from which one time step of physics will be
	 *            simulated.
	 * @param dir
	 *            the direction of force applied; should be -1, or 1 and is
	 *            multiplied to this objects movementForceMag parameter. 0 would
	 *            cause no force.
	 * @param physParams
	 *            the
	 *            {@link burlap.domain.singleagent.cartpole.CartPoleDomain.CPPhysicsParams}
	 *            object specifying the physics to use for movement
	 * @return the input state, which has been modified to the next state after
	 *         one time step of simulation.
	 */
	public static State moveClassicModel(State s, double dir,
		...
		double anumCosFactor = (-f
				- (physParams.poleMass * physParams.halfPoleLength * av0 * av0 * Math
						.sin(a0)) + (physParams.cartFriction * Math.signum(xv0)))
				/ sMass;
		double anumPFricTerm = (physParams.poleFriction * av0)
		...
		double adenom = physParams.halfPoleLength
				* ((4. / 3.) - ((physParams.poleMass * Math.pow(Math.cos(a0),
						2.)) / sMass));

		double a_2 = anum / adenom;

		double xnum = f + physParams.poleMass * physParams.halfPoleLength
				* (av0 * av0 * Math.sin(a0) - a_2 * Math.cos(a0))
	 ...
	 * direction of force applied. The input state will be directly modified to
	 * be the next state. Physics simulated using one step of Euler's method on
	 * the corrected non-linear differential equations [1].
	 * 
	 * @param s
	 *            the current state from which one time step of physics will be
	 *            simulated.
	 ...
	 *         one time step of simulation.
	 */
	public static State moveCorrectModel(State s, double dir,
			CPPhysicsParams physParams) {
