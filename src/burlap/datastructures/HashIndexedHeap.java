...
/**
 * An implementation of a hash map backed heap/priority queue. This data
 * structure allows efficient O(1) "contains" checks and efficient O(lg(n))
 * modifications of entries already in the heap that with a typical Java
 * PriorityQueue would require linear updates. This data structure is especially
 * useful for planning algorithms like A* that keep a priority queue of states,
 * but may need to update their priority if a better path to them is found.
 * <p/>
 * By default, the heap is a max heap (which results in elements with the
 * highest priority being dequeued first), but it may also be set to be a min
 * heap.
 * 
...
public class HashIndexedHeap<T> {
    ...
    /**
     * If true, this is ordered according to a max heap; if false ordered
     * according to a min heap.
     */
     ...
     * 
     * @param pcompare
     *            the comparator to compare the priority of elements of this
     *            heap
     */
     ...
     *            heap
     * @param capacity
     *            the initial compacity size of the heap
     */
    ...
    /**
     * Checks if the heap contains this object and returns the pointer to the
     * stored object if it does; otherwise null is returned.
     * 
     * @param inst
     *            the inst to look for in the heap
     * @return the pointer to the stored object if it is in the heap; null
     *         otherwise.
     */
     ...
     * 
     * @param i
     *            the index on which to perform heapify
