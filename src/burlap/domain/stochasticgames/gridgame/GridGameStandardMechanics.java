 ...
 * default, the dynamics set a 0.5 probability of agents being able to move
 * through semi-walls. The semi-wall pass through probability, however, may be
 * changed by a parameter to a constructor. If two agents adjacent agents try to
 * move into each others locations, no one will move. If an agent tries to move
 * into a cell that the other agent is leaving, then they both will move. If two
 * agents try to move into the same location, then only one of them will
 * randomly make it, while the other will be unmoved in their position.
 * 
...
public class GridGameStandardMechanics extends JointActionModel {
	...
	/**
	 * Initializes the mechanics for the given domain and sets the semi-wall
	 * pass through probability to 0.5;
	 * 
	 * @param d
	 *            the domain object
	 */
	...
	/**
	 * Initializes the mechanics for the given domain and sets the semi-wall
	 * pass through probability to semiWallPassThroughProb.
	 * 
	 * @param d
	 *            d the domain object
	 * @param semiWallPassThroughProb
	...
	protected State actionHelper(State s, JointAction ja) {
		...
		for (int i = 0; i < ja.size(); i++) {
			...
			basicMoveResults.add(this.sampleBasicMovement(s, loc,
					this.attemptedDelta(gsa.action.actionName), noopLocations));
		}

		// resolve swaps
		basicMoveResults = this.resolvePositionSwaps(previousLocations,
				basicMoveResults);

		List<Location2> finalPositions = this.resolveCollisions(
				previousLocations, basicMoveResults);
	 ...
	 *            the original position of agents in the previous state
	 * @param desiredPositions
	 *            the desired position where the agents want to go
	 * @param noops
	 *            the locations in which agents have been forced to stay from
	 *            other events
	 * @return the new resolved locations of all agents
	 */
	List<Location2> backupNoOps(List<Location2> originalPositions,
			List<Location2> desiredPositions, List<Location2> noops) {

		List<Location2> resolved = new ArrayList<GridGameStandardMechanics.Location2>(
				desiredPositions);
	 ...
	 * 
	 * @param candidatePositions
	 *            a list of the positions to which each agent would like to go;
	 *            the Location2 in index 1 indicates the location the 1th index
	 *            agent wants to go to.
	 * @return A map from the index of each agent to a list of the index of
	 *         agents with whom there is competition for a cell
	 */
	protected Map<Integer, List<Integer>> getColissionSets(
			List<Location2> candidatePositions) {
		...
		for (int i = 0; i < candidatePositions.size(); i++) {
			...
			if (collisions.size() > 1) { // greater than one because an agent
	 ...
	 * 
	 * @param s
	 *            the state in which the changes would occur
	 * @param p0
	...
	protected List<Location2Prob> getPossibleLocationsFromWallCollisions(
			State s, Location2 p0, Location2 delta,
			List<Location2> agentNoOpLocs) {

		List<Location2Prob> locs = new ArrayList<GridGameStandardMechanics.Location2Prob>(
				2);
	 ...
	 * @param delta
	 *            the desired change in direction of the agent
	 * @param walls
	 *            the list of walls in the world
	 * @param vertical
	 *            whether the list of provided walls are vertical or horizontal
	 *            walls
	 * @return true if the agent is able to move in the desired location; false
	 *         otherwise
	 */
	protected boolean sampleWallCollision(Location2 p0, Location2 delta,
			List<ObjectInstance> walls, boolean vertical) {
	 ...
	 * @param vertical
	 *            true if the wall objects are vertical; false if they are
	 *            horizontal
	 * @return 0 if there is no collision with a wall, 1 if there is a collision
	 *         with a solid wall, 2 if there is a potential collision with a
	 *         semi-wall
	 */
	protected int wallCollision(Location2 p0, Location2 delta,
			List<ObjectInstance> walls, boolean vertical) {
