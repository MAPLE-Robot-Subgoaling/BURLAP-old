 ...
 * useful when the source policy may require some degree of computation and
 * you'd like to store the results of that computation to memory so that it does
 * not have be recomputed for states that have their policy queried multiple
 * times.
 * 
...
public class CachedPolicy extends Policy {
	 ...
	 * @param sourcePolicy
	 *            the source policy that will be lazily cached.
	 */
	 ...
	 *            cache
	 */
	public CachedPolicy(HashableStateFactory hashingFactory,
			Policy sourcePolicy, int cacheCapacity) {
		this.hashingFactory = hashingFactory;
		this.sourcePolicy = sourcePolicy;
		this.actionSelection = new HashMap<HashableState, List<ActionProb>>(
				cacheCapacity);
	}
