...
/**
 * A single action is an action specification for individual agents in a
 * stochastic game. The single action specifies preconditions and any parameters
 * that are required for it to operate. Like The
 * {@link burlap.oomdp.singleagent.Action} class and
 * {@link burlap.oomdp.core.PropositionalFunction} class, SingleAction's can
 * also specify parameter order groups for parameter effect symmetry. See the
 * {@link burlap.oomdp.core.PropositionalFunction} class documentation for a
 * larger discussion on parameter order groups.
 * 
 * Unlike The {@link burlap.oomdp.singleagent.Action} class, SingleAction's do
 * not specify the actual transition dynamics, because transition dynamics in a
 * stochastic game depend on the full joint action of all agents (that is, the
 * SingleAction selected by each agent in the world).
 * 
...
public abstract class SingleAction {
     ...
     * @param actingAgent
     *            the agent who would be executing the action
     * @param params
     *            the parameters applied
     * @return true if this action can be executed; false otherwise.
     */
    public abstract boolean isApplicableInState(State s, String actingAgent,
	    String[] params);
    ...
    /**
     * Initializes this single action to be for the given domain, with the given
     * name, and with the given parameter class types. This action is
     * automatically added to the given domain
     * 
     * @param d
     *            the domain to which this action belongs
     * @param name
     ...
     *            the object class names for the possible parameters of this
     *            action.
     */
    ...
    /**
     * Initializes this single action to be for the given domain, with the given
     * name, with the given parameter class types, and with the given parameter
     * order groups. This action is automatically added to the given domain
     * 
     * @param d
     *            the domain to which this action belongs
     * @param name
     ...
     *            the parameter order groups to use
     */
     ...
     */
    public static List<GroundedSingleAction> getAllPossibleGroundedSingleActions(
	    State s, String actingAgent, List<SingleAction> actions) {
     ...
     *         state and acting agent.
     */
    public List<GroundedSingleAction> getAllGroundedActionsFor(State s,
	    String actingAgent) {
	...
	if (this.parameterTypes.length == 0) {
		...
		res.add(new GroundedSingleAction(actingAgent, this,
			new String[] {}));
	    }
	...
	List<List<String>> bindings = s
		.getPossibleBindingsGivenParamOrderGroups(this.parameterTypes,
			this.parameterOrderGroups);
