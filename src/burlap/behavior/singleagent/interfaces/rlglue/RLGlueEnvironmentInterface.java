...
/**
 * A BURLAP {@link burlap.oomdp.singleagent.environment.Environment} that
 * manages the interface to an RLGlue Environment. To implement this interface,
 * it requires that this class is both a BURLAP
 * {@link burlap.oomdp.singleagent.environment.Environment} and an RLGlue
 * {@link org.rlcommunity.rlglue.codec.AgentInterface}, since BURLAP agents will
 * communicate to the RL Glue Environment through the an RLGlue
 * {@link org.rlcommunity.rlglue.codec.AgentInterface}. However, you can use
 * this class like a normal BURLAP
 * {@link burlap.oomdp.singleagent.environment.Environment}. <br/>
 * <br/>
 * To initialize this environment, use the {@link #loadAgent()} or
 * {@link #loadAgent(String, String)} methods so that RLGlue knows to tell this
 * class about the environment this class is wrapping. After the load method,
 * you can get the corresponding BURLAP domain that represents it by calling the
 * {@link #getDomain()} method. You can also get the RLGlue preferred discount
 * factor using the {@link #getDiscountFactor()} method. <br/>
 * <br/>
 * As with normal BURLAP
 * {@link burlap.oomdp.singleagent.environment.Environment} implementations, you
 * should call the {@link #resetEnvironment()} method whenever a terminal state
 * is reached. This will cause this class to block until RLGlue has started a
 * new episode with a new initial state, unless the RLGlue experiment has
 * finished, in which case the method will return immediately. If you want to
 * check whether the RLGLue experiment has finished manually, use the
 * {@link #rlGlueExperimentFinished} method.
 * 
...
public class RLGlueEnvironmentInterface implements Environment, AgentInterface {
	...
	/**
	 * The BURLAP {@link Domain} specifying the RLGlue problem representation
	 * and action space.
	 */
	...
	/**
	 * Loads this RLGlue {@link org.rlcommunity.rlglue.codec.AgentInterface}
	 * into RLGlue and runs its event loop in a separate thread.
	 */
	...
	/**
	 * Loads this RLGlue {@link org.rlcommunity.rlglue.codec.AgentInterface}
	 * into RLGlue using the specified host address and port nd runs its event
	 * loop in a separate thread.
	 * 
	 * @param hostAddress
	 *            the RLGlue host address.
	 * @param portString
	 *            the port on which to connect to RLGlue.
	 */
	public void loadAgent(String hostAddress, String portString) {
		DPrint.toggleCode(debugCode, this.printDebug);
		final AgentLoader loader = new AgentLoader(hostAddress, portString,
				this);
		Thread eventThread = new Thread(new Runnable() {
			@Override
			public void run() {
	...
	/**
	 * Returns the domain for this environment. This method will block until the
	 * domain for this environment is set by RLGLue via the
	 * {@link #agent_init(String)} method, which means you ought to have called
	 * {@link #loadAgent()} or {@link #loadAgent(String, String)} before calling
	 * this method, otherwise RLGlue will not know to set the environment.
	 * 
	 * @return the BURLAP {@link burlap.oomdp.core.Domain} specification for
	 *         this RLGlue environment.
	 */
	...
	/**
	 * Returns the discount factor for this environment. This method will block
	 * until the domain for this environment is set by RLGLue via the
	 * {@link #agent_init(String)} method, which means you ought to have called
	 * {@link #loadAgent()} or {@link #loadAgent(String, String)} before calling
	 * this method, otherwise RLGlue will not know to set the environment.
	 * 
	...
	public Action agent_start(Observation observation) {
		...
		synchronized (nextStateReference) {
			...
			this.lastReward = 0.;
			final State s = RLGlueDomain.stateFromObservation(domain,
					observation);
			this.curState = s;
	...
	public Action agent_step(double v, Observation observation) {
		...
		synchronized (nextStateReference) {
			nextStateReference.val = RLGlueDomain.stateFromObservation(
	...
	public EnvironmentOutcome executeAction(GroundedAction ga) {
		...
		if (!(ga.action instanceof RLGlueDomain.RLGlueActionSpecification)) {
			throw new RuntimeException(
					"RLGlueEnvironment cannot execute actions that are not instances of RLGlueDomain.RLGlueSpecification.");
		}

		State prevState = this.curState;

		int actionId = ((RLGlueDomain.RLGlueActionSpecification) ga.action)
				.getInd();
		synchronized (nextAction) {
			this.nextStateReference.val = null;
		...
		EnvironmentOutcome eo = new EnvironmentOutcome(prevState, ga, toRet,
				this.lastReward, this.curStateIsTerminal);
	 ...
	 */
	public static org.rlcommunity.rlglue.codec.types.Action getRLGlueAction(
			int id) {
