...
public class RBFFeatureDatabase implements FeatureDatabase {
    ...
    /**
     * A map for return a multiplier to the number of RBF statefeatures for each
     * action. Effecitively this ensures a unieque feature ID fo reach RBF for
     * each action.
     */
	...
	if (ga.isParameterized()) {
	    throw new RuntimeException(
		    "RBF Feature Database does not support parameterized actions.");
	}

	Integer stored = this.actionFeatureMultiplier.get(ga.actionName());
	if (stored == null) {
	    this.actionFeatureMultiplier.put(ga.actionName(),
