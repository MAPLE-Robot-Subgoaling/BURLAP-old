 ...
 * Typically this should be unnecessary because options and actions can coexist
 * in a planner, but if an algorithm wishes to homogenize its interface it may
 * be useful for it to wrap all of the primitives as options.
 * 
...
public class PrimitiveOption extends Option {
    ...
    /**
     * Creates an option wrapper for a given primitive action. This option will
     * take on the same name as the primitive action.
     * 
     * @param srcAction
     *            the primitive action to wrap.
     */
    public PrimitiveOption(Action srcAction) {
	this.srcAction = srcAction;
	this.init(srcAction.getName(), srcAction.getDomain(),
		srcAction.getParameterClasses(),
		srcAction.getParameterOrderGroups());
    }
