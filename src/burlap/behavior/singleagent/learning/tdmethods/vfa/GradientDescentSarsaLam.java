 ...
 * which is performed by running many learning episodes in succession. The
 * number of episodes used for planning can be determined by a threshold maximum
 * number of episodes, or by a maximum change in the VFA weight threshold.
 * 
 ...
 * 
 */
public class GradientDescentSarsaLam extends OOMDPPlanner implements
	QComputablePlanner, LearningAgent {
     ...
     */
    public GradientDescentSarsaLam(Domain domain, RewardFunction rf,
	    TerminalFunction tf, double gamma, ValueFunctionApproximation vfa,
	    double learningRate, double lambda) {

	this.GDSLInit(domain, rf, tf, gamma, vfa, learningRate,
		new EpsilonGreedy(this, 0.1), Integer.MAX_VALUE, lambda);
	...
	this.GDSLInit(domain, rf, tf, gamma, vfa, learningRate,
		new EpsilonGreedy(this, 0.1), maxEpisodeSize, lambda);
     ...
     * will cause the planner to use only one episode for planning; this should
     * probably be changed to a much larger value if you plan on using this
	...
	this.GDSLInit(domain, rf, tf, gamma, vfa, learningRate, learningPolicy,
     ...
     * @param gamma
     *            the discount factor
     * @param ValueFunctionApproximation
     *            the value function approximation method to use for estimate
     *            Q-values
     * @param learningRate
     *            the learning rate
     ...
     *            1 for full propagation)
     */
    protected void GDSLInit(Domain domain, RewardFunction rf,
	    TerminalFunction tf, double gamma, ValueFunctionApproximation vfa,
	    double learningRate, Policy learningPolicy, int maxEpisodeSize,
	    double lamda) {
	...
	while (!tf.isTerminal(curState) && eStepCounter < maxSteps) {
	    ...
	    double learningRate = this.learningRate.pollLearningRate(curState,
		    action);
